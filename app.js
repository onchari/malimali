...
const KEY_LAST_VIEW = "mg_last_view";
...
function initDB() {
  ...
  DB_NAME =
    getFirebaseEnv() === "development" ? "InventoryApp_dev" : "InventoryApp";
  ...
  req.onupgradeneeded = (e) => {
    ...
    if (!d.objectStoreNames.contains("items")) {
      const s = d.createObjectStore("items", {
        keyPath: "id",
        autoIncrement: true,
      });
    ...
    if (!d.objectStoreNames.contains("shoe_sizes")) {
      const ss = d.createObjectStore("shoe_sizes", {
        keyPath: "id",
        autoIncrement: true,
      });
    ...
    if (!d.objectStoreNames.contains("sales")) {
      const sa = d.createObjectStore("sales", {
        keyPath: "id",
        autoIncrement: true,
      });
    ...
    if (!d.objectStoreNames.contains("finances")) {
      const fi = d.createObjectStore("finances", {
        keyPath: "id",
        autoIncrement: true,
      });
    ...
    if (!d.objectStoreNames.contains("business_days")) {
      const bd = d.createObjectStore("business_days", {
        keyPath: "id",
        autoIncrement: true,
      });
    ...
    if (!d.objectStoreNames.contains("wishlist")) {
      const wl = d.createObjectStore("wishlist", {
        keyPath: "id",
        autoIncrement: true,
      });
    ...
    // Purchase budgets for planned and actual stock spending.
    if (!d.objectStoreNames.contains("purchase_budgets")) {
      const pb = d.createObjectStore("purchase_budgets", {
        keyPath: "id",
        autoIncrement: true,
      });
      pb.createIndex("idx_status", "status", { unique: false });
      pb.createIndex("idx_start_date", "startDate", { unique: false });
      pb.createIndex("idx_fbid", "fbId", { unique: false });
    }
    ...
    if (!d.objectStoreNames.contains("sync_queue")) {
      const sq = d.createObjectStore("sync_queue", {
        keyPath: "id",
        autoIncrement: true,
      });
    ...
    if (old >= 13 && d.objectStoreNames.contains("sync_queue")) {
      ...
      if (!sq.indexNames.contains("idx_op"))
        sq.createIndex("idx_op", "opId", { unique: false });
      if (!sq.indexNames.contains("idx_next_attempt"))
        sq.createIndex("idx_next_attempt", "nextAttemptAt", { unique: false });
    }
    ...
    if (old < 12) {
      if (!d.objectStoreNames.contains("customers")) {
        const cu = d.createObjectStore("customers", {
          keyPath: "id",
          autoIncrement: true,
        });
      ...
      if (!d.objectStoreNames.contains("customer_txns")) {
        const ct = d.createObjectStore("customer_txns", {
          keyPath: "id",
          autoIncrement: true,
        });
  ...
  req.onsuccess = (e) => {
    db = e.target.result;
    db.onerror = (ev) =>
      console.error("[DB] Unhandled error:", ev.target.error);
      ...
      .then(async () => {
        ...
        if (db.objectStoreNames.contains("stock_moves")) {
          try {
            ...
            console.warn(
              "[stock-ledger] startup migration failed:",
              ledgerError.message,
            );
          }
        }
        ...
        try {
          ...
          console.warn(
            "[SYNC] wishlist backfill failed:",
            syncBackfillError.message,
          );
        }
        setLoginReady(true);
        ...
        if (sessionRestored && currentUser) {
          ...
          _origShowPage(
            resolveLandingPage(
              currentUser,
              localStorage.getItem(KEY_LAST_PAGE),
            ),
          );
        }
      })
...
function _nextHlc() {
  ...
  const counter =
    wall === Number(state.wall || 0) ? Number(state.counter || 0) + 1 : 0;
...
async function recomputeDerivedQtyForItem(itemId, sizeId = null) {
  ...
  if (duty !== null && db.objectStoreNames.contains("shoe_sizes")) {
    ...
    item.qtyCache = sizes
      ...
      .reduce(
        (total, record) => total + Number(record.qtyCache ?? record.qty ?? 0),
        0,
      );
  } else {
...
async function appendStockMove({
  ...
  await recordStockAudit("stock_move_written", {
    itemId,
    sizeId,
    delta,
    reason,
...
function persistSaleAtomically({
  item,
  shoeSize = null,
  sale,
  delta,
  ...
  return new Promise((resolve, reject) => {
    ...
    const hasShoeSize =
      !!shoeSize && db.objectStoreNames.contains("shoe_sizes");
    const hasLedger =
    ...
    try {
      ...
      const finalize = () => {
        if (finalized || saleId == null || (syncable && state === undefined))
          return;
        finalized = true;
        sale.id = saleId;
        ...
        const derivedQty = matchingMoves.length
          ? matchingMoves.reduce(
              (total, existing) => total + Number(existing.delta || 0),
              0,
            ) + Number(delta)
      ...
      tx.onabort = () =>
        reject(tx.error || new Error("Sale transaction aborted"));
      tx.oncomplete = () => {
...
function persistOffStockSaleAtomically(sale, monitorRow) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      tx.onerror = (event) => reject(event.target.error || tx.error);
      tx.onabort = () =>
        reject(tx.error || new Error("Off-stock sale transaction aborted"));
      tx.oncomplete = () => {
        _notifySyncLocalChange("sales");
...
function persistStockMutationAtomically({
  item,
  delta,
  reason,
  refId,
    ...
    try {
      ...
      if (syncable) {
        ...
        stateRequest.onsuccess = () => {
          const meta = stateRequest.result || {
            key: "state",
            clock: 0,
            clientId: _syncClient(),
          };
      ...
      tx.onerror = (event) => reject(event.target.error || tx.error);
      tx.onabort = () =>
        reject(tx.error || new Error("Stock mutation aborted"));
      tx.oncomplete = () => {
...
function persistSaleEditAtomically({
  item,
  shoeSize = null,
  sale,
  delta,
    ...
    try {
      ...
      const move = hasLedger
        ? {
            id: _stockMoveId({
              reason: "sale_edit",
              refId: `sale:${sale.id}:edit`,
              itemId: item.id,
              sizeId: hasSize ? shoeSize.id : null,
            }),
            itemId: String(item.id),
      ...
      tx.onerror = (event) => reject(event.target.error || tx.error);
      tx.onabort = () =>
        reject(tx.error || new Error("Sale edit transaction aborted"));
      tx.oncomplete = () => {
        _notifySyncLocalChange("sales");
        resolve();
      };
    ...
    if (sizeId != null && sizeId !== "") {
      ...
      const sizeRec = rec.find(
        (s) =>
          ...
          String(s.size ?? s.id) === String(sizeId),
      );
...
async function readableStockQty(item, sizeId = null) {
  ...
  if (sizeId != null && sizeId !== "") {
    ...
    if (item.id != null || item.itemId != null) {
      ...
      return hasLedgerMoves
        ? await deriveQty(itemId, sizeId)
        : Number(item.qty ?? 0);
    }
    return Number(item.qty || 0);
  }
...
async function backfillStockMovesFromCurrentQty({ dryRun = false } = {}) {
  ...
  if (!db.objectStoreNames.contains("items"))
    return { skipped: true, dryRun, reason: "items store missing" };
  if (!db.objectStoreNames.contains("stock_moves")) {
...
async function runStockLedgerMigrationCutover({
  dryRun = false,
  force = false,
} = {}) {
  if (!db) {
...
async function runStockLedgerIntegrityCheck() {
  ...
  for (const item of items) {
    ...
    if (Number(derived) !== Number(legacy)) {
      ...
      console.warn("[stock-ledger] mismatch", {
        itemId: item.id,
        legacy,
        derived,
      });
...
function _syncIdentity(store, record) {
  ...
  if (store === "items" && typeof stableItemFbId === "function")
    return stableItemFbId(record);
  if (store === "shoe_sizes" && typeof stableShoeSizeFbId === "function")
    return stableShoeSizeFbId(record);
  if (store === "business_days" && typeof stableBusinessDayFbId === "function")
    return stableBusinessDayFbId(record);
  if (store === "wishlist" && typeof stableWishFbId === "function")
    return stableWishFbId(record);
  if (store === "purchase_budgets")
    return "budget_" + (record.id || _syncUuid());
  if (store === "customers" && record.customerId != null)
    return "cust_" + record.customerId;
...
function _queueRecord(store, record, operation, clock, now) {
  ...
  const payload =
    operation === "delete"
      ? {
          fbId: record.fbId || _syncIdentity(store, record),
          _syncDeleted: true,
        }
...
function _writeLocal(store, operation, data, id) {
  return new Promise((resolve, reject) => {
    ...
    try {
      const syncable =
        _SYNCABLE_STORES.includes(store) &&
        db.objectStoreNames.contains("sync_queue") &&
        _appDbReady;
...
async function backfillWishlistSyncOutbox() {
  ...
  if (
    !db.objectStoreNames.contains("wishlist") ||
    !db.objectStoreNames.contains("sync_meta")
  )
    return 0;
...
class DB {
  ...
  static add(store, data) {
    return dbAdd(store, data);
  }
  static put(store, data) {
  ...
  static async salesForDay(businessDate) {
    ...
    return all.filter(
      (s) => (s.businessDate || s.business_date) === businessDate,
    );
  }
...
function walkCategoryTree(visitor) {
  ...
  function walkChildren(parentId, depth) {
    types
      .filter((t) => _typeParentMatches(t.parentId, parentId))
      .sort(_sortTypes)
      .forEach((child) => {
        visitor(child, depth);
...
function collectCategoryDescendantIds(parentId) {
  ...
  function walk(pid) {
    types
      .filter((t) => _typeParentMatches(t.parentId, pid))
      .forEach((c) => {
        ids.push(c.id);
        walk(c.id);
      });
...
function _isFootwearAddFormActive() {
  return !!document
    .getElementById("page-add")
    ?.classList.contains("footwear-add-mode");
}
...
function itemMatchesTypeFilter(item, filterName) {
  ...
  if (itemRec && filterRec && isDescendantOfType(itemRec, filterRec.id))
    return true;
  return false;
}
...
const Validate = {
    ...
    if (el.classList.contains("cat-pick-hidden-select")) {
      const btn =
        ...
        document.querySelector(
          "#" +
            id +
            "-cascade .cat-pick-btn:not(.has-value), #" +
            id +
            "-cascade .cat-pick-btn",
        );
      if (btn) {
  ...
  price(buy, sell, buyFieldId, sellFieldId) {
    if (!buy || buy <= 0)
      return this.fail("Enter buying price (must be > 0)", buyFieldId);
    if (!sell || sell <= 0)
      return this.fail("Enter selling price (must be > 0)", sellFieldId);
    if (sell < buy)
      return this.fail(
        "Selling price (" +
          fmt(sell) +
  ...
  qty(qty, qtyFieldId) {
    if (qty === "" || qty === null || isNaN(qty))
      return this.fail("Enter a quantity", qtyFieldId);
    if (qty < 0) return this.fail("Quantity cannot be negative", qtyFieldId);
    if (qty === 0)
      return this.fail(
        "Quantity must be at least 1 when adding new stock",
        qtyFieldId,
      );
    if (qty > 999999)
      return this.fail("Quantity exceeds maximum (999,999)", qtyFieldId);
    return true;
  },
  ...
  restockQty(qty, qtyFieldId) {
    if (!qty || isNaN(qty) || qty <= 0)
      return this.fail(
        "Enter a quantity to add (must be at least 1)",
        qtyFieldId,
      );
    if (qty > 999999)
      return this.fail("Quantity exceeds maximum (999,999)", qtyFieldId);
  ...
  stock(wantQty, inStock, itemName) {
    if (inStock <= 0)
      return this.fail((itemName || "Item") + " is out of stock", null);
    if (wantQty > inStock)
      return this.fail(
        "Only " + inStock + " in stock - cannot sell " + wantQty,
        null,
      );
    if (wantQty <= 0)
      return this.fail("Quantity to sell must be at least 1", null);
    return true;
  },
  // Sale price check
  salePrice(priceUsed, buyPrice, sellPrice) {
    if (!priceUsed || priceUsed <= 0)
      return this.fail("Enter a selling price", null);
  ...
  text(value, fieldId, label) {
    if (!value || !String(value).trim())
      return this.fail((label || "This field") + " is required", fieldId);
    return true;
  },
  ...
  moneyRequired(value, fieldId, label) {
    if (value === null || value === "")
      return this.fail("Enter " + (label || "amount").toLowerCase(), fieldId);
    if (!Number.isFinite(value))
      return this.fail("Enter a valid number", fieldId);
    if (value < 0)
      return this.fail((label || "Amount") + " cannot be negative", fieldId);
    if (value <= 0)
      return this.fail(
    ...
    if (value > 99999999)
      return this.fail((label || "Amount") + " is too large", fieldId);
    return true;
  },
  ...
  dayOpening(cash, till, mpesa) {
    ...
    if (vals.every((v) => v === null)) {
      return this.fail(
        "Enter opening balances - type 0 if a pocket is empty",
        "op-cash",
      );
    }
    for (let i = 0; i < vals.length; i++) {
      ...
      if (!Number.isFinite(vals[i]))
        return this.fail("Enter valid numbers only", ids[i]);
      if (vals[i] < 0)
        return this.fail("Opening balance cannot be negative", ids[i]);
    }
    return true;
  },
  ...
  dayClosingPhysical(cash, till, mpesa) {
    ...
    if (vals.every((v) => v === null)) {
      return this.fail(
        "Enter closing cash, till, or M-Pesa - type 0 if empty",
        "cl-cash",
      );
    }
    for (let i = 0; i < vals.length; i++) {
      ...
      if (!Number.isFinite(vals[i]))
        return this.fail("Enter valid numbers only", ids[i]);
      if (vals[i] < 0)
        return this.fail("Closing amount cannot be negative", ids[i]);
    }
    return true;
  },
  ...
  intOptional(value, fieldId, label) {
    if (value === null) return true;
    if (!Number.isFinite(value))
      return this.fail("Enter a valid whole number", fieldId);
    if (value < 0)
      return this.fail((label || "Quantity") + " cannot be negative", fieldId);
    return true;
  },
};
...
const Input = {
  ...
  moneyZero(...values) {
    return values.map((v) =>
      v === null || v === "" ? 0 : Number.isFinite(v) ? v : 0,
    );
  },
};
  ...
  const sumType = (type) =>
    fins
      .filter(
        (e) =>
          e.type === type &&
          (e.date || (e.createdAt || "").split("T")[0]).slice(0, 10) === day,
      )
      .reduce((s, e) => s + (e.amount || 0), 0);
  return {
    injection: sumType("injection"),
    expense: sumType("expense"),
    withdrawal: sumType("withdrawal"),
  };
}
...
function _warnFinanceClosingMismatch(finTotals, closing, tolerance) {
  ...
  const lines = [];
  if (
    Math.abs((finTotals.expense || 0) - closing.expenses) > tol &&
    ((finTotals.expense || 0) > 0 || closing.expenses > 0)
  ) {
    lines.push(
      "Business expenses: closing " +
        fmt(closing.expenses) +
        " vs Finance tab " +
        fmt(finTotals.expense || 0),
    );
  }
  if (
    Math.abs((finTotals.withdrawal || 0) - closing.withdrawn) > tol &&
    ((finTotals.withdrawal || 0) > 0 || closing.withdrawn > 0)
  ) {
    lines.push(
      "Personal withdraws: closing " +
        fmt(closing.withdrawn) +
        " vs Finance tab " +
        fmt(finTotals.withdrawal || 0),
    );
  }
  if (
    Math.abs((finTotals.injection || 0) - closing.injected) > tol &&
    ((finTotals.injection || 0) > 0 || closing.injected > 0)
  ) {
    lines.push(
      "Cash to business: closing " +
        fmt(closing.injected) +
        " vs Finance tab " +
        fmt(finTotals.injection || 0),
    );
  }
  return lines;
}
    ...
    this._timer = setInterval(() => {
      ...
      if (arc)
        arc.style.strokeDashoffset =
          this._circumference * (1 - this._progress / 100);
...
async function migrateData() {
  ...
  try {
    ...
    for (const item of items) {
      ...
      normalizeNumber("buyPrice");
      normalizeNumber("sellPrice");
      normalizeNumber("qty");
      ...
      if (
        item.isShoe &&
        (!item.category || ["S", "M", "L"].includes(item.category))
      ) {
      ...
      if (item.buyPrice != null && item.sellPrice != null) {
        ...
        if (item.profit !== expected) {
          item.profit = expected;
          changed = true;
        }
      }
      ...
      if (Number.isFinite(item.qty) && !Number.isInteger(item.qty)) {
    ...
    for (const sz of sizes) {
      let changed = false;
      for (const field of ["size", "qty", "buyPrice", "sellPrice"]) {
        if (sz[field] == null || sz[field] === "") continue;
        const value = Number(sz[field]);
        if (Number.isFinite(value) && sz[field] !== value) {
          sz[field] = value;
          changed = true;
        }
      }
      if (Number.isFinite(sz.qty) && sz.qty < 0) {
        sz.qty = 0;
        changed = true;
      }
      if (Number.isFinite(sz.qty) && !Number.isInteger(sz.qty)) {
        sz.qty = Math.floor(sz.qty);
        changed = true;
      }
      ...
      if (sz.buy != null && sz.buyPrice == null) {
        sz.buyPrice = sz.buy;
        changed = true;
      }
      if (sz.sell != null && sz.sellPrice == null) {
        sz.sellPrice = sz.sell;
        changed = true;
      }
      if (!sz.createdAt) {
        sz.createdAt = new Date().toISOString();
        changed = true;
      }
      if (changed) {
    ...
    if (db.objectStoreNames.contains("wishlist")) {
      ...
      if (wFixed)
        console.log(`[MIGRATE] wishlist vendorQuotes: ${wFixed} initialized`);
    }
    ...
    for (const s of sales) {
      ...
      if (Number.isFinite(s.qty) && s.qty < 1) {
        s.qty = 1;
        changed = true;
      }
      if (Number.isFinite(s.qty) && !Number.isInteger(s.qty)) {
        s.qty = Math.floor(s.qty);
        changed = true;
      }
    ...
    for (const bd of bdays) {
      ...
      if (bd.opened_at && !bd.openedAt) {
        bd.openedAt = bd.opened_at;
        changed = true;
      }
      if (bd.closed_at && !bd.closedAt) {
        bd.closedAt = bd.closed_at;
        changed = true;
      }
      if (bd.reopened_count != null && bd.reopenedCount == null) {
        bd.reopenedCount = bd.reopened_count;
        changed = true;
      }
      if (changed) {
    ...
    for (const f of finances) {
      ...
      if (f.amount != null && f.amount !== "") {
        ...
        if (Number.isFinite(amount) && f.amount !== amount) {
          f.amount = amount;
          changed = true;
        }
        if (Number.isFinite(f.amount) && f.amount < 0) {
          f.amount = Math.abs(f.amount);
          changed = true;
        }
      }
      if (!f.createdAt) {
  ...
  try {
    localStorage.setItem(
      ...
      JSON.stringify({
        ...
        wishlistSection:
          typeof _activeWishlistSection === "string"
            ? _activeWishlistSection
            : "list",
...
function restoreLastView() {
  try {
    ...
    if (["stock", "stock-control", "add"].includes(saved.inventoryTab))
      _activeInventoryTab = saved.inventoryTab;
    ...
    if (["wishlist", "monitor"].includes(saved.stockControlTab))
      _activeStockControlTab = saved.stockControlTab;
    if (saved.wishlistSection === "list") _activeWishlistSection = "list";
    if (["day", "finance"].includes(saved.operationsTab))
      _activeOperationsTab = saved.operationsTab;
    if (["sell", "history"].includes(saved.salesTab))
      _activeSalesTab = saved.salesTab;
  } catch (_) {}
}
...
function showInventoryTab(tab) {
  if (tab === "wishlist" || tab === "monitor") {
    ...
    _activeInventoryTab = ["stock", "stock-control", "add"].includes(tab)
      ? tab
      : "stock";
  }
  mountInventoryPage();
    ...
    sub.textContent =
      {
        ...
        "stock-control":
          _activeStockControlTab === "monitor"
  ...
  if (
    _activeInventoryTab === "stock-control" &&
    _activeStockControlTab === "wishlist"
  ) {
...
function showPage(id) {
  ...
  if (id === "customer-detail") {
    /* content already populated by openCustomerDetail */
  }
}
...
async function normalizeTypeRecords() {
  ...
  for (const t of types) {
    ...
    if (t.isFootwear == null) {
      t.isFootwear = _legacyFootwearName(t.name);
      changed = true;
    }
    if (t.sortOrder == null) {
      t.sortOrder = t.id || 0;
...
function _activeChildTypes(parentId) {
  return types
    .filter(
      (t) => _typeParentMatches(t.parentId, parentId) && isCategoryActive(t),
    )
    .sort(_sortTypes);
}
...
function _updateCascadeBreadcrumb(config, pathIds, committed) {
  ...
  if (committed && pathRecs.length) {
    ...
    breadcrumb.textContent = pathRecs
      .map((t) => (t.emoji || "📦") + " " + t.name)
      .join(" > ");
  } else if (pathRecs.length) {
    breadcrumb.hidden = false;
    breadcrumb.textContent =
      pathRecs.map((t) => (t.emoji || "📦") + " " + t.name).join(" > ") +
      " >...";
  } else {
...
function _appendCascadePickButton(
  wrap,
  config,
  depth,
  parentId,
  ...
  if (useNativeSelect) {
    ...
    btn.addEventListener("click", () => {
      ...
      if (parentId) {
        ...
        if (parentRec)
          subtitle =
            "Under: " + (parentRec.emoji || "📦") + " " + parentRec.name;
      }
      openCategoryPicker({
...
function renderCategoryCascade(config, selectedValue, opts) {
  ...
  const isComplete =
    deepestRec &&
    (!config.requireLeaf || !_categoryHasActiveChildren(deepestId));
  ...
  _updateCascadeBreadcrumb(
    config,
    pathIds,
    !!(isComplete && config.valueEl && config.valueEl.value),
  );
...
function _makeCascadeConfig(base) {
  ...
  return {
    ...
    breadcrumbEl:
      base.breadcrumbEl || document.getElementById(idPrefix + "-breadcrumb"),
    idPrefix,
...
function renderAddTypeCascade(selectedTypeName, opts) {
  ...
  const selectedName =
    selectedTypeName != null ? selectedTypeName : hidden.value || "";
...
function openCategoryPicker(opts) {
  ...
  sheet.className =
    "cat-picker-overlay" + (opts.compact ? " cat-picker-compact" : "");
    ...
    return filtered
      .map((it) => {
        ...
        const pad = depth
          ...
          : "";
        const sel =
          String(opts.currentId || "") === String(it.id) ? " selected" : "";
        return (
          ...
          pad +
          ">" +
          '<span class="cat-picker-emoji">' +
          (it.emoji || "📦") +
          "</span>" +
          '<span class="cat-picker-body">' +
          '<span class="cat-picker-name">' +
          escapeHtml(it.name || "") +
          "</span>" +
          (it.hint
            ? '<span class="cat-picker-hint">' + escapeHtml(it.hint) + "</span>"
            : "") +
          "</span>" +
  ...
  sheet.innerHTML =
    ...
    '<div class="cat-picker-title">' +
    escapeHtml(opts.title || "Choose category") +
    "</div>" +
    (opts.subtitle
      ? '<div class="cat-picker-sub">' + escapeHtml(opts.subtitle) + "</div>"
      : "") +
    "</div>" +
    ...
    "</div>" +
    (opts.allowClear
      ? '<button type="button" class="cat-picker-clear">Clear selection</button>'
      : "") +
  ...
  sheet
    .querySelector(".cat-picker-panel")
  ...
  sheet
    .querySelector(".cat-picker-cancel")
...
const _RESTOCK_PRICING_TITLE =
  '<span class="add-step-badge">3</span> Stock &amp; pricing';
...
function showRestockView(meta) {
  ...
  setCell(
    "rs-category",
    typeObj ? (typeObj.emoji || "📦") + " " + meta.type : meta.type,
  );
  ...
  if (ml) {
    ml.hidden = false;
    ml.textContent =
      meta.size != null ? "Restock - Size " + meta.size : "Restock";
  }
  setAddFormSubtitle(
    meta.code ? meta.code + (meta.name ? " - " + meta.name : "") : "",
  );

  const sizeLabel = meta.size != null ? String(meta.size) : "";
  setSaveBtnLabel(
    sizeLabel ? "RESTOCK (" + sizeLabel + ")" : "RESTOCK",
    "fa-boxes-stacked",
  );
  ...
  const pricingTitle = document.querySelector(
    "#std-pricing-section .add-card-title",
  );
...
function setAddFormType(typeName, opts) {
  ...
  renderAddTypeCascade(typeName || "", {
    skipTypeChange: opts && opts.skipTypeChange,
  });
}
...
function renderTypeChips() {
  ...
  if (!chips) return;
  const topActive = types
    .filter((t) => t.parentId == null && isCategoryActive(t))
    .sort(_sortTypes);
  chips.innerHTML =
    '<span class="chip ' +
    (activeTypeFilter === "all" ? "active" : "") +
    '" onclick="setTypeFilter(\'all\', this)">All</span>' +
    topActive
      .map(
        (t) =>
          '<span class="chip ' +
          (activeTypeFilter === t.name ? "active" : "") +
...
async function renderCategorySettings() {
  try {
    ...
    if (!types.length) {
      list.innerHTML =
        '<div style="color:var(--muted);font-size:13px;padding:8px;">No categories yet</div>';
      return;
    }
    mountNewSubParentCascade();
    ...
    const rowHtml = (t, depth) => {
      ...
      return (
        ...
        escapeHtml(t.name) +
        (depth
          ? ' <span class="cat-subcount">L' + (depth + 1) + "</span>"
          : "") +
        (subCount
          ? ' <span class="cat-subcount">' + subCount + " nested</span>"
          : "") +
        "</div>" +
        '<div class="cat-meta">' +
        (active ? "Active in dropdowns" : "Hidden from dropdowns") +
        (footwear ? " - Size-grid mode" : "") +
        "</div>" +
        "</div>" +
        ...
        t.id +
        ')" title="Show in dropdowns">' +
        (active ? "ON" : "OFF") +
        "</button>" +
        '<button type="button" class="cat-toggle foot' +
        (footwear ? " on" : "") +
        '" onclick="toggleCategoryFootwear(' +
        t.id +
        ')" title="Use shoe size grid">' +
        "Sizes</button>" +
        '<button type="button" class="type-del" onclick="deleteType(' +
        t.id +
        ')">X</button>' +
        "</div>" +
        "</div>" +
...
function _shoeGroupSettingRowHtml(id, config) {
  ...
  return (
    '<div class="sg-setting-row" data-group="' +
    safeId +
    '">' +
    '<div class="sg-setting-fields">' +
    '<span class="sg-setting-code">' +
    safeId +
    "</span>" +
    '<input id="sg-label-' +
    safeId +
    '" type="text" class="type-input sg-label-input" placeholder="Group name" value="' +
    ...
    '<input id="sg-min-' +
    safeId +
    '" type="number" min="1" max="60" class="type-input sg-num" placeholder="Min" value="' +
    config.min +
    '" aria-label="' +
    safeId +
    ' minimum size">' +
    ...
    '<input id="sg-max-' +
    safeId +
    '" type="number" min="1" max="60" class="type-input sg-num" placeholder="Max" value="' +
    config.max +
    '" aria-label="' +
    safeId +
    ' maximum size">' +
    '<button type="button" class="sg-delete-btn" onclick="removeShoeGroupSetting(\'' +
    safeId +
    '\')" aria-label="Delete ' +
    safeId +
    ' group" title="Delete group"><i class="fa-solid fa-trash" aria-hidden="true"></i></button>' +
    "</div></div>"
  );
}
...
async function addShoeGroupSetting() {
  ...
  rows.forEach((row) => {
    const min = parseInt(
      document.getElementById("sg-min-" + row.dataset.group)?.value,
      10,
    );
    const max = parseInt(
      document.getElementById("sg-max-" + row.dataset.group)?.value,
      10,
    );
    if (
      Number.isFinite(min) &&
...
async function removeShoeGroupSetting(id) {
  ...
  if (
    hasSavedSizes &&
    !confirm(
      "Existing sizes use this group. Remove the group settings? Stock records will remain.",
    )
  )
    return;
  row.remove();
...
async function saveShoeGroupSettings() {
  ...
  const rows = [
    ...(document
      .getElementById("shoe-groups-settings")
      ?.querySelectorAll(".sg-setting-row") || []),
  ];
  if (!rows.length) {
  ...
  for (let index = 1; index < sortedGroups.length; index++) {
    ...
    if (current[1].min <= previous[1].max) {
      toast(
        "Size ranges for " + previous[0] + " and " + current[0] + " overlap",
        "err",
      );
      return;
    }
...
async function toggleCategoryFootwear(id) {
  ...
  toast(
    t.isFootwear
      ? "Size-grid mode ON for " + t.name
      : "Size-grid mode OFF for " + t.name,
    "ok",
  );
}
  ...
  try {
    ...
    if (descIds.length)
      msg +=
        ...
        descIds.length +
        " nested sub-categor" +
        (descIds.length === 1 ? "y" : "ies") +
        ".";
    if (inUse > 0)
      msg +=
        "\n\n" +
        inUse +
        " item(s) still use these names - they will keep the label.";
...
function updateProfitPreview() {
  ...
  if (buy > 0 && sell > 0) {
    ...
    if (ppMargin) {
      ppMargin.textContent = marginMax + "%";
      ppMargin.style.color = profitMax >= 0 ? "var(--accent)" : "var(--red)";
    }
...
async function compressImageForStorage(source, presetName) {
  ...
  for (const mime of mimes) {
    ...
    while (quality >= preset.minQuality) {
      const candidate = await compressImageDataUrl(
        dataUrl,
        preset.maxW,
        preset.maxH,
        quality,
...
function compressImageFile(file, maxW, quality) {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (ev) =>
      compressImageDataUrl(ev.target.result, maxW, maxW, quality)
        .then(resolve)
        .catch(reject);
...
async function initPhotoStore() {
  ...
  try {
    ...
    rows.forEach((r) => {
      if (r && r.key && r.dataUrl) _photoCache.set(r.key, r.dataUrl);
    });
...
async function _migrateLegacyLocalStoragePhotos() {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (k && (k.startsWith("item_photo_") || k.startsWith("wish_photo_")))
      keys.push(k);
  }
  ...
  for (const k of keys) {
    ...
    try {
      ...
      /* keep in localStorage if migrate fails */
    }
    localStorage.removeItem(k);
  }
}
...
async function _imageBlobToStorage(blob, preset) {
  ...
  const rough = await compressImageFile(
    new File([blob], "screenshot.jpg", { type }),
    960,
    0.82,
  );
...
function _isWishlistVisible() {
  ...
  if (
    inv &&
    inv.classList.contains("active") &&
    _activeInventoryTab === "wishlist"
  )
    return true;
...
function cancelClipboardScreenshotWait() {
  ...
  if (_clipboardPastePending.timeoutId)
    clearTimeout(_clipboardPastePending.timeoutId);
...
function _updateClipboardWaitOverlay(state) {
  ...
  if (state === "ready") {
    ...
    if (text)
      text.textContent =
        "You're back - tap Import now to attach the screenshot.";
  } else if (state === "waiting") {
    if (importBtn) importBtn.classList.remove("pulse");
    if (text)
      text.innerHTML =
        "Open another app, take your screenshot, tap <strong>Done</strong> or <strong>Complete</strong>, then switch back here.";
  }
}
...
function _showClipboardWaitOverlay() {
  ...
  el.innerHTML =
    ...
    "</div>";
  el.querySelector("#clipboard-wait-cancel-btn").addEventListener(
    "click",
    () => {
  ...
  el.querySelector("#clipboard-wait-import-btn").addEventListener(
    "click",
    () => {
  ...
  el.querySelector("#clipboard-wait-gallery-btn").addEventListener(
    "click",
    () => {
...
async function pasteImageFromClipboard(options) {
  ...
  if (navigator.clipboard && navigator.clipboard.read) {
    try {
      ...
      if (!silent) {
        if (e.name === "NotAllowedError") {
          ...
          toast(
            "Could not read clipboard - tap Import now or use Gallery",
            "err",
          );
        }
      }
    ...
    toast(
      "Clipboard not supported - use Gallery and pick your screenshot",
      "err",
    );
  }
...
function startClipboardScreenshotImport(opts) {
  ...
  async function tryImport(fromUserTap) {
    ...
    try {
      ...
      if (fromUserTap) {
        toast(
          "No screenshot in clipboard - try Gallery or take the screenshot again",
          "err",
        );
      }
      return false;
...
function initWishlistScreenshotWatch() {
  ...
  document.addEventListener("visibilitychange", () => {
    ...
    if (!_isWishlistVisible() || _clipboardPastePending || _wishFormPhotoData)
      return;
    delays.forEach((ms) => {
      setTimeout(async () => {
        if (
...
function showImagePickerSheet(opts) {
  ...
  sheet.style.cssText =
    "position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,0.5);display:flex;align-items:flex-end;justify-content:center;";
  sheet.onclick = (e) => {
    if (e.target === sheet) _closeImagePickerSheet();
  };
  const btnStyle =
    "width:100%;padding:14px;border-radius:var(--r);font-size:15px;font-weight:700;cursor:pointer;font-family:var(--sans);margin-bottom:8px;border:none;";
  sheet.innerHTML =
    ...
    '<div style="font-size:15px;font-weight:800;color:var(--text);margin-bottom:6px;text-align:center;">' +
    escapeHtml(title) +
    "</div>" +
    ...
    '<button type="button" data-src="camera" style="' +
    btnStyle +
    'background:var(--accent);color:white;">Take photo</button>' +
    '<button type="button" data-src="gallery" style="' +
    btnStyle +
    'background:var(--surface2);color:var(--text);border:1.5px solid var(--border);">Choose from gallery</button>' +
    '<button type="button" data-src="clipboard" style="' +
    btnStyle +
    'background:var(--surface2);color:var(--text);border:1.5px solid var(--border);">Screenshot from another app</button>' +
...
function triggerAddPhotoUpload() {
  showImagePickerSheet({
    ...
    onPick: async (dataUrl) => {
      ...
      if (photoImg) {
        photoImg.src = _addFormPhotoData;
...
function parseWishPhotoFileName(fileName) {
  ...
  const base = fileName
    .trim()
    .replace(/\.(jpe?g|png|gif|webp|heic|heif|bmp|avif)$/i, "")
    .trim();
  if (!base) return null;
...
function updateWishPhotoPreview() {
  ...
  if (_wishFormPhotoData) {
    if (photoImg) {
      photoImg.src = _wishFormPhotoData;
      photoImg.style.display = "block";
    }
    if (placeholder) placeholder.style.display = "none";
    if (removeBtn) removeBtn.style.display = "block";
...
let _selectedSavedWishlistListId = "";
...
function getSavedWishlistLists() {
  try {
    const saved = JSON.parse(
      localStorage.getItem(KEY_WISHLIST_SAVED_LISTS) || "[]",
    );
    return Array.isArray(saved)
      ? saved
          ...
          .map((list) => ({
            ...list,
            category: String(list.category || "").trim(),
            supply: String(list.supply || "").trim(),
            budget: Number(list.budget || 0) || 0,
            status: ["planned", "ordering", "partial", "complete"].includes(
              list.status,
            )
              ? list.status
              : "planned",
            dueDate: String(list.dueDate || "").trim(),
            itemIds: [
              ...new Set(
                list.itemIds
                  .map((id) => Number(id))
...
function getWishlistListMembership(itemId, lists) {
  return (
    lists.find((list) =>
      (list.itemIds || []).some((id) => String(id) === String(itemId)),
    ) || null
  );
}

function removeWishlistItemsFromLists(lists, itemIds) {
...
async function runAtomicWishlistOperation(itemIds, updateRecord, updateLists) {
  const ids = [
    ...new Set(
      itemIds.map((id) => Number(id)).filter((id) => Number.isInteger(id)),
    ),
  ];
  const previousRecords = (
    await Promise.all(ids.map((id) => dbGet("wishlist", id)))
  ).filter(Boolean);
  ...
  try {
    ...
    if (previousLists === null)
      localStorage.removeItem(KEY_WISHLIST_SAVED_LISTS);
...
function moveWishlistItemsToSavedList(itemIds, targetListId) {
  ...
  if (!target) return { ok: false, reason: "Saved list not found" };
  const targetIds = new Set((target.itemIds || []).map((id) => String(id)));
  const ids = [
    ...new Set(
      itemIds.map((id) => Number(id)).filter((id) => Number.isInteger(id)),
    ),
  ];
  const duplicates = ids.filter((id) => targetIds.has(String(id)));
  if (duplicates.length)
    return {
      ok: false,
      reason: "One or more items already exist in the destination list",
    };
  removeWishlistItemsFromLists(lists, ids);
  target.itemIds = [...new Set([...(target.itemIds || []), ...ids])];
  persistSavedWishlistLists(lists);
  return { ok: true, target };
}

function moveWishlistItemsToGeneralList(itemIds) {
  const lists = getSavedWishlistLists();
...
function assignWishlistItemToSavedList(itemId, listId) {
  return moveWishlistItemsToSavedList([itemId], listId).ok;
}
...
function createSavedWishlistList() {
  ...
  lists.push({
    id:
      "wish-list-" + Date.now() + "-" + Math.random().toString(36).slice(2, 8),
    name,
    category: "",
    supply: "",
    budget: 0,
...
function editSavedWishlistList(listId) {
  ...
  if (
    lists.some(
      (entry) =>
        entry.id !== listId && entry.name.toLowerCase() === name.toLowerCase(),
    )
  ) {
    return toast('"' + name + '" already exists', "info");
  }
  ...
  list.name = name;
  if (_activeWishlistSavedList === previousName)
    _activeWishlistSavedList = name;
  persistSavedWishlistLists(lists);
  renderWishlistPage();
...
function duplicateSavedWishlistList(listId) {
  const lists = getSavedWishlistLists();
  const source = lists.find((list) => list.id === listId);
  if (!source) return toast("Saved list not found", "err");
  const copyName = source.name + " copy";
  if (
    lists.some((list) => list.name.toLowerCase() === copyName.toLowerCase())
  ) {
    return toast('"' + copyName + '" already exists', "info");
  }
  lists.push({
    ...source,
    id:
  ...
  persistSavedWishlistLists(lists);
  renderWishlistPage();
  toast('Stock list "' + copyName + '" duplicated', "ok");
}
...
async function deleteSavedWishlistList(listId) {
  ...
  const list = lists.find((entry) => entry.id === listId);
  if (!list) return toast("Saved list not found", "err");
  const wishes = db.objectStoreNames.contains("wishlist")
    ? await dbAll("wishlist")
    : [];
  const liveIds = (list.itemIds || []).filter((id) => {
    const wish = wishes.find((entry) => String(entry.id) === String(id));
    return (
      wish && wishStatus(wish) !== "stocked" && wishStatus(wish) !== "discarded"
    );
  });
  ...
  const transferMessage = [
    ...
    stockedIds.length +
      " item" +
      (stockedIds.length === 1 ? "" : "s") +
      " marked as stocked to Stocked list",
  ].join("\n");
  if (!confirm(transferMessage + '\n\nDelete wishlist "' + list.name + '"?'))
    return;
  ...
  const movedSummary = [
    ...
    .join("; ");
  toast(
    movedSummary ? "Saved list deleted. " + movedSummary : "Saved list deleted",
    "ok",
  );
}
...
function setWishlistView(view, savedListName) {
  ...
  if (nextView === "saved" && savedListName)
    _activeWishlistSavedList = String(savedListName);
  _selectedWishlistIds.clear();
  renderWishlistPage();
}
...
async function toggleSavedWishlistListSelection(listId, checked) {
  const list = getSavedWishlistLists().find((entry) => entry.id === listId);
  if (!list) return;
  _selectedSavedWishlistListId = checked ? listId : "";
  const wishes = db.objectStoreNames.contains("wishlist")
    ? await dbAll("wishlist")
    : [];
  const liveIds = wishes
    .filter(
      (wish) =>
        (list.itemIds || []).some((id) => String(id) === String(wish.id)) &&
        wishStatus(wish) !== "stocked" &&
        wishStatus(wish) !== "discarded",
    )
    .map((wish) => wish.id);
  liveIds.forEach((id) => {
    if (checked) _selectedWishlistIds.add(id);
    else _selectedWishlistIds.delete(id);
  });
  renderWishlistPage();
}
window.toggleSavedWishlistListSelection = toggleSavedWishlistListSelection;

function toggleSavedWishlistMoveButton(select) {
  const button = select
    ?.closest(".wish-saved-list-transfer")
    ?.querySelector("button");
  if (button) button.disabled = !select.value;
}
window.toggleSavedWishlistMoveButton = toggleSavedWishlistMoveButton;

function moveAllItemsFromSavedWishlistList(sourceListId, targetListId) {
  const lists = getSavedWishlistLists();
  const source = lists.find((list) => list.id === sourceListId);
  const target = lists.find(
    (list) => list.id === targetListId && list.id !== sourceListId,
  );
  if (!source) return toast("Source list not found", "err");
  if (!target) return toast("Choose another active list", "info");
  const sourceIds = [
    ...new Set(
      (source.itemIds || [])
        .map((id) => Number(id))
  ...
  if (!sourceIds.length)
    return toast("There are no items in this list", "info");
  const existingTargetIds = [...(target.itemIds || [])];
  removeWishlistItemsFromLists(lists, sourceIds);
  target.itemIds = [...new Set([...existingTargetIds, ...sourceIds])];
  persistSavedWishlistLists(lists.filter((list) => list.id !== sourceListId));
  _selectedSavedWishlistListId = "";
  _selectedWishlistIds.clear();
  _activeWishlistSavedList = target.name;
  renderWishlistPage();
  toast(
    sourceIds.length +
      " item" +
      ...
      ' moved to "' +
      target.name +
      '". Empty source list deleted.',
    "ok",
  );
}
window.moveAllItemsFromSavedWishlistList = moveAllItemsFromSavedWishlistList;
...
async function addActiveWishlistItemsToSavedList(listId) {
  ...
  const activeIds = wishes
    .filter(
      (wish) =>
        !isWishlistSaleMonitorEntry(wish) &&
        wishStatus(wish) !== "stocked" &&
  ...
  for (const other of lists) {
    other.itemIds = (other.itemIds || []).filter(
...
function getWishlistListMetrics(list, wishesById, includeStocked = false) {
  ...
  return {
    ...
    quantity,
    estimated,
    remaining: Math.max(0, Number(list.budget || 0) - estimated),
    stocked: items.filter((wish) => wishStatus(wish) === "stocked").length,
  };
}

function removeWishlistItemFromSavedList(listId, itemId) {
  const lists = getSavedWishlistLists();
  const list = lists.find((entry) => entry.id === listId);
  if (!list) return;
  list.itemIds = (list.itemIds || []).filter(
    (id) => String(id) !== String(itemId),
  );
  persistSavedWishlistLists(lists);
  renderWishlistPage();
  toast("Item removed from list", "ok");
}
...
function moveWishlistItemToList(itemId, targetListId, sourceListId) {
  ...
  renderWishlistPage();
  toast('Item moved to "' + result.target.name + '"', "ok");
}
window.moveWishlistItemToList = moveWishlistItemToList;

function beginWishlistItemDrag(event, itemId) {
  if (event.dataTransfer)
    event.dataTransfer.setData("text/plain", String(itemId));
}

function dropWishlistItem(event, listId, beforeItemId) {
  event.preventDefault();
  const itemId = event.dataTransfer?.getData("text/plain");
  if (!itemId || itemId === String(beforeItemId)) return;
  const lists = getSavedWishlistLists();
  const list = lists.find((entry) => entry.id === listId);
  if (!list) return;
  const current = (list.itemIds || []).find((id) => String(id) === itemId);
  if (current === undefined) return;
  list.itemIds = list.itemIds.filter((id) => String(id) !== itemId);
  const index = list.itemIds.findIndex(
    (id) => String(id) === String(beforeItemId),
  );
  list.itemIds.splice(index < 0 ? list.itemIds.length : index, 0, current);
  persistSavedWishlistLists(lists);
  renderWishlistPage();
}
...
function chooseWishlistShareOptions() {
  ...
  modal.hidden = false;
  document
    .querySelector('input[name="wish-share-scope"][value="all"]')
    ?.focus();
  return new Promise((resolve) => {
    ...
    submit.onclick = () => {
      const scope =
        document.querySelector('input[name="wish-share-scope"]:checked')
          ?.value || "all";
...
async function shareSavedWishlistListViaWhatsApp(listId) {
  ...
  const wishesById = new Map(
    (await dbAll("wishlist")).map((wish) => [wish.id, wish]),
  );
  ...
  const notStockedItems = items.filter(
    (wish) => wishStatus(wish) !== "stocked",
  );
  ...
  const lines = [
    "*🏠 " + String(list.name || "Wishlist").toLocaleUpperCase() + "*",
    "",
  ];
  ...
  lines.push(
    "✅ *Stocked:* " + stockedItems.length,
    "⏳ *Not Yet Stocked:* " + notStockedItems.length,
  );
  const shareUrl =
...
window.shareSavedWishlistListViaWhatsApp = shareSavedWishlistListViaWhatsApp;

function exportSavedWishlistList(listId) {
  const list = getSavedWishlistLists().find((entry) => entry.id === listId);
  if (!list) return;
  dbAll("wishlist").then((wishes) => {
    ...
    (list.itemIds || []).forEach((id) => {
      ...
      if (!wish) return;
      const qty = Number(wish.qty || 0);
      const price = Number(wish.estimatedCost || 0);
      rows.push([
        list.name,
        wish.name || "",
        wish.type || "",
        wish.supplierId || wish.supplier || "",
        qty,
        price,
        qty * price,
      ]);
...
function renderSavedWishlistListsPanel(savedLists, allWishes) {
  ...
  const cards = savedLists
    .map((list) => {
      ...
      const itemRows = items
        .map((wish) => {
          ...
          const amount =
            qty > 0 && estimatedPrice > 0 ? fmtN(qty * estimatedPrice) : "";
          const unit = String(wish.unit || "").trim();
          const itemName = wish.name || "Unnamed item";
          const stockedClass =
            wishStatus(wish) === "stocked" ? " is-stocked" : "";
          return (
            '<tr class="wish-saved-list-row' +
            stockedClass +
            '" draggable="true" onclick="openWishlistDetail(' +
            wish.id +
            ')" ondragstart="beginWishlistItemDrag(event,' +
            wish.id +
            ')" ondragover="event.preventDefault()" ondrop="dropWishlistItem(event,\'' +
            escapeHtml(list.id) +
            "'," +
            wish.id +
            ')"><td class="wish-table-check-cell"><input type="checkbox" class="wish-select-item" value="' +
            wish.id +
            '" aria-label="Select ' +
            escapeHtml(itemName) +
            '"' +
            (_selectedWishlistIds.has(Number(wish.id)) ? " checked" : "") +
            ' onchange="event.stopPropagation();updateWishlistSelection()"></td><td class="wish-table-name"><button type="button" class="wish-saved-list-item-name" onclick="event.stopPropagation();openWishlistDetail(' +
            wish.id +
            ')">' +
            escapeHtml(itemName) +
            "</button></td><td>" +
            (qty > 0 ? qty + (unit ? " " + escapeHtml(unit) : "") : "") +
            '</td><td class="wish-table-amount">' +
            escapeHtml(amount) +
            "</td></tr>"
          );
        })
        .join("");
      const clearStockedButton =
        isActive && stockedItems.length
          ? '<button type="button" class="wish-saved-list-clear-stocked" onclick="clearStockedItemsFromSavedList(\'' +
            escapeHtml(list.id) +
            '\')"><i class="fa-solid fa-broom"></i> Clear stocked</button>'
          : "";
      const itemPreview = isActive
        ? '<div class="wish-saved-list-items">' +
          (itemRows
            ? '<div class="wish-table-wrap"><table class="wish-table wish-saved-list-table"><thead><tr><th scope="col">Select</th><th scope="col">Item</th><th scope="col">Qty</th><th scope="col">Est. amount</th></tr></thead><tbody>' +
              itemRows +
              '</tbody><tfoot><tr><th colspan="2">List totals</th><th>' +
              fmtN(metrics.quantity) +
              "</th><th>" +
              fmtN(metrics.estimated) +
        ...
        : "";
      const listActions =
        '<div class="wish-saved-list-actions"><button type="button" title="Edit list" aria-label="Edit list ' +
        escapeHtml(list.name) +
        '" onclick="editSavedWishlistList(\'' +
        escapeHtml(list.id) +
        '\')"><i class="fa-solid fa-pen"></i></button><button type="button" title="Delete list" aria-label="Delete list ' +
        escapeHtml(list.name) +
        '" onclick="deleteSavedWishlistList(\'' +
        escapeHtml(list.id) +
        '\')"><i class="fa-solid fa-trash"></i></button><button type="button" class="wish-saved-list-whatsapp" title="Share list via WhatsApp" aria-label="Share list ' +
        escapeHtml(list.name) +
        ' via WhatsApp" onclick="shareSavedWishlistListViaWhatsApp(\'' +
        escapeHtml(list.id) +
        '\')"><i class="fa-brands fa-whatsapp"></i></button></div>';
      ...
      const bulkActions = isActive
        ? '<div class="wish-saved-list-bulk-actions" hidden>' +
          ...
          (moveOptions
            ? '<select class="wish-saved-list-bulk-select wish-saved-list-move-select" title="Move selected items" aria-label="Move selected items" onchange="if(this.value){moveSelectedSavedWishlistItemsToList(this.value);this.value=\'\';}"><option value="">Move to...</option>' +
              moveOptions +
              "</select>"
            : "") +
      ...
      const actionRow = isActive
        ? '<div class="wish-saved-list-actions-row"><div class="wish-saved-list-add-group"><button type="button" class="wish-saved-list-add-items" onclick="showWishlistSection(\'add\')"><i class="fa-solid fa-plus"></i> Add item</button></div>' +
          bulkActions +
          "</div>"
        : "";
      const moveTargets = savedLists
        .filter((target) => target.id !== list.id)
        .map(
          (target) =>
            '<option value="' +
        ...
        .join("");
      const listTransfer =
        _selectedSavedWishlistListId === list.id && moveTargets
          ? '<div class="wish-saved-list-transfer"><label for="wish-move-target-' +
            escapeHtml(list.id) +
            '">Move to</label><select id="wish-move-target-' +
            escapeHtml(list.id) +
            '" class="wish-saved-list-transfer-select" onchange="toggleSavedWishlistMoveButton(this)"><option value="">Choose list...</option>' +
            moveTargets +
            '</select><button type="button" class="wish-saved-list-transfer-btn" disabled onclick="moveAllItemsFromSavedWishlistList(\'' +
            escapeHtml(list.id) +
            "', this.closest('.wish-saved-list-transfer').querySelector('select').value)\">Move all</button></div>"
          : "";
      return (
        '<article class="wish-saved-list-card' +
        (isActive ? " active" : "") +
        '"><div class="wish-saved-list-card-head"><input type="checkbox" class="wish-saved-list-select"' +
        (_selectedSavedWishlistListId === list.id ? " checked" : "") +
        ' aria-label="Select list ' +
        escapeHtml(list.name) +
        '" onchange="event.stopPropagation();toggleSavedWishlistListSelection(\'' +
        escapeHtml(list.id) +
        "',this.checked)\">" +
        listActions +
        '<button type="button" class="wish-saved-list-open" onclick="openSavedWishlistList(\'' +
        escapeHtml(list.id) +
        '\')"><span class="wish-saved-list-icon"><i class="fa-solid fa-list-check"></i></span><span class="wish-saved-list-copy"><strong>' +
        escapeHtml(list.name) +
        "</strong></span></button>" +
        listTransfer +
        "</div>" +
    ...
    .join("");
  return (
    '<div class="wish-saved-lists-panel"><div class="wish-saved-lists-head"><strong><i class="fa-solid fa-list-ul"></i> Wish lists</strong><button type="button" class="wish-filter-btn" onclick="createSavedWishlistList()"><i class="fa-solid fa-plus"></i> Add list</button></div><div class="wish-saved-list-grid">' +
    (cards ||
      '<div class="wish-saved-empty"><i class="fa-solid fa-list"></i><span>No saved lists yet.</span></div>') +
    "</div></div>"
  );
}
...
function addVisibleWishlistItemsToSavedList(selectedListName) {
  ...
  const visibleIds = [
    ...document.querySelectorAll("#wishlist-list .wish-row[data-wish-id]"),
  ]
  ...
  const result = moveWishlistItemsToSavedList(visibleIds, list.id);
  if (!result.ok) return toast(result.reason, "info");
  renderWishlistPage();
  toast(
    visibleIds.length +
...
function toggleWishlistAddMenu(targetListId) {
  const menu = document.getElementById("wish-add-menu-" + targetListId);
  if (menu) menu.hidden = !menu.hidden;
}
window.toggleWishlistAddMenu = toggleWishlistAddMenu;

function beginWishlistAddFromSource(targetListId, sourceListId) {
  const lists = getSavedWishlistLists();
  const target = lists.find((list) => list.id === targetListId);
  if (!target) return toast("Saved list not found", "err");
  _wishlistAddToSavedListId = target.id;
  if (sourceListId === "general") {
    _activeWishlistView = "main";
    _activeWishlistSavedList = "";
    _activeWishlistSection = "list";
    _selectedWishlistIds.clear();
    renderWishlistPage();
    toast("Select General List items, then use Add to current list", "info");
    return;
  }
  const source = lists.find(
    (list) => list.id === sourceListId && list.id !== target.id,
  );
  if (!source) return toast("Source list not found", "err");
  _activeWishlistView = "saved";
  _activeWishlistSavedList = source.name;
  _activeWishlistSection = "list";
  _selectedWishlistIds.clear();
  renderWishlistPage();
  toast(
    "Select items from " + source.name + ", then use Add to current list",
    "info",
  );
}
window.beginWishlistAddFromSource = beginWishlistAddFromSource;

function addSelectedItemsToPendingWishlistList() {
  if (!_wishlistAddToSavedListId)
    return toast("Choose a destination list first", "info");
  const ids = getSelectedWishlistIds();
  if (!ids.length) return toast("Select wishlist items first", "info");
  const result = moveWishlistItemsToSavedList(ids, _wishlistAddToSavedListId);
  if (!result.ok) return toast(result.reason, "info");
  _wishlistAddToSavedListId = "";
  _selectedWishlistIds.clear();
  _activeWishlistView = "saved";
  _activeWishlistSavedList = result.target.name;
  renderWishlistPage();
  toast(
    ids.length +
      " item" +
...
window.addSelectedItemsToPendingWishlistList =
  addSelectedItemsToPendingWishlistList;
...
function updateWishlistSelection() {
  ...
  visible.forEach((checkbox) => {
    ...
    if (checkbox.checked) _selectedWishlistIds.add(id);
    else _selectedWishlistIds.delete(id);
  });
  const count = _selectedWishlistIds.size;
  const bar = document.getElementById("wishlist-bulk-actions");
  if (bar) bar.hidden = count === 0;
  const addSelectedButton = document.getElementById(
    "wishlist-add-selected-btn",
  );
  if (addSelectedButton)
    addSelectedButton.hidden = count === 0 || !_wishlistAddToSavedListId;
  const selectedRows = visible
    .filter((checkbox) => checkbox.checked)
    .map((checkbox) => checkbox.closest("tr"));
  const hasUnstockedSelection = selectedRows.some(
    (row) => !row?.classList.contains("is-stocked"),
  );
  const hasStockedSelection = selectedRows.some((row) =>
    row?.classList.contains("is-stocked"),
  );
  const stockedAction = document.querySelector(".wish-bulk-stocked");
  const stockedDestination = document.getElementById(
    "wishlist-stocked-destination",
  );
  ...
  const savedListBulkActions = document.querySelector(
    ".wish-saved-list-bulk-actions",
  );
  if (savedListBulkActions) {
    const savedListSelection = [
      ...document.querySelectorAll(
        ".wish-saved-list-table .wish-select-item:checked",
      ),
    ];
    const savedListSelectionCount = savedListSelection.length;
    const hasUnstocked = savedListSelection.some(
      (checkbox) => !checkbox.closest("tr")?.classList.contains("is-stocked"),
    );
    savedListBulkActions.hidden = savedListSelectionCount === 0;
    const stockButton = savedListBulkActions.querySelector(
      ".wish-saved-list-stock-btn",
    );
    const moveSelect = savedListBulkActions.querySelector(
      ".wish-saved-list-move-select",
    );
    if (stockButton) stockButton.hidden = !hasUnstocked;
    if (moveSelect) moveSelect.hidden = savedListSelectionCount === 0;
  }
  const selectAll = document.getElementById("wish-select-all");
  if (selectAll) {
    selectAll.checked =
      visibleIds.size > 0 && visible.every((checkbox) => checkbox.checked);
    selectAll.indeterminate =
      visible.some((checkbox) => checkbox.checked) && !selectAll.checked;
  }
}
...
async function markSelectedWishlistItemsStocked() {
  const ids = getSelectedWishlistIds();
  if (!ids.length) return toast("Select wishlist items first", "info");
  const now = new Date().toISOString();
  try {
    await runAtomicWishlistOperation(
      ids,
      (wish) => {
        wish.status = "stocked";
        wish.stockedAt = now;
  ...
  toast(
    ids.length + " item" + (ids.length === 1 ? "" : "s") + " marked stocked",
    "ok",
  );
}
window.markSelectedWishlistItemsStocked = markSelectedWishlistItemsStocked;

async function moveSelectedWishlistItemsToGeneral() {
  const ids = getSelectedWishlistIds();
  if (!ids.length) return toast("Select wishlist items first", "info");
  const wishes = (
    await Promise.all(ids.map((id) => dbGet("wishlist", id)))
  ...
  if (!movable.length)
    return toast("Discarded items cannot move to the General List", "info");
  const now = new Date().toISOString();
  try {
    await runAtomicWishlistOperation(
      movable.map((wish) => wish.id),
      (wish) => {
...
window.moveSelectedWishlistItemsToGeneral = moveSelectedWishlistItemsToGeneral;

function moveSelectedSavedWishlistItemsToList(targetListId) {
  const ids = [
    ...document.querySelectorAll(
      ".wish-saved-list-table .wish-select-item:checked",
    ),
    ...
    .filter((id) => Number.isInteger(id));
  if (!ids.length) return toast("Select wishlist items first", "info");
  const target = getSavedWishlistLists().find(
    (list) => list.id === targetListId,
  );
  if (!target) return toast("Choose another saved list", "info");
  const result = moveWishlistItemsToSavedList(ids, target.id);
  if (!result.ok) return toast(result.reason, "info");
  _selectedWishlistIds.clear();
  _selectedSavedWishlistListId = "";
  _activeWishlistView = "saved";
...
window.moveSelectedSavedWishlistItemsToList =
  moveSelectedSavedWishlistItemsToList;

async function moveSelectedStockedItemsToList(targetListId) {
  const ids = getSelectedWishlistIds();
  const wishes = (
    await Promise.all(ids.map((id) => dbGet("wishlist", id)))
  ).filter((wish) => wish && wishStatus(wish) === "stocked");
  if (!wishes.length) return toast("Select stocked items first", "info");
  const target =
    targetListId === "general"
      ? null
      : getSavedWishlistLists().find((list) => list.id === targetListId);
  if (targetListId !== "general" && !target)
    return toast("Choose an active destination list", "info");
  if (target) {
    const duplicate = wishes.some((wish) =>
      (target.itemIds || []).some((id) => String(id) === String(wish.id)),
    );
    if (duplicate)
      return toast(
        "One or more items already exist in the destination list",
        "info",
      );
  }
  const now = new Date().toISOString();
  ...
  toast(
    wishes.length +
      ...
      (wishes.length === 1 ? "" : "s") +
      " moved to " +
      (target ? target.name : "General List"),
    "ok",
  );
}
window.moveSelectedStockedItemsToList = moveSelectedStockedItemsToList;

async function discardSelectedWishlistItems() {
  const ids = getSelectedWishlistIds();
  if (!ids.length) return toast("Select stocked items first", "info");
  const wishes = (
    await Promise.all(ids.map((id) => dbGet("wishlist", id)))
  ).filter(Boolean);
  const stocked = wishes.filter((wish) => wishStatus(wish) === "stocked");
  ...
  await renderWishlistPage();
  toast(
    stocked.length + " item" + (stocked.length === 1 ? "" : "s") + " discarded",
    "ok",
  );
}
window.discardSelectedWishlistItems = discardSelectedWishlistItems;
...
async function deleteSelectedWishlistItems() {
  ...
  if (
    ...
    return;
  for (const id of ids) await deleteWishlistItem(id, true);
  _selectedWishlistIds.clear();
  await renderWishlistPage();
...
window.deleteSelectedWishlistItems = deleteSelectedWishlistItems;
...
async function openWishlistBulkEdit(ids) {
  ...
  if (!records.length)
    return toast("Selected wishlist items were not found", "err");
  const fields = {
    name: records.map((wish) => String(wish.name || "")),
    unit: records.map((wish) => String(wish.unit || "Pcs")),
    qty: records.map((wish) => String(Number(wish.qty || 0))),
    estimatedCost: records.map((wish) =>
      String(Number(wish.estimatedCost || 0)),
    ),
    status: records.map((wish) => normalizeWishlistStatus(wish.status)),
    category: records.map((wish) => String(wish.type || wish.category || "")),
    supplierId: records.map((wish) =>
      String(wish.supplierId || wish.supplier || ""),
    ),
    note: records.map((wish) => String(wish.note || "")),
  };
  const values = {
    name: document.getElementById("wish-bulk-name"),
    unit: document.getElementById("wish-bulk-unit"),
    qty: document.getElementById("wish-bulk-qty"),
    estimatedCost: document.getElementById("wish-bulk-cost"),
...
async function saveWishlistBulkEdit() {
  ...
  const selectedFields = [
    ...document.querySelectorAll("[data-bulk-edit-field]:checked"),
  ].map((checkbox) => checkbox.dataset.bulkEditField);
  if (!ids.length) return closeWishlistBulkEdit();
  if (!selectedFields.length)
    return toast("Choose at least one field to update", "info");
  const statusRaw = document.getElementById("wish-bulk-status")?.value || "";
  if (selectedFields.includes("status") && !statusRaw)
    return toast("Choose a status before applying it", "info");
  const values = {
    name: Input.text("wish-bulk-name"),
    unit: Input.text("wish-bulk-unit") || "Pcs",
    qty: Number(document.getElementById("wish-bulk-qty")?.value || 0),
    estimatedCost: Number(
      document.getElementById("wish-bulk-cost")?.value || 0,
    ),
    status: normalizeWishlistStatus(statusRaw),
  ...
  for (const id of ids) {
    ...
    selectedFields.forEach((field) => {
      if (field === "category") {
        ...
        wish.qty = Math.max(0, values.qty);
      } else if (field === "estimatedCost") {
        wish.estimatedCost = Math.max(0, values.estimatedCost);
      } else {
  ...
  if (selectedFields.includes("status") && values.status === "stocked") {
  ...
  await renderStockMonitorSummary();
  toast(
    updated + " wishlist item" + (updated === 1 ? "" : "s") + " updated",
    "ok",
  );
}
...
function addSelectedWishlistItemsToSavedList(selectedListName) {
  ...
  if (!listName) return toast("Choose a saved list first", "info");
  if (!selectedIds.length) return toast("Select wishlist items first", "info");
  const list = getSavedWishlistLists().find((entry) => entry.name === listName);
  if (!list) return toast("Saved list not found", "err");
  const result = moveWishlistItemsToSavedList(selectedIds, list.id);
  if (!result.ok) return toast(result.reason, "info");
  _selectedWishlistIds.clear();
  renderWishlistPage();
  toast(
...
window.addSelectedWishlistItemsToSavedList =
  addSelectedWishlistItemsToSavedList;

let _selectedPurchaseBudgetId = "";
let _purchaseProcessing = null;

function normalizeWishlistStatus(value) {
  const status = String(value || "")
    .trim()
    .toLowerCase();
  if (status === "planned" || status === "prospective" || status === "new")
    return "planned";
  if (
    status === "in_progress" ||
    status === "in-progress" ||
...
async function markWishlistStockedById(
  wishId,
  itemId,
  stockedQty,
  actualBuyPrice,
  ...
  if (!wish) return;
  const now = new Date().toISOString();
  const requestedQty = Number(wish.requestedQty || wish.qty || stockedQty || 0);
  const purchaseQty = Number(stockedQty || requestedQty);
  const previousPurchased = Number(wish.purchasedQty || 0);
  const totalPurchased = previousPurchased + purchaseQty;
  ...
  wish.stockedAt =
    totalPurchased >= requestedQty ? now : wish.stockedAt || null;
  if (totalPurchased >= requestedQty) wish.dayPurchaseDate = null;
  wish.stockedItemId = itemId || null;
  if (
    wish.purchaseBudgetId &&
    db.objectStoreNames.contains("purchase_budgets")
  ) {
    const budget = await dbGet("purchase_budgets", wish.purchaseBudgetId);
    if (budget) {
      if (!Array.isArray(budget.purchaseHistory)) budget.purchaseHistory = [];
      budget.purchaseHistory.push({
        wishId: wish.id,
        itemId: itemId || null,
        name: wish.name || wish.code || "Item",
        qty: purchaseQty,
        actualBuyPrice: Number(actualBuyPrice || wish.estimatedCost || 0),
        total: purchaseQty * Number(actualBuyPrice || wish.estimatedCost || 0),
        date: now,
        createdBy: currentUser ? currentUser.username : "system",
      });
      await dbPut("purchase_budgets", budget);
    }
  }
  await dbPut("wishlist", wish);
  scheduleSync();
...
function normalizeWishVendorQuotes(wish) {
  ...
  return wish.vendorQuotes
    .map((q) => ({
      id:
        q.id ||
        "vq_" + Date.now() + "_" + Math.random().toString(36).slice(2, 8),
...
function sortWishVendorQuotes(quotes) {
  return [...quotes].sort(
    (a, b) => a.price - b.price || a.vendor.localeCompare(b.vendor),
  );
}
...
function buildWishVendorDetailHtml(quotes) {
  ...
  sorted.forEach((q, i) => {
    const best = i === 0;
    html +=
      '<div class="wd-vendor-row' +
      (best ? " wd-vendor-row-best" : "") +
      '">' +
      '<span class="wd-vendor-name">' +
      escapeHtml(q.vendor) +
      "</span>" +
      '<span class="wd-vendor-amt">' +
      fmt(q.price) +
...
function renderWishDetailItemInfo(wish) {
  ...
  const rows = [
    ...
    {
      label: "Supply",
      value: wish.supplierId || wish.supplier,
      icon: "fa-truck",
    },
  ...
  const html = rows
    ...
    .map((r) => {
      if (r.className === "price") {
        return (
          '<div class="wish-detail-dl-row price-row">' +
          '<span class="wish-detail-price-icon">KSh</span>' +
          '<div class="wish-detail-price-copy"><dt>' +
          escapeHtml(r.label) +
          "</dt>" +
          "<dd>" +
      ...
      return (
        ...
        '<span class="wish-detail-row-icon"><i class="fa-solid ' +
        r.icon +
        '"></i></span>' +
        "<dt>" +
        escapeHtml(r.label) +
...
function buildWishTableHtml(rows, wishById) {
  const activeRows = rows.filter(
    (row) => wishStatus(wishById.get(row.wishId) || {}) !== "stocked",
  );
  const totalQty = activeRows.reduce(
    (sum, row) =>
      sum + Number((wishById.get(row.wishId) || {}).qty || row.qty || 0),
    0,
  );
  const totalAmount = activeRows.reduce((sum, row) => {
    const wish = wishById.get(row.wishId) || {};
    return (
      sum + Number(wish.qty || row.qty || 0) * Number(wish.estimatedCost || 0)
    );
  }, 0);
  const stockedMoveOptions = getSavedWishlistLists()
    .map(
      (list) =>
        '<option value="' +
        escapeHtml(list.id) +
    ...
    .join("");
  const bulkActions =
    '<div id="wishlist-bulk-actions" class="wishlist-bulk-actions" hidden>' +
    '<button type="button" id="wishlist-add-selected-btn" class="wish-bulk-btn" onclick="addSelectedItemsToPendingWishlistList()" hidden><i class="fa-solid fa-arrow-right"></i> Add to current list</button>' +
    '<button type="button" class="wish-bulk-btn wish-bulk-stocked" onclick="markSelectedWishlistItemsStocked()"><i class="fa-solid fa-check"></i> Mark Stocked</button>' +
    ...
    '<select id="wishlist-stocked-destination" class="wish-bulk-destination" aria-label="Move stocked items to active list" onchange="if(this.value){moveSelectedStockedItemsToList(this.value);this.value=\'\';}"><option value="">Move stocked to...</option><option value="general">General List</option>' +
    stockedMoveOptions +
    "</select>" +
  ...
  return (
    ...
    rows
      .map((row) => {
        ...
        const estimatedPrice = Number(wish.estimatedCost || 0);
        const amount =
          qty > 0 && estimatedPrice > 0 ? fmt(qty * estimatedPrice) : "";
        const unit = String(wish.unit || "").trim();
        ...
        const statusMarkup =
          wishStatus(wish) === "stocked" || _activeWishlistView === "stocked"
            ? ""
            : '<div class="wish-row-status"><span class="' +
              statusClass +
              '">' +
        ...
        return (
          ...
          '"' +
          (isSelected ? " checked" : "") +
          ' onchange="event.stopPropagation();updateWishlistSelection()"></td>' +
          '<td class="wish-table-name"><div class="wish-table-name-main">' +
          escapeHtml(itemName) +
          "</div>" +
          statusMarkup +
          "</td>" +
      ...
      .join("") +
    '</tbody><tfoot><tr><th colspan="3">Visible totals</th><th>' +
    fmtN(totalQty) +
    "</th><th>" +
    escapeHtml(fmt(totalAmount)) +
    "</th></tr></tfoot></table></div>"
  );
}
...
async function openWishlistDetail(wishId) {
  ...
  await renderPurchaseListOptions(
    "wd-purchase-list",
    wish.purchaseBudgetId || "",
  );
  [
...
async function saveWishlistDetail() {
  ...
  wish.purchaseBudgetId =
    document.getElementById("wd-purchase-list")?.value || "";
  ...
  if (vendorName || vendorPriceRaw > 0) {
    if (!vendorName)
      return Validate.fail("Enter vendor name", "wd-vendor-name");
    if (!Validate.moneyRequired(vendorPriceRaw, "wd-vendor-price", "Amount"))
      return;
    const key = vendorName.toLowerCase();
    const existing = wish.vendorQuotes.find(
      (q) =>
        String(q.vendor || "")
          .trim()
          .toLowerCase() === key,
...
async function wishlistDetailEdit() {
  ...
  if (!id) return;
  closeWishlistDetail();
  await openEditWishlistItem(id);
}
window.wishlistDetailEdit = wishlistDetailEdit;
...
async function saveItem() {
  ...
  try {
    ...
    const editIdRaw =
      UI.el("edit-id")?.value || (_editingItemId ? String(_editingItemId) : "");
    ...
    if (editIdRaw && editIdRaw.startsWith("shoe_restock_")) {
      ...
      if (!sizeRec) {
        toast("Size record not found", "err");
        return;
      }
      ...
      const nextSell = !isNaN(sell)
        ? sell
        : sizeRec.sellPrice || item?.sellPrice || item?.sell || 0;
      if (!Validate.price(nextBuy, nextSell, "f-buy", "f-sell")) return;
      const newQty = (sizeRec.qty || 0) + addQty;
      if (newQty > 999999) {
        toast("Warning: Exceeds max 999,999", "err");
        return;
      }
      sizeRec.qty = newQty;
    ...
    if (editIdRaw && editIdRaw.startsWith("shoe_edit_")) {
      ...
      const buy =
        parsePriceInput(UI.el("f-buy")?.value) || sizeRec.buyPrice || 0;
      const sell =
        parsePriceInput(UI.el("f-sell")?.value) || sizeRec.sellPrice || 0;
      if (isNaN(qty) || qty < 0)
        return Validate.fail("Enter a valid quantity (0 or more)", "f-qty");
      if (!Validate.price(buy, sell, "f-buy", "f-sell")) return;
      sizeRec.qty = qty;
      sizeRec.buyPrice = buy;
      sizeRec.sellPrice = sell;
    ...
    if (editIdRaw && editIdRaw.startsWith("restock_")) {
      const existing = await dbGet(
        "items",
        parseInt(editIdRaw.replace("restock_", "")),
      );
      if (!existing) {
      ...
      if (
        addQty > CODE_MAX_QTY &&
        !confirm("Adding " + addQty + " units - confirm?")
      )
        return;
      ...
      fbSyncItem(existing);
      const actualBuy =
        _purchaseProcessing?.wishId === _wishStockingFromId
          ? _purchaseProcessing.unitBuy
          : existing.buyPrice || existing.buy || 0;
      await recordStockInvestment(
        existing,
        addQty * actualBuy,
        addQty,
      ...
      exitRestockMode();
      toast(
        "\U0001f4e6 " + existing.code + ": +" + addQty + " to " + newQty,
        "ok",
      );
      return;
    }
    ...
    if (!editIdRaw) {
      ...
      const addingShoeSizes =
        existingCode && isFootwearType(type) && existingCode.isShoe;
      if (existingCode && !addingShoeSizes) {
        showCodeDropdown(codeMatches, code);
        toast(
          "Warning: Item code already exists - select it from the dropdown",
          "err",
        );
        UI.el("f-code")?.focus();
        return;
    ...
    if (isFootwearType(type) && !editIdRaw && !isRecord) {
      ...
      if (_wishStockingFromId) {
        ...
        if (stocked)
          await markWishlistStockedById(_wishStockingFromId, stocked.id);
        _wishStockingFromId = null;
      }
      clearForm();
      clearAddFormPhoto();
    ...
    if (
      !isRecord &&
      qtyRaw !== "" &&
      (!Number.isFinite(qty) || !Number.isInteger(qty))
    )
      return Validate.fail("Enter a whole-number quantity", "f-qty");
    if (!isRecord && qty < 0)
      return Validate.fail("Quantity cannot be negative", "f-qty");
    if (!isRecord && qty > 999999)
      return Validate.fail("Quantity exceeds maximum (999,999)", "f-qty");
    if (
      !isRecord &&
      qty > CODE_MAX_QTY &&
      !confirm("Adding " + qty + " units - confirm?")
    )
      return;
    // Record Only: buy price is the one mandatory pricing field
    if (isRecord && (!buy || buy <= 0))
      return Validate.fail("Buy price is required for Record items", "f-buy");
    if (
      !Validate.moneyOptional(buyRaw === "" ? null : buy, "f-buy", "Buy price")
    )
      return;
    if (
      !Validate.moneyOptional(
        sellRaw === "" ? null : sell,
        "f-sell",
        "Sell price",
      )
    ...
    if (sellPriceMin > 0 && sell > 0 && sellPriceMin >= sell) {
      return Validate.fail(
        "Min sell price must be lower than sell price (" + fmt(sell) + ")",
        "f-sell-min",
      );
    }
    ...
    if (editIdRaw) {
      ...
      if (!resolvedId || isNaN(resolvedId)) {
        toast("Warning: Cannot save: item ID missing", "err");
        return;
      }
      ...
      await recordStockInvestment(item, qty * actualBuy, qty, "New stock");
      fbSyncItem(item);
    ...
    if (err.name === "ConstraintError") {
      toast(
        "Warning: Code already exists - select from dropdown to restock",
        "err",
      );
    } else {
...
function clearForm() {
  ...
  const _defaultType = types.find(
    (t) => t.name === "General" && isCategoryActive(t),
  )
    ? "General"
    : "";
...
async function findCodeMatchesForSave(code) {
  ...
  const startsWith = unique.filter(
    (i) => i.code !== clean && i.code.startsWith(clean),
  );
  const contains = unique.filter(
    (i) =>
      i.code !== clean && !i.code.startsWith(clean) && i.code.includes(clean),
  );
  const nameMatch = unique.filter(
    (i) =>
      i.name &&
...
async function onCodeInput() {
  ...
  const matches = [...exact, ...startsWith, ...contains, ...nameMatch].slice(
    0,
    10,
  );

  if (!matches.length) {
    clearCodeMatchSelect("No match");
    hideCodeDropdown();
    return;
  }
  ...
  if (
    ...
    exactMatches[0].isShoe &&
    isAddFormFootwearContext() &&
    !UI.el("edit-id")?.value
  ) {
...
async function preloadShoeSizesForAdd(code) {
  ...
  records.forEach((sz) =>
    groupsNeeded.add(sz.sizeGroup || _shoeState.groupFor(sz.size)),
  );
  ...
  const szWrap = UI.el("shoe-rows-wrap");
  if (szWrap)
    szWrap.style.display = _shoeState.sizes.size > 0 ? "block" : "none";
  renderShoeGroupButtons();
  renderShoeSummary();
...
async function preloadShoeSizesForRestock(code) {
  ...
  records
    .filter((r) => (r.qty || 0) > 0)
    .forEach((r) => _shoeState.lockedSizes.add(Number(r.size)));
  const grid = UI.el("sz-grid");
  if (grid) grid.innerHTML = "";
  renderAllShoeGroupCards();
  showShoePricingPanel();
...
async function openShoeCollectionRestock(code) {
  ...
  setTimeout(async () => {
    ...
    if (ml) {
      ml.hidden = false;
      ml.textContent =
        "Restock " + item.code + " - missing & out-of-stock sizes only";
    }
...
function showCodeDropdown(items, typedCode) {
  ...
  if (select) {
    ...
    select.innerHTML =
      '<option value="">Match existing code...</option>' +
      items
        .map(
          (item) =>
            '<option value="' +
...
async function selectExistingItem(itemId) {
  try {
    ...
    console.error("[selectExistingItem]", e);
    toast("Error: " + e.message, "err");
  }
}
...
async function renderList() {
  ...
  for (const _gi of filtered) {
    ...
    if (_gtObj && _gtObj.parentId) {
      ...
      if (_gpt) {
        if (!_typeGroupMap[_gpt.id])
          _typeGroupMap[_gpt.id] = { parentType: _gpt, items: [] };
  ...
  for (const item of filtered) {
    ...
    if (item.isShoe) {
      ...
      const groupTotalPcs = sizes.reduce(
        (s, sz) => s + (ledgerSizeQty.get(sz.id) || 0),
        0,
      );
      ...
      const groupBuyCost = sizes.reduce(
        (s, sz) =>
          s + (sz.buyPrice || item.buyPrice || item.buy || 0) * (sz.qty || 0),
        0,
      );
      let _grpRevenue = 0;
      allSales
      ...
      if (!isExpanded) {
        ...
        const filteredSizes =
          activeSgf === "all"
            ? sizes
            : sizes.filter((sz) => sz.sizeGroup === activeSgf);
      ...
      const variants = allSizes
        .filter((s) => s.itemCode === item.code)
        .sort((a, b) =>
          String(a.sizeGroup || "").localeCompare(String(b.sizeGroup || "")),
        );
      ...
      const groupTotalPcs = variants.reduce(
        (s, v) => s + (ledgerSizeQty.get(v.id) || 0),
        0,
      );
      ...
      const groupTotalQty = groupItems.reduce(
        (s, i) => s + (ledgerItemQty.get(i.id) || 0),
        0,
      );
      const groupRevenue = allSales
        .filter((s) => groupItems.some((i) => i.id === s.itemId))
        .reduce((s, x) => s + (x.revenue || 0), 0);
      const allOut = groupItems.every(
        (i) => (ledgerItemQty.get(i.id) || 0) <= 0,
      );
      const hasOut = groupItems.some(
        (i) => (ledgerItemQty.get(i.id) || 0) <= 0,
      );
...
async function getStockMonitorRows() {
  ...
  await enrichShoeItems(items);
  const sizes = await dbAll("shoe_sizes");
  const wishlist = db.objectStoreNames.contains("wishlist")
    ? await dbAll("wishlist")
    : [];
  const rows = [];
  ...
  for (const item of items) {
    if (item.isShoe) {
      for (const sz of sizes.filter(
        (record) => record.itemCode === item.code,
      )) {
  ...
  wishlist
    ...
    .forEach((w) =>
      rows.push({
        kind:
          (w.status || "") === "unaccounted" ? "unaccounted" : "prospective",
        wishId: w.id,
        ...
        note: w.note || "",
        label:
          (w.status || "") === "unaccounted" ? "Not accounted" : "Prospective",
      }),
    );
...
async function renderStockMonitor() {
  const targets = [
    {
      list: document.getElementById("stock-monitor-list"),
      counts: document.getElementById("stock-monitor-counts"),
      sheetOnly: true,
    },
    {
      list: document.getElementById("inventory-monitor-list"),
      counts: document.getElementById("inventory-monitor-counts"),
      sheetOnly: false,
    },
  ].filter((t) => t.list);
  ...
  const rows = allRows.filter(
    (row) => row.kind === "out" || row.kind === "unaccounted",
  );
  const outCount = rows.filter((row) => row.kind === "out").length;
  const unaccountedCount = rows.filter(
    (row) => row.kind === "unaccounted",
  ).length;
  const html = rows.length
    ? rows
        .map((row) => {
          ...
          const deleteBtn =
            row.kind !== "out"
              ? '<button class="stock-monitor-action delete" onclick="event.stopPropagation();deleteWishlistItem(' +
                row.wishId +
                ')" title="Remove"><i class="fa-solid fa-trash"></i></button>'
              : "";
          return (
            '<div class="stock-monitor-row ' +
            cls +
            '" onclick="' +
            restockAction +
            '" role="button" tabindex="0">' +
            '<div class="stock-monitor-body">' +
            '<div class="stock-monitor-name">' +
            escapeHtml(row.name) +
            "</div>" +
            '<div class="stock-monitor-meta">' +
            escapeHtml(row.code || "No code") +
            ...
            status +
            (row.size
              ? '<span class="tag tag-gray">Size ' +
                escapeHtml(row.size) +
                "</span>"
              : "") +
            (row.note
              ? '<span class="tag tag-gray">' + escapeHtml(row.note) + "</span>"
              : "") +
            "</div>" +
            "</div>" +
            '<div class="stock-monitor-actions">' +
            '<button class="stock-monitor-action restock" onclick="event.stopPropagation();' +
            restockAction +
            '" title="Restock"><i class="fa-solid fa-boxes-stacked"></i></button>' +
            deleteBtn +
            "</div>" +
            "</div>"
          );
        ...
        .join("")
    : '<div class="empty" style="padding:28px 12px;"><div class="e-icon">OK</div><p>No monitored items.</p></div>';

  targets.forEach((target) => {
    if (target.counts) {
      target.counts.innerHTML =
        '<div class="stock-monitor-pill red">' +
        outCount +
        " Out of stock</div>" +
        '<div class="stock-monitor-pill blue">' +
        unaccountedCount +
        " Not accounted</div>";
    }
  ...
  if (isAdd) {
    ...
    if (context) {
      ...
      context.innerHTML = currentList
        ? '<i class="fa-solid fa-list-check"></i> Choose the destination list below. Main list is always the master list.'
        : "";
    }
  }
...
function setWishCurrentStep(stepName, shouldFocus) {
  document.querySelectorAll(".wish-question-step").forEach((step) => {
    const active = step.dataset.wishStep === stepName;
    step.classList.toggle("is-current", active);
    step.setAttribute("aria-hidden", active ? "false" : "true");
  });
  document.querySelectorAll(".wish-question-step-tab").forEach((tab) => {
    tab.classList.toggle("is-current", tab.dataset.stepTarget === stepName);
    tab.setAttribute(
      "aria-selected",
      tab.dataset.stepTarget === stepName ? "true" : "false",
    );
  });
  if (shouldFocus) {
    const current = document.querySelector(
      '.wish-question-step[data-wish-step="' + stepName + '"]',
    );
    current
...
function updateWishFormProgress() {
  const steps = [
    Boolean(Input.text("wish-name")),
    Boolean(
      Input.text("wish-type") ||
      Input.text("wish-category") ||
      Input.raw("wish-cost"),
    ),
  ...
  const label = document.getElementById("wish-form-progress-label");
  if (label) label.textContent = answered + " of 4 answered";
  const stepEls = Array.from(document.querySelectorAll(".wish-question-step"));
  document.querySelectorAll(".wish-question-step-tab").forEach((tab) => {
    const index = ["item", "target", "plan", "context"].indexOf(
      tab.dataset.stepTarget,
    );
    tab.classList.toggle("is-answered", !!steps[index]);
  });
  stepEls.forEach((step, index) => {
    step.classList.toggle("is-answered", !!steps[index]);
  });
}
window.updateWishFormProgress = updateWishFormProgress;

function focusWishStep(stepName) {
  const step = document.querySelector(
    '.wish-question-step[data-wish-step="' + stepName + '"]',
  );
  if (!step) return;
  setWishCurrentStep(stepName, false);
  step.scrollIntoView({ behavior: "smooth", block: "start" });
  step.querySelector("input, select, button")?.focus({ preventScroll: true });
}
window.focusWishStep = focusWishStep;
function toggleWishAddMore(forceOpen) {
  ...
  if (label)
    label.textContent = open ? "Hide other fields" : "Show other fields";
}
...
async function renderWishlistPage() {
  ...
  const allWishes = db.objectStoreNames.contains("wishlist")
    ? await dbAll("wishlist")
    : [];
  ...
  if (controls) {
    if (showList) {
      controls.innerHTML =
        '<div class="wish-view-tabs" role="tablist" aria-label="Wishlist views">' +
        '<button type="button" class="wish-view-tab' +
        (_activeWishlistView === "saved" ? " active" : "") +
        '" role="tab" aria-selected="' +
        (_activeWishlistView === "saved") +
        '" onclick="chooseWishlistSavedList()"><i class="fa-solid fa-list-ul"></i> Wish lists</button>' +
        '<button type="button" class="wish-view-tab' +
        (_activeWishlistView === "main" ? " active" : "") +
        '" role="tab" aria-selected="' +
        (_activeWishlistView === "main") +
        '" onclick="setWishlistView(\'main\')"><i class="fa-solid fa-layer-group"></i> Main list</button>' +
        '<button type="button" class="wish-view-tab' +
        (_activeWishlistView === "stocked" ? " active" : "") +
        '" role="tab" aria-selected="' +
        (_activeWishlistView === "stocked") +
        '" onclick="setWishlistView(\'stocked\')">Stocked</button>' +
        "</div>" +
        (_activeWishlistView === "saved"
          ? renderSavedWishlistListsPanel(savedLists, allWishes)
          : "") +
  ...
  let filteredWishes = allWishes
    ...
    .filter((wish) => {
      if (_activeWishlistView === "stocked")
        return wishStatus(wish) === "stocked";
      if (_activeWishlistView === "saved") {
        const savedList = savedLists.find(
          (list) => list.name === _activeWishlistSavedList,
        );
        return Boolean(
          savedList &&
          savedList.itemIds.includes(wish.id) &&
          wishStatus(wish) !== "stocked" &&
          wishStatus(wish) !== "discarded",
        );
      }
      return (
        wishStatus(wish) !== "stocked" &&
        wishStatus(wish) !== "discarded" &&
        !getWishlistListMembership(wish.id, savedLists)
      );
    })
    .sort((a, b) => {
      const aStocked = wishStatus(a) === "stocked" ? 1 : 0;
  ...
  const wishlistRecords = allWishes.filter(
    (wish) =>
      !isWishlistSaleMonitorEntry(wish) && wishStatus(wish) !== "discarded",
  );
  const activeCount = wishlistRecords.filter(
    (wish) => wishStatus(wish) !== "stocked",
  ).length;
  const stockedCount = wishlistRecords.filter(
    (wish) => wishStatus(wish) === "stocked",
  ).length;
  const visibleActiveWishes = filteredWishes.filter(
    (wish) => wishStatus(wish) !== "stocked",
  );
  const visibleQty = visibleActiveWishes.reduce(
    (sum, wish) => sum + Number(wish.qty || 0),
    0,
  );
  ...
  const summary = document.getElementById("wish-list-summary");
  if (summary) {
    summary.style.display = _activeWishlistView === "saved" ? "none" : "";
    if (_activeWishlistView !== "saved") {
      summary.innerHTML =
        "<span><strong>" +
  ...
  const wishSub = document.getElementById("wishlist-sub");
  if (wishSub)
    wishSub.textContent = rows.length
      ? rows.length + " item" + (rows.length === 1 ? "" : "s")
      : "Nothing yet";

  if (!rows.length) {
    list.innerHTML =
      '<div class="wish-empty"><div class="wish-empty-icon">+</div><p class="wish-empty-title">No item to display.</p>' +
      (!showAdd
        ? '<button type="button" class="wish-empty-btn" onclick="window.showWishlistSection(\'add\')">Add item</button>'
        : "") +
      "</div>";
    return;
  }
  ...
  list.innerHTML = buildWishTableHtml(rows, wishById);
  updateWishlistSelection();
}

async function toggleWishlistStockedState(wishId) {
  const wish = await dbGet("wishlist", wishId);
  if (!wish) return;
  const isStocked = wishStatus(wish) === "stocked";
  if (isStocked) {
    ...
    if (
      confirm(
        'Delete "' + label + '" from the wishlist? This cannot be undone.',
      )
    ) {
      await deleteWishlistItem(wishId);
      toast("Wishlist item deleted", "ok");
      return;
    }
    await renderWishlistPage();
    return;
  ...
  await renderWishlistPage();
  await renderStockMonitorSummary();
  toast("Marked as stocked", "ok");
}
window.toggleWishlistStockedState = toggleWishlistStockedState;

async function saveWishlistItem() {
  const isEditing = !!_editingWishlistId;
  const name = Input.text("wish-name");
  const unitField = document.getElementById("wish-unit");
  const typeField = document.getElementById("wish-type");
  const noteField = document.getElementById("wish-note");
  const statusField = document.getElementById("wish-status");
  const unit = unitField ? Input.text("wish-unit") || "Pcs" : "Pcs";
  const type = typeField ? Input.text("wish-type") : "";
  const note = noteField ? Input.text("wish-note") : "";
  const status = statusField
    ? normalizeWishlistStatus(Input.text("wish-status"))
    : "planned";
  ...
  if (isEditing) {
    ...
    Object.assign(existing, {
      name,
      unit,
      type: typeField ? type : existing.type || existing.category || "",
      category: typeField ? type : existing.category || existing.type || "",
      qty,
      estimatedCost: estimatedCost > 0 ? estimatedCost : 0,
      supplierId,
      note: noteField ? note : existing.note || "",
      status: statusField ? status : existing.status || "planned",
      updatedAt: new Date().toISOString(),
      updatedBy: currentUser ? currentUser.username : "system",
    });
    ...
    if (
      wishStatus(existing) === "stocked" ||
      wishStatus(existing) === "discarded"
    ) {
      moveWishlistItemsToGeneralList([existing.id]);
    }
    _editingWishlistId = null;
    clearWishPhotoForm();
    ["wish-name", "wish-qty", "wish-cost", "wish-supplier"].forEach((id) => {
      const el = document.getElementById(id);
      if (el) el.value = "";
    });
    scheduleSync();
    ...
    await renderWishlistPage();
    await renderStockMonitorSummary();
    toast("Wishlist item updated", "ok");
    return;
  }
  ...
  await renderWishlistPage();
  await renderStockMonitorSummary();
  const budgetSummary = await computePurchaseBudget();
  toast(
    budgetSummary.unfunded > 0
      ? "Added to wishlist - budget warning"
      : "Added to wishlist",
    budgetSummary.unfunded > 0 ? "info" : "ok",
  );
}

async function openEditWishlistItem(wishId) {
  const wish = await dbGet("wishlist", wishId);
  if (!wish) {
    toast("Wishlist item not found", "err");
    return;
  ...
  if (saveBtn)
    saveBtn.innerHTML =
      '<i class="fa-solid fa-floppy-disk"></i> Update wishlist item';
  const addTitle = document.querySelector(
    "#wishlist-add-panel .wish-section-title",
  );
  if (addTitle)
    addTitle.innerHTML =
      '<span class="wish-section-icon"><i class="fa-solid fa-pen"></i></span>Edit wishlist item';
  setTimeout(() => document.getElementById("wish-name")?.focus(), 100);
}
window.openEditWishlistItem = openEditWishlistItem;
...
function cancelWishlistEdit() {
  ...
  if (saveBtn)
    saveBtn.innerHTML =
      '<i class="fa-solid fa-floppy-disk"></i> Save to wishlist';
  const addTitle = document.querySelector(
    "#wishlist-add-panel .wish-section-title",
  );
  if (addTitle)
    addTitle.innerHTML =
      '<span class="wish-section-icon"><i class="fa-solid fa-cube"></i></span>What do you want to stock? <span class="add-required">*</span>';
  [
    "wish-name",
    "wish-unit",
    "wish-qty",
...
async function markWishlistStockedForItem(item) {
  ...
  const codeMatches = wishlist.filter((wish) => {
    if (wishStatus(wish) === "stocked") return false;
    const wishCode = (wish.code || "").trim().toLowerCase();
    return itemCode && wishCode && itemCode === wishCode;
  });
  const nameMatches = wishlist.filter((wish) => {
    if (wishStatus(wish) === "stocked") return false;
    const wishName = (wish.name || "").trim().toLowerCase();
    return itemName && wishName && itemName === wishName;
  });
  const match = (codeMatches.length ? codeMatches : nameMatches).sort(
    (a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0),
  )[0];
  if (!match) return;
  match.status = "stocked";
  match.stockedAt = new Date().toISOString();
  match.stockedItemId = item.id || null;
  await dbPut("wishlist", match);
  moveWishlistItemsToGeneralList([match.id]);
}
...
async function startWishlistRestock(wishId) {
  const wish = await dbGet("wishlist", wishId);
  if (!wish) return;
  const existingItems = await dbAll("items");
  const existing = existingItems.find(
    (item) =>
      (wish.code &&
        item.code &&
        item.code.toLowerCase() === wish.code.toLowerCase()) ||
      (!wish.code &&
        wish.name &&
        item.name &&
        item.name.toLowerCase() === wish.name.toLowerCase()),
  );
  ...
  setTimeout(() => {
    setItemMode(false);
    if (existing && _purchaseProcessing?.wishId === wishId) {
      UI.el("edit-id").value = "restock_" + existing.id;
      UI.el("f-code").value = existing.code || wish.code || "";
      UI.el("f-name").value = existing.name || wish.name || "";
      UI.el("f-qty").value = _purchaseProcessing.qty;
      UI.el("f-buy").value = _purchaseProcessing.unitBuy;
      setRestockBanner(false);
      showRestockView({
        code: existing.code,
        name: existing.name,
        type: existing.type,
        stock: existing.qty || 0,
        buy: existing.buyPrice || wish.estimatedCost || 0,
        sell: existing.sellPrice || 0,
      });
      updateProfitPreview();
      return;
    }
    ...
    if (UI.el("f-qty"))
      UI.el("f-qty").value =
        _purchaseProcessing?.wishId === wishId
          ? _purchaseProcessing.qty
          : wish.qty || 1;
    if (UI.el("f-buy"))
      UI.el("f-buy").value =
        _purchaseProcessing?.wishId === wishId
...
async function openShoeSizeCard(itemCode, size) {
  ...
  const groupLbl =
    getShoeGroups()[sizeRec.sizeGroup]?.label ||
    sizeRec.sizeGroup ||
    "Size group";
  ...
  if (!sheet) {
    ...
    sheet.innerHTML =
      '<div class="sheet detail-sheet-unified" id="shoe-size-action-inner"></div>';
    sheet.addEventListener("click", (e) => {
      if (e.target === sheet) closeShoeSizeActions();
    });
    document.body.appendChild(sheet);
  }
  ...
  inner.innerHTML = `
    ...
    <div class="detail-sell-wrap">
      ${
        !isOut
          ? `<button type="button" class="detail-sell-btn" onclick="closeShoeSizeActions();closeSheet();openSellShoeModal(${item.id},${size})">SELL - Size ${size}</button>`
...
function toggleDetailShoeSize(itemId, size) {
  ...
  document.querySelectorAll("#sh-shoe-sizes [data-sh-size]").forEach((btn) => {
    btn.classList.toggle(
      "selected",
      _selectedShoeSizes.has(Number(btn.dataset.shSize)),
    );
  });
...
function _updateDetailShoeSelectionBar(itemId) {
  ...
  bar.innerHTML =
    ...
    '<span style="font-family:var(--mono);font-size:13px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' +
    selected.join(", ") +
    "</span>" +
    '<button onclick="restockSelectedShoeSizes(' +
    itemId +
    ')" style="padding:7px 12px;background:var(--green);color:white;border:none;border-radius:var(--r);font-size:12px;font-weight:800;cursor:pointer;font-family:var(--sans);flex-shrink:0;">Restock</button>' +
    "</div>";
}
...
function openBulkShoeRestockSheet(itemId, sizes) {
  ...
  if (!sheet) {
    ...
    sheet.addEventListener("click", (e) => {
      if (e.target === sheet) closeBulkShoeRestock();
    });
    document.body.appendChild(sheet);
  }
  ...
  inner.innerHTML =
    ...
    '<div style="font-size:14px;font-weight:900;font-family:var(--mono);color:var(--accent);margin-bottom:12px;">' +
    sizes.join(", ") +
    "</div>" +
  ...
  sheet.classList.add("open");
  setTimeout(
    () => document.getElementById("bulk-shoe-restock-qty")?.focus(),
    80,
  );
}
...
async function confirmBulkShoeRestock() {
  ...
  const qty = parseInt(
    document.getElementById("bulk-shoe-restock-qty")?.value || "0",
  );
  ...
  await recordStockInvestment(
    ...
    changed.reduce(
      (sum, rec) =>
        sum + qty * (rec.buyPrice || rec.buy || item.buyPrice || item.buy || 0),
      0,
    ),
    qty * changed.length,
    "Shoe restock",
  ...
  setTimeout(async () => {
    ...
    if (item.isShoe) {
      ...
      if (item.qty <= 0 && !item.isRecord) {
        toast("Warning: Out of stock", "err");
        return;
      }
      openSellModal(id);
    }
...
async function openSheet(id) {
  ...
  const photoPan =
    ...
    document.getElementById("sh-photo-area-inner");
  const panHint = document.getElementById("sh-photo-pan-hint");
  if (photo) {
    photoImg.src = photo;
    if (photoPan) {
      const panEl = document.getElementById("sh-photo-pan");
  ...
  const tbadge = document.getElementById("sh-type-badge");
  if (tbadge) tbadge.textContent = t.emoji + " " + item.type;
  ...
  if (item.qty <= 0 && !item.isShoe && !item.isRecord) {
    ...
    if (sellBtn) {
      ...
      sellBtn.textContent = "OUT OF STOCK";
    }
  } else {
    ...
    if (sellBtn) {
      ...
      sellBtn.textContent = item.isShoe ? "SELECT SIZE & SELL" : "SELL";
    }
  }
  ...
  const restockBtn = document.querySelector(
    "#detail-sheet .detail-action-restock",
  );
  ...
  if (actionRow) {
    ...
    actionRow.style.justifyContent = item.isShoe
      ? "flex-start"
      : "space-between";
  }
...
async function deleteItem() {
  try {
    ...
    const allSales = await dbAll("sales");
    const itemSales = allSales.filter(
      (s) => s.itemId === currentDetailId || s.itemCode === toDelete.code,
    );
    let msg = 'Delete "' + (toDelete.name || toDelete.code) + '"?';
    if (itemSales.length > 0)
      msg +=
        "\n\nWarning: This item has " +
        itemSales.length +
        " sale record(s). The sales history will remain but the item cannot be restocked.";
    if (!confirm(msg)) return;
    if (toDelete.isShoe) {
...
async function editItem() {
  try {
    ...
    if (item.isShoe) {
      ...
      if (!size) {
        toast("Warning: Select a size first before editing", "err");
        setTimeout(() => openSheet(item.id), 100);
        return;
      }
      const sizes = await getShoeSizes(item.code);
      ...
      if (_ml1) {
        _ml1.hidden = false;
        _ml1.textContent = "Edit - " + item.code + " Size " + size;
      }
      UI.el("cancel-edit-btn").style.display = "block";
    ...
    if (_ml2) {
      _ml2.hidden = false;
      _ml2.textContent = "Edit - " + (item.name || item.code);
    }
    UI.el("cancel-edit-btn").style.display = "block";
    _editOriginItemId = item.id;
    onTypeChange();
...
function dashSetPeriod(p) {
  _dashPeriod = p;
  document
    .querySelectorAll('[id^="dash-period-"]')
    .forEach((b) => b.classList.remove("active"));
...
function _dashSumCard(icon, val, lbl, note, tone, navTarget) {
  ...
  const navAttr = navTarget
    ? ' class="dash-sum-card dash-sum-card-link" role="button" tabindex="0" onclick="goDashNav(\'' +
      navTarget +
      "')\" onkeydown=\"if(event.key==='Enter'||event.key===' '){event.preventDefault();goDashNav('" +
      navTarget +
      "');}\""
    : ' class="dash-sum-card"';
  return (
    ...
    navAttr +
    ">" +
    '<div class="dash-sum-card-icon">' +
    icon +
    "</div>" +
    '<div class="dash-sum-card-val"' +
    toneStyle +
    ">" +
    val +
    "</div>" +
    '<div class="dash-sum-card-lbl">' +
    lbl +
    "</div>" +
...
async function _renderDashSummary(ctx) {
  ...
  try {
    if (db.objectStoreNames.contains("wishlist")) {
      wishCount = (await dbAll("wishlist")).filter(
        (w) => w.status !== "stocked",
      ).length;
    }
  } catch (_) {
  ...
  let money = null;
  try {
    money = await _computeFinanceMovement();
  } catch (_) {
    /* intentionally ignored */
  }
  ...
  if (_dashPeriod !== "all" && prevRev > 0) {
    ...
    trendNote =
      chg >= 0
        ? "↑ " + chg.toFixed(0) + "% vs prior period"
        : "↓ " + Math.abs(chg).toFixed(0) + "% vs prior period";
  ...
  if (totalItems === 0) {
    headlineChips.push({
      icon: "box-open",
      text: "Add items to start tracking",
      tone: "muted",
    });
  } else {
    ...
    if (totalSalesCount > 0) {
      headlineChips.push({
        icon: "chart-line",
        text: fmtN(totalSalesCount) + " sales · " + fmt(totalRevenue),
        tone: "sales",
      });
    }
    if (alertCount > 0) {
      headlineChips.push({
        ...
        text: alertCount + " stock alert" + (alertCount !== 1 ? "s" : ""),
        tone: "warn",
      });
  ...
  if (_headlineEl) {
    _headlineEl.innerHTML = headlineChips
      .map(
        (c) =>
          '<span class="dash-hchip dash-hchip-' +
          c.tone +
          '"><i class="fa-solid fa-' +
          c.icon +
          '"></i>' +
  ...
  cards.push(
    _dashSumCard(
      "",
      fmtN(totalPiecesSold),
      "Items sold for this period",
      null,
      null,
      "itemsSold",
    ),
  ...
  cards.push(
    _dashSumCard(
      ...
      periodLbl + " revenue",
      trendNote ||
        fmt(totalProfitEarned) + " profit - " + margin.toFixed(1) + "% margin",
      totalProfitEarned >= 0 ? "var(--green)" : "var(--red)",
    ),
  );
  cards.push(
    _dashSumCard(
      "",
      fmtN(totalSalesCount),
      "Sales",
      totalSalesCount
        ? "Avg " + fmt(totalRevenue / totalSalesCount) + " per sale"
        : "No sales in period",
    ),
  );
  cards.push(
    _dashSumCard(
      ...
      _dashPeriod === "today" && todayDashSales.length
        ? fmtN(todayDashSales.length) +
            " sales - " +
            fmt(todayDashProf) +
            " profit today"
        : money
          ? "Pool " + fmt(money.businessPool)
          : dayLabel,
    ),
  );
  if (wishCount > 0) {
  ...
  if (alertCount > 0) {
    cards.push(
      _dashSumCard(
        ...
        "Needs attention",
        (outStk.length ? outStk.length + " out - " : "") +
          (lowStk.length ? lowStk.length + " low" : ""),
        "var(--amber)",
        "stock",
      ),
    );
  ...
  if (money && (money.businessPool || money.salesProfit)) {
    cards.push(
      _dashSumCard(
        "",
        fmt(money.businessPool),
        "Business pool",
        "Profit " +
          fmt(money.salesProfit) +
          " - cost out " +
          fmt(money.salesCostOut),
        money.businessPool >= 0 ? "var(--accent2)" : "var(--red)",
      ),
    );
  }
...
async function renderDashboard() {
  ...
  if (sparkWrap && sparkEl) {
    ...
    const dayRevs = days7.map((date) =>
      allSales
        .filter(
          (s) => (s.businessDate || (s.date || "").split("T")[0]) === date,
        )
        .reduce((s, x) => s + (x.revenue || 0), 0),
    );
    ...
    if (hasData) {
      ...
      sparkEl.innerHTML = dayRevs
        ...
        .join("");
      if (sparkLbls)
        sparkLbls.innerHTML = days7
          .map(
            (d, i) =>
              `<div style="flex:1;text-align:center;font-size:9px;color:var(--muted);font-weight:${days7[i] === today ? 800 : 600};">${new Date(d + "T12:00:00").toLocaleDateString("en-GB", { weekday: "short" }).slice(0, 2)}</div>`,
          )
          .join("");
    }
  }
...
function getMotivationMessages() {
  ...
  if (!Array.isArray(saved) || !saved.length) {
    saved = DEFAULT_SALE_MOTIVATIONS.map((text, index) => {
      ...
      return {
        id: "default_" + index,
        category: split > 0 ? text.slice(0, split) : "Keep Going",
        text: split > 0 ? text.slice(split + 2) : text,
        builtIn: true,
      };
    });
  ...
  return saved.filter(
    (m) =>
      m &&
      MOTIVATION_CATEGORIES.includes(m.category) &&
      String(m.text || "").trim(),
  );
}
...
function removeMotivationMessage(id) {
  ...
  if (messages.length <= 1)
    return toast("Keep at least one motivation message", "err");
...
function renderMotivationSettings() {
  ...
  list.innerHTML = MOTIVATION_CATEGORIES.map((category) => {
    ...
    return (
      '<div class="motivation-category-group">' +
      '<div class="motivation-category-heading">' +
      escapeHtml(category) +
      "<span>" +
      ...
      categoryMessages
        .map(
          (m) =>
            ...
            '<div class="motivation-message-text">' +
            escapeHtml(m.text) +
            "</div>" +
            '<button type="button" class="motivation-delete-btn" onclick="removeMotivationMessage(\'' +
            escapeHtml(m.id) +
            '\')" aria-label="Remove message" title="Remove message"><i class="fa-solid fa-trash"></i></button>' +
            "</div>",
        )
        .join("") +
      "</div>"
...
function getSaleMotivation() {
  ...
  let state = {};
  try {
    state = JSON.parse(localStorage.getItem(MOTIVATION_STATE_KEY) || "{}");
  } catch (_) {}
  if (state.year !== year)
    state = { year, used: [], categoryCounts: {}, lastUsed: {} };
  if (!Array.isArray(state.used)) state.used = [];
  if (!state.categoryCounts || typeof state.categoryCounts !== "object")
    state.categoryCounts = {};
  if (!state.lastUsed || typeof state.lastUsed !== "object")
    state.lastUsed = {};
  ...
  const minCount = Math.min(
    ...candidates.map((m) => state.categoryCounts[m.category] || 0),
  );
  const balanced = candidates.filter(
    (m) => (state.categoryCounts[m.category] || 0) === minCount,
  );
  const chosen =
    balanced[Math.floor(Math.random() * balanced.length)] || candidates[0];
  state.used.push(chosen.id);
  state.categoryCounts[chosen.category] =
    (state.categoryCounts[chosen.category] || 0) + 1;
...
function showSplash(name, sell, profit, persist) {
  ...
  const profitLabel = document.querySelector(
    "#splash-profit-wrap .splash-profit-lbl",
  );
...
async function _legacySearchSell() {
  try {
    const q = (document.getElementById("sell-search").value || "")
      .trim()
      .toLowerCase();
    ...
    if (!matched.length) {
      results.innerHTML =
        '<div class="empty" style="padding:24px 0;"><div class="e-icon" style="font-size:36px;"><i class="fa-solid fa-magnifying-glass"></i></div><p>No items found</p></div>';
      return;
    }
...
async function searchSell() {
  try {
    ...
    for (const item of items) {
      ...
      const meta =
        item.code +
        (item.variant || item.size ? " · " + (item.variant || item.size) : "");
      rows.push({
        item,
        t,
        score,
    ...
    rows.sort((a, b) =>
      q ? b.score - a.score : (a.label || "").localeCompare(b.label || ""),
    );

    const offStockBtn =
      '<button onclick="openOffStockSale()" class="stock-add-wish-btn" ' +
        ...
        .map((row) => {
          ...
          const stockCls = isRec
            ...
            : qty <= LOW_STOCK_LEVEL
              ...
              : "var(--green)";
          const bgCls = isRec
            ? "var(--amber-light,#fffbeb)"
            : qty <= LOW_STOCK_LEVEL
              ? "var(--amber-light)"
              : "var(--green-light)";
          const sellMin = row.item.sellPriceMin || 0;
          const priceStr =
            sellMin > 0 && sellMin < row.price
...
async function confirmOffStockSale() {
  ...
  if (buyPrice !== null && buy >= sellPrice)
    return Validate.fail(
      "Sale price must be higher than buy price",
      "off-sell",
    );
...
async function openSellModal(itemId) {
  try {
    ...
    document.getElementById("sm-icon").style.background =
      t.color || "var(--surface2)";
    ...
    if (_smProfit) {
      _smProfit.textContent =
        _itemSellMin > 0 && _itemSellMin < _itemSell
    ...
    const _tpel = document.getElementById("sm-total-profit");
    if (_tpel)
      _tpel.textContent = (_maxProfit >= 0 ? "+" : "") + fmt(_maxProfit);
    ...
    document.getElementById("sm-qty").max = item.isRecord
      ? 999999
      : availableQty;
...
async function updateSellModal() {
  try {
    ...
    const priceUsed =
      !isNaN(actualRaw) && actualRaw > 0 ? actualRaw : basePrice;
    const totalRev = qty * priceUsed;
    const totalProfit = qty * (priceUsed - baseBuy);
    const overridden =
      !isNaN(actualRaw) && actualRaw > 0 && actualRaw !== basePrice;
    document.getElementById("sm-price-used").textContent =
      fmt(priceUsed) + (overridden ? " (custom)" : " (default)");
    document.getElementById("sm-total-rev").textContent = fmt(totalRev);
    document.getElementById("sm-total-profit").textContent =
      (totalProfit >= 0 ? "+" : "") + fmt(totalProfit);
    document.getElementById("sm-total-profit").style.color =
      totalProfit >= 0 ? "var(--green)" : "var(--red)";
    ...
    if (smProfit) {
      smProfit.textContent =
        (profitPerItem >= 0 ? "+" : "") + fmt(profitPerItem);
      smProfit.style.color = profitPerItem >= 0 ? "var(--green)" : "var(--red)";
    }
    // ── Min-price floor warning ────────────────────────────────────
    const sellMin =
      _isShoeSale && _sellShoeSize
        ? _sellShoeSize.sellPriceMin || 0
        : item.sellPriceMin || 0;
    ...
    if (confirmBtn) {
      confirmBtn.textContent = belowMin
        ? "⚠ CONFIRM BELOW MIN"
        : "CONFIRM SALE";
      confirmBtn.style.background = belowMin ? "#d97706" : "#1e7a3e";
      confirmBtn.title =
        priceUsed < baseBuy && priceUsed > 0
          ? "Warning: selling below cost price"
          : "";
    }
  } catch (e) {
    console.error("[updateSellModal]", e);
    toast("Error: " + e.message, "err");
  }
}
...
function adjSellQty(d) {
  ...
  if (v > max) {
    toast("Warning: Only " + max + " in stock", "err");
    v = max;
  }
...
async function confirmSaleUnlocked() {
  ...
  try {
    ...
    const buyPriceEntered =
      buyOnRecord <= 0 && !isNaN(buyEntered) && buyEntered >= 0;
    ...
    if (!item.isRecord) {
      if (qty > maxQty) {
        Validate.fail(
          "Only " + maxQty + " in stock - cannot sell " + qty,
          "sm-qty",
        );
        _overlay.hide();
        return;
      }
      if (!Validate.stock(qty, maxQty, itemLabel)) {
        _overlay.hide();
        return;
      }
    }
    ...
    try {
      const financeVisible =
        document.getElementById("page-finance")?.classList.contains("active") ||
        (document
          .getElementById("page-operations")
          ?.classList.contains("active") &&
          _activeOperationsTab === "finance");
      if (financeVisible) {
        renderFinancePage();
      }
    } catch (_) {
...
async function renderSellPage() {
  try {
    ...
    console.error("[renderSellPage]", e);
    toast("Error: " + e.message, "err");
  }
}
...
if (_sellModal)
  _sellModal.addEventListener("click", function (e) {
...
function clearNotifs() {
  ...
  if (l)
    l.innerHTML =
      '<div style="color:var(--muted);font-size:13px;padding:8px;">No events yet</div>';
}
...
function closePastSessionSheet() {
  const s = document.getElementById("past-session-sheet");
...
async function resetAndRebuildDB() {
  const msg =
    ...
    "• Recreate the local database schema clean (v" +
    DB_VER +
    ")\n\n" +
...
const DEV_SEED_SHOE_ITEMS = [
  {
    ...
    sell: 1400,
    sizes: [
      { size: "32", group: "M", qty: 6 },
      { size: "34", group: "M", qty: 4 },
      { size: "36", group: "M", qty: 0 },
    ],
  },
  {
    code: "SH-002",
    ...
    sell: 4200,
    sizes: [
      { size: "40", group: "L", qty: 3 },
      { size: "42", group: "L", qty: 5 },
      { size: "44", group: "L", qty: 2 },
    ],
  },
  {
    code: "SH-003",
    name: "Ladies Sandals - Tan",
    buy: 600,
    sell: 1100,
    sizes: [
      { size: "37", group: "L", qty: 7 },
      { size: "38", group: "L", qty: 1 },
      { size: "39", group: "L", qty: 0 },
    ],
  },
  {
    code: "SH-004",
    name: "Men's Official Shoes",
    buy: 1800,
    sell: 3200,
    sizes: [
      { size: "41", group: "L", qty: 4 },
      { size: "42", group: "L", qty: 6 },
      { size: "43", group: "L", qty: 3 },
    ],
  },
];
...
async function seedDevDatabase() {
  ...
  for (let dayOffset = 20; dayOffset >= 0; dayOffset--) {
    ...
    for (let i = 0; i < salesToday; i++) {
      ...
      const actualPrice = overridden
        ? Math.max(
            pick.buyPrice,
            pick.sellPrice - Math.round(pick.sellPrice * 0.1),
          )
        : pick.sellPrice;
  ...
  const wishNotes = [
    "Add to next stock order",
    "Compare prices before buying",
    "Customer requests this item",
    "Check quality before purchase",
    "Keep a few pieces in reserve",
    "Preferred colour is black",
    "Ask supplier for bulk price",
    "Restock when current stock is low",
    "Confirm warranty before ordering",
    "Check seasonal demand before buying",
  ];
  const wishUnits = ["Pcs", "Boxes", "Bags", "Sets", "Pairs"];
  const wishPriorities = ["urgent", "high", "medium", "low"];
  const wishStatuses = ["planned", "in_progress", "stocked", "cancelled"];
  const wishSuppliers = [
    "Nairobi Wholesale",
    "City Traders",
    "Eastside Supplies",
  ...
  for (let i = 0; i < 60; i++) {
    ...
    const supplierId = wishSuppliers[i % wishSuppliers.length];
    const unit = wishUnits[i % wishUnits.length];
    const qty = 2 + ((i * 3) % 24);
    const requestedQty = qty + (i % 4) * 2;
    const purchasedQty =
      status === "stocked"
        ? requestedQty
        : status === "in_progress"
          ? Math.max(0, Math.floor(requestedQty / 2))
          : 0;
    const estimatedCost = 150 + ((i * 211) % 2600);
    const note = wishNotes[i % wishNotes.length];
    const source = wishSources[i % wishSources.length];
      ...
      status === "stocked"
        ...
        : status === "cancelled"
          ...
          : purchasedQty > 0
            ? "partial"
            : "unstocked";
    const dateOffset = ((i % 9) + 1) * 2;
    const purchaseDate =
      status === "stocked" || status === "in_progress"
        ? new Date(now)
        : new Date(now);
    purchaseDate.setDate(purchaseDate.getDate() - dateOffset);
    const dayPurchaseDate =
      status === "stocked" || status === "in_progress"
        ? _seedLocalDateStr(purchaseDate)
        : null;
    const stockedAt = status === "stocked" ? new Date(now).toISOString() : null;
    const vendorQuotes = [
      {
        supplier: supplierId,
        price: estimatedCost,
      ...
      {
        supplier: wishSuppliers[(i + 1) % wishSuppliers.length],
        price: Math.max(120, estimatedCost - 180),
    ...
    wishSeed.push({
      ...
      source,
      purchaseStatus,
      purchaseBudgetId: i % 3 === 0 ? "budget-" + (i + 1) : "",
      type: category,
      dayPurchaseDate,
      stockedAt,
      stockedItemId: status === "stocked" ? i + 100 : null,
      vendorQuotes,
      createdAt: now.toISOString(),
      createdBy: "system",
      updatedAt: now.toISOString(),
  ...
  const existingWishlist = (await dbAll("wishlist")) || [];
  const existingWishlistByName = new Map(
    existingWishlist
      .filter((item) => item && item.name)
      .map((item) => [String(item.name).trim().toLowerCase(), item]),
  );

  for (let i = 0; i < wishSeed.length; i++) {
    const w = wishSeed[i];
    const normalizedName = String(w.name || "")
      .trim()
      .toLowerCase();
    const match = normalizedName
      ? existingWishlistByName.get(normalizedName)
      : null;
    const seedEntry = {
      ...w,
      name: w.name,
      unit: w.unit || "Pcs",
      qty: w.qty,
      requestedQty: w.requestedQty || w.qty,
      purchasedQty: w.purchasedQty || 0,
      ...
      status: normalizeWishlistStatus(w.status),
      priority: w.priority || wishPriorities[i % wishPriorities.length],
      source: w.source || "app",
      purchaseStatus: w.purchaseStatus || "unstocked",
      purchaseBudgetId: w.purchaseBudgetId || "",
      type: w.type || w.category || "",
      dayPurchaseDate: w.dayPurchaseDate || null,
      stockedAt: w.stockedAt || null,
      stockedItemId: w.stockedItemId || null,
      vendorQuotes: Array.isArray(w.vendorQuotes) ? w.vendorQuotes : [],
      updatedAt: now.toISOString(),
      updatedBy: "system",
    };

    if (match) {
      Object.assign(match, seedEntry, {
        updatedAt: now.toISOString(),
        updatedBy: "system",
      });
      await dbPut("wishlist", match);
  ...
  const seededWishlistItems = await dbAll("wishlist");
  const seededWishListMaps = {
    Electronics: seededWishlistItems.filter(
      (item) => String(item.category || "").toLowerCase() === "electronics",
    ),
    Clothes: seededWishlistItems.filter(
      (item) => String(item.category || "").toLowerCase() === "clothes",
    ),
    General: seededWishlistItems.filter(
      (item) => String(item.category || "").toLowerCase() === "general",
    ),
    Plastics: seededWishlistItems.filter(
      (item) => String(item.category || "").toLowerCase() === "plastics",
    ),
    Food: seededWishlistItems.filter(
      (item) => String(item.category || "").toLowerCase() === "food",
    ),
    Household: seededWishlistItems.filter(
      (item) => String(item.category || "").toLowerCase() === "household",
    ),
  };

  const savedLists = [
    {
      name: "Electronics Run",
      category: "Electronics",
      supply: "Nairobi Wholesale",
      budget: 60000,
      status: "planned",
      dueDate: _seedLocalDateStr(new Date(now.getTime() + 5 * 86400000)),
      itemIds: (seededWishListMaps.Electronics || [])
        .slice(0, 10)
        .map((item) => item.id),
    },
    {
      ...
      budget: 42500,
      status: "ordering",
      dueDate: _seedLocalDateStr(new Date(now.getTime() + 7 * 86400000)),
      itemIds: (seededWishListMaps.Clothes || [])
        .slice(0, 10)
        .map((item) => item.id),
    },
    {
      ...
      category: "Household",
      supply: "Market Centre",
      budget: 35000,
      status: "partial",
      dueDate: _seedLocalDateStr(new Date(now.getTime() + 9 * 86400000)),
      itemIds: (seededWishListMaps.Household || [])
        .slice(0, 10)
        .map((item) => item.id),
    },
    {
      ...
      category: "General",
      supply: "Eastside Supplies",
      budget: 28000,
      status: "planned",
      dueDate: _seedLocalDateStr(new Date(now.getTime() + 10 * 86400000)),
      itemIds: (seededWishListMaps.General || [])
        .slice(0, 10)
        .map((item) => item.id),
    },
    {
      ...
      budget: 32000,
      status: "planned",
      dueDate: _seedLocalDateStr(new Date(now.getTime() + 12 * 86400000)),
      itemIds: (seededWishListMaps.Plastics || [])
        .slice(0, 10)
        .map((item) => item.id),
    },
    {
      ...
      budget: 26000,
      status: "planned",
      dueDate: _seedLocalDateStr(new Date(now.getTime() + 14 * 86400000)),
      itemIds: (seededWishListMaps.Food || [])
        .slice(0, 10)
        .map((item) => item.id),
    },
  ].map((list) => ({
    ...
    id:
      ...
      "-" +
      list.category.toLowerCase().replace(/[^a-z]+/g, "-"),
    itemIds: [
      ...new Set(
        (list.itemIds || []).filter((id) => id !== undefined && id !== null),
      ),
    ],
  }));

  localStorage.setItem(KEY_WISHLIST_SAVED_LISTS, JSON.stringify(savedLists));
  ...
  const financeSeed = [
    ...
    },
    {
      type: "expense",
      amount: 3500,
      description: "Shop rent contribution",
      daysAgo: 15,
    },
    {
      type: "expense",
      amount: 1200,
      description: "Transport for restock",
      daysAgo: 9,
    },
    {
      type: "withdrawal",
...
async function rebuildDevDatabaseWithSeed() {
  if (getFirebaseEnv() !== "development" || !DB_NAME.endsWith("_dev")) {
    toast(
      "Error: Rebuild with sample data is only available in Development mode",
      "err",
    );
    return;
  }
...
async function resetAllData() {
  ...
  try {
    ...
    if (fbReady && fbDb) {
      try {
        const { collection, getDocs, deleteDoc, doc, writeBatch } =
        ...
        toast(
          "Warning: Firebase may not be fully cleared: " + e.message,
          "err",
        );
      }
    }
...
async function clearLocalData(skipConfirm = false) {
  if (
    !skipConfirm &&
    !confirm(
      "Clear ALL data stored on this device?\n\nCloud data is not affected. Login and settings are kept.",
    )
  )
    return;
  try {
...
async function clearCloudData(skipConfirm = false) {
  if (
    !skipConfirm &&
    !confirm(
      "Delete ALL data in Firebase cloud?\n\nLocal data on this device is not affected. Other devices will lose cloud copies.",
    )
  )
    return;
  if (!fbReady || !fbDb) {
...
async function clearBothLocalAndCloud() {
  if (
    !confirm(
      "Permanently delete ALL local AND cloud data?\n\nItems, sales, finances, and day records. Cannot be undone.",
    )
  )
    return;
  await resetAllData();
...
async function clearAppCacheAndReload() {
  if (
    !confirm(
      "Clear cached app files and reload?\n\nFixes outdated screens; does not delete your business data.",
    )
  )
    return;
  try {
  ...
  try {
    ...
    if (_syncChannel)
      _syncChannel.postMessage({
        type: "sync-local-change",
        store,
        at: Date.now(),
      });
    localStorage.setItem(
      "mgs_sync_signal",
      JSON.stringify({ at: Date.now(), store }),
    );
  } catch (_) {}
}
...
function _setSyncProgress(mode, done, total) {
  ...
  label.textContent =
    mode === "pull" ? "Downloading" : mode === "push" ? "Uploading" : "Syncing";
  count.textContent =
    total > 0 ? done + " of " + total + " changes" : "Preparing...";
...
async function updateSyncDot() {
  ...
  function _applySyncState(state, label, barMsg) {
    ...
    if (btnOffline)
      btnOffline.style.display = state === "offline" ? "flex" : "none";
    // Highlight Sync in red when there's an error.
    if (btnSync)
      btnSync.style.borderColor = state === "error" ? "rgba(220,38,38,.6)" : "";
  }
  ...
  if (!fbReady || !fbDb) {
    _applySyncState(
      "offline",
      "Not connected",
      "Firebase not connected — tap Reconnect",
    );
    return;
  }
  ...
  try {
    ...
    if (unsynced.length) {
      _applySyncState(
        "ahead",
        "Not uploaded",
        unsynced.length + " local item(s) waiting to upload — tap Upload",
      );
      return;
    }

    // 3. Pending local operations
    const pending = db.objectStoreNames.contains("sync_queue")
      ? (await dbAll("sync_queue")).filter((q) => q.status !== "done").length
      : 0;
    if (pending) {
      ...
      if (_lastSyncError) {
        _applySyncState(
          "error",
          "Sync error",
          pending + " " + noun + " waiting · " + _lastSyncError,
        );
      } else {
        _applySyncState(
          "ahead",
          "Waiting",
          pending + " " + noun + " waiting to upload",
        );
      }
      return;
    }
    ...
    _applySyncState(
      "synced",
      "Up to date",
      "No changes waiting to upload · " + now,
    );
    _clearSyncProgress();
  } catch (e) {
...
async function _doRefresh() {
  ...
  try {
    ...
    console.warn("[SYNC] refresh error:", e.message);
  }
}
...
function setFbStatus(status) {
  ...
  if (dot) {
    ...
    dot.style.boxShadow = status === "on" ? "0 0 6px var(--green)" : "none";
  }
  ...
  if (toggleBtn) {
    const connected =
      status === "on" || status === "connecting" || status === "syncing";
  ...
  if (barTime && (status === "on" || status === "syncing"))
    barTime.textContent = now;
}
...
function updateFirebaseEnvUI() {
  ...
  if (projectEl)
    projectEl.textContent = isDev
      ? "None - local database only"
      : cfg.projectId;
  const prefixEl = document.getElementById("fb-env-prefix");
  if (prefixEl)
    prefixEl.textContent = isDev
      ? "Collections: none (cloud sync disabled)"
      : "Collections: items, sales,...";
  const autosyncEl = document.getElementById("fb-env-autosync");
  if (autosyncEl)
    autosyncEl.textContent = isDev
      ? "Disabled (local database only)"
      : "On every change (when online)";
  document
    .getElementById("fb-env-prod")
    ?.classList.toggle("active", env === "production");
...
async function initFirebase() {
  try {
    ...
    const { initializeApp, getApps, getFirestore, onSnapshot } =
      await waitForFbImports();

    const envCfg = getFirebaseEnvConfig();
    const apps = getApps();
    fbApp =
      apps.find((a) => a.name === envCfg.appName) ||
      initializeApp(FIREBASE_CONFIG, envCfg.appName);
    ...
    if (typeof window._fbUnsubSales === "function") {
      window._fbUnsubSales();
    }
    if (typeof window._fbUnsubFin === "function") {
      window._fbUnsubFin();
    }
    if (typeof window._fbUnsubWish === "function") {
      window._fbUnsubWish();
    }
    if (typeof window._fbUnsubSz === "function") {
      window._fbUnsubSz();
    }
    if (typeof window._fbUnsubBd === "function") {
      window._fbUnsubBd();
    }
    if (typeof window._fbUnsubCust === "function") {
      window._fbUnsubCust();
    }
    if (typeof window._fbUnsubCustTxn === "function") {
      window._fbUnsubCustTxn();
    }
    if (typeof window._fbUnsubBudget === "function") {
      window._fbUnsubBudget();
    }
    fbUnsub = null;
    ...
    const realtimeStores = [
      ...
      ["purchase_budgets", "purchase_budgets", "purchase_budgets"],
    ...
    const listenerRefs = {
      items: "fbUnsub",
      sales: "_fbUnsubSales",
      shoe_sizes: "_fbUnsubSz",
      finances: "_fbUnsubFin",
      business_days: "_fbUnsubBd",
      wishlist: "_fbUnsubWish",
      customers: "_fbUnsubCust",
      customer_txns: "_fbUnsubCustTxn",
      purchase_budgets: "_fbUnsubBudget",
    };
          ...
          try {
            ...
            for (const change of snap.docChanges()) {
              ...
              if (change.type === "removed") {
                const local = await _findLocalForRemote(
                  store,
                  data,
                  change.doc.id,
                );
            ...
            if (changed) {
              if (store === "customer_txns") {
                const ids = new Set(
                  snap
                    .docChanges()
                    .map((c) => c.doc.data()?.customerId)
                    .filter(Boolean),
                );
                for (const id of ids) {
                  try {
                    await _recalcBalance(id);
                  } catch (_) {}
                }
              }
              try {
              ...
              try {
                if (store === "customers") renderCustomerList("");
              } catch (_) {}
              setFbStatus("on");
              updateSyncDot();
            }
...
function waitForFbImports() {
  return new Promise((res, rej) => {
    ...
    const check = () => {
      ...
      if (++attempts > 150) {
        rej(new Error("Firebase SDK timeout after 15s"));
        return;
      }
...
async function saveFirebaseConfig() {
  try {
    ...
    console.error("[saveFirebaseConfig]", e);
    toast("Error: " + e.message, "err");
  }
}

function _fbSlug(s, fallback) {
  const out = String(s || fallback || "x")
    .toLowerCase()
    .replace(/[^a-z0-9]/g, "");
...
function stableItemFbId(item) {
  ...
  const variant =
    item && item.isShoe
      ? "shoe"
      : _fbSlug(item && (item.variant || item.size), "std");
...
function stableWishFbId(wish) {
  ...
  const when =
    ((wish && wish.createdAt) || "").replace(/[^0-9]/g, "").slice(0, 14) || "0";
...
function stableBusinessDayFbId(bd) {
  const d = (
    (bd && (bd.businessDate || bd.business_date)) ||
    "unknown"
  ).replace(/[^0-9-]/g, "");
  return "bd_" + d;
}
...
function stableShoeSizeFbId(sz) {
  ...
  return (
    ...
    _fbSlug(sz && sz.code) +
    "_" +
    String(sz && sz.size != null ? sz.size : 0)
  );
}
...
function stableSaleFbId(sale) {
  ...
  const lid =
    sale && sale.id ? String(sale.id) : Math.random().toString(36).slice(2, 7);
...
function _syncRecordKey(collection, record) {
  ...
  return String(
    record.fbId ||
      _syncIdentity(collection, record) ||
      record.id ||
      record.codeSize ||
      record.code ||
      record.customerId ||
...
async function _queueSyncUnlocked(collection, record, operation = "upsert") {
  ...
  if (
    operation === "delete" &&
    same.some((q) => q.operation === "delete" && q.status !== "done")
  )
    return;
  if (
    operation === "upsert" &&
    record._syncMutationId &&
    same.some(
      (q) => q.payload && q.payload._syncMutationId === record._syncMutationId,
    )
  )
    return;
  ...
  metaReq.onsuccess = () => {
    ...
    tx.objectStore("sync_queue").add(
      _queueRecord(collection, record, operation, clock, now),
    );
  };
  await new Promise((resolve, reject) => {
    tx.oncomplete = resolve;
    tx.onerror = () => reject(tx.error);
    tx.onabort = () =>
      reject(tx.error || new Error("Unable to queue sync operation"));
  });
}
...
async function fbSyncShoeSize(sizeRec) {
  try {
    ...
    if (sizeRec.fbId !== stable) {
      sizeRec.fbId = stable;
      if (sizeRec.id) await dbPut("shoe_sizes", sizeRec);
    }
    if (!sizeRec._syncVersion || !sizeRec._syncMutationId)
      await dbPut("shoe_sizes", sizeRec);
    await _queueSync("shoe_sizes", sizeRec);
  } catch (e) {
    _lastSyncError = e.message;
    console.error("[SYNC] queue shoe size:", e);
    updateSyncDot();
  }
}
...
async function fbDeleteItem(fbId) {
  if (!fbId) return;
  try {
    await _queueSync("items", { fbId }, "delete");
  } catch (e) {
    _lastSyncError = e.message;
    console.error("[SYNC] queue item delete failed:", e);
    updateSyncDot();
  }
}
...
async function fbSyncSale(sale) {
  try {
    ...
    _lastSyncError = e.message;
    console.error("[SYNC] queue sale failed:", e);
    updateSyncDot();
  }
}
...
async function fbDeleteSale(sale) {
  ...
  try {
    await _queueSync("sales", sale, "delete");
    return 1;
  } catch (e) {
    _lastSyncError = e.message;
    console.error("[SYNC] queue sale delete:", e);
    updateSyncDot();
    return 0;
  }
}
...
async function fbDeleteFinanceEntry(entry) {
  ...
  try {
    await _queueSync("finances", entry, "delete");
    return 1;
  } catch (e) {
    _lastSyncError = e.message;
    console.error("[SYNC] queue finance delete:", e);
    updateSyncDot();
    return 0;
  }
}
...
async function fbSyncCustomer(customer) {
  try {
    ...
    if (!customer._syncVersion || !customer._syncMutationId)
      await dbPut("customers", customer);
    await _queueSync("customers", customer);
  } catch (e) {
    _lastSyncError = e.message;
    console.error("[SYNC] queue customer:", e);
    updateSyncDot();
  }
}
...
async function fbSyncCustTxn(txn) {
  try {
    if (!txn.fbId) {
      const ts = (txn.createdAt || new Date().toISOString())
        .replace(/[^0-9]/g, "")
        .slice(0, 14);
      txn.fbId =
        "ctxn_" +
        ...
        ts +
        "_" +
        (txn.id || Math.floor(Math.random() * 9999));
      if (txn.id) await dbPut("customer_txns", txn);
    }
    if (!txn._syncVersion || !txn._syncMutationId)
      await dbPut("customer_txns", txn);
    await _queueSync("customer_txns", txn);
  } catch (e) {
    _lastSyncError = e.message;
    console.error("[SYNC] queue customer txn:", e);
    updateSyncDot();
  }
}
...
async function fbDeleteCustTxn(txn) {
  ...
  try {
    await _queueSync("customer_txns", txn, "delete");
    return 1;
  } catch (e) {
    _lastSyncError = e.message;
    console.error("[SYNC] queue customer txn delete:", e);
    updateSyncDot();
    return 0;
  }
}
...
function _financeDeleteMarkers() {
  try {
    return JSON.parse(localStorage.getItem(KEY_DELETED_FIN) || "[]");
  } catch (_) {
    return [];
  }
}
...
function _rememberDeletedFinance(entry) {
  const markers = _financeDeleteMarkers().filter(
    (m) => Date.now() - (m.deletedAt || 0) < 30 * 24 * 60 * 60 * 1000,
  );
  const marker = {
  ...
  if (
    !markers.some(
      (m) =>
        (marker.fbId && m.fbId === marker.fbId) ||
        (marker.sig && m.sig === marker.sig),
    )
  ) {
    markers.push(marker);
  }
...
function _isDeletedFinanceRemote(fbId, entry) {
  ...
  return _financeDeleteMarkers().some(
    (m) =>
      (fbId && m.fbId && m.fbId === fbId) || (sig && m.sig && m.sig === sig),
  );
}
...
function _financeRecordsMatch(local, remote) {
  ...
  if (
    local.saleId &&
    remote.saleId &&
    String(local.saleId) === String(remote.saleId)
  )
    return true;
...
function _saleDeleteMarkers() {
  try {
    return JSON.parse(localStorage.getItem(KEY_DELETED_SALE) || "[]");
  } catch (_) {
    return [];
  }
}
...
function _rememberDeletedSale(sale) {
  const markers = _saleDeleteMarkers().filter(
    (m) => Date.now() - (m.deletedAt || 0) < 30 * 24 * 60 * 60 * 1000,
  );
  const marker = {
...
function _isDeletedSaleRemote(fbId, sale) {
  ...
  return _saleDeleteMarkers().some(
    (m) =>
      (fbId && m.fbId && m.fbId === fbId) || (sig && m.sig && m.sig === sig),
  );
}
...
async function deduplicateSales() {
  ...
  for (const loser of losers) {
    ...
    if (loser.fbId && fbReady && fbDb) {
      try {
        ...
        console.warn("[DEDUP] cloud delete failed for", loser.fbId, e.message);
      }
    }
  ...
  console.log(
    "[DEDUP] Removed " +
      losers.length +
      " duplicate sale(s) from local + cloud",
  );
...
function _compareSyncVersion(a, b) {
  const aHlc =
    a &&
    (a._hlc || {
      wall: Number(a._syncVersion) || 0,
      counter: 0,
      clientId: a._syncClient || "",
    });
  const bHlc =
    b &&
    (b._hlc || {
      wall: Number(b._syncVersion) || 0,
      counter: 0,
      clientId: b._syncClient || "",
    });
  ...
  const ac = String((a && a._syncClient) || ""),
    bc = String((b && b._syncClient) || "");
...
async function _findLocalForRemote(store, data, fbId) {
  ...
  if (data.fbId || fbId) {
    const byFb = rows.find(
      (r) => String(r.fbId || "") === String(fbId || data.fbId || ""),
    );
    if (byFb) return byFb;
  }
  if (store === "items" && data.code)
    return rows.find((r) => r.code === data.code);
  if (store === "shoe_sizes" && data.codeSize)
    return rows.find((r) => r.codeSize === data.codeSize);
...
async function _acknowledgeMatchingQueue(store, fbId, remote) {
  ...
  for (const entry of queue) {
    if (
      entry.collection !== store ||
      entry.recordKey !== String(fbId) ||
...
async function _applyRemoteRecord(store, data, fbId) {
  ...
  if (
    existing &&
    _compareSyncVersion(remote, existing) === 0 &&
    (await _hasPendingSync(store, String(fbId)))
  )
    return false;
  if (remote._syncDeleted) {
    if (existing) {
      _syncWriteContext++;
      try {
        await dbDelete(store, existing.id);
      } finally {
        _syncWriteContext--;
      }
      return true;
...
async function _releaseSyncLease() {
  ...
  await new Promise((resolve) => {
    ...
    const req = os.get("lease");
    req.onsuccess = () => {
      if (req.result && req.result.owner === _syncLeaseOwner)
        os.delete("lease");
    };
    tx.oncomplete = resolve;
    tx.onerror = tx.onabort = resolve;
    ...
    try {
      ...
      for (const entry of allQueue) {
        if (
          ...
          (entry.processingAt || entry.createdAt || 0) < staleBefore
        ) {
      ...
      for (const entry of queue) {
        ...
        const prevVersion = previous?.payload?._hlc || {
          wall: Number(previous?.payload?._syncVersion || 0),
          counter: 0,
          clientId: previous?.payload?._syncClient || "",
        };
        const nextVersion = entry.payload?._hlc || {
          wall: Number(entry.payload?._syncVersion || 0),
          counter: 0,
          clientId: entry.payload?._syncClient || "",
        };
      ...
      const latestQueueIds = new Set(
        Array.from(newestByRecord.values()).map((entry) => entry.id),
      );
      for (const entry of queue) {
      ...
      for (const q of queue) {
        try {
          ...
          const payload = sanitiseForFirestore(
            q.payload || {
              fbId: q.recordKey,
              _syncDeleted: q.operation === "delete",
            },
          );
          ...
          const retryDelay = Math.min(
            30000,
            2000 * Math.pow(2, Math.min((q.attempts || 1) - 1, 4)),
          );
...
async function forcePushToFirebase(silent = false) {
  if (!fbReady || !fbDb) {
    if (!silent) toast("Warning: Connect Firebase first", "err");
    return;
  }
    ...
    try {
        ...
        try {
          ...
          if (incremental && query && where) {
            try {
              ...
              console.warn(
                "[SYNC] incremental pull unavailable for " + store + ":",
          ...
          if (
            ...
            console.warn(
              "[SYNC] optional collection unavailable:",
              store,
              e.message,
            );
          } else {
      ...
      for (const [store, snap, cursorKey] of snapshots) {
        ...
        for (const d of snap.docs) {
          ...
          if (data._syncUpdatedAt && data._syncUpdatedAt > maxCursor)
            maxCursor = data._syncUpdatedAt;
          if (store === "sales" && _isDeletedSaleRemote(d.id, data)) continue;
          if (store === "finances" && _isDeletedFinanceRemote(d.id, data))
            continue;
...
async function syncRestoreFromCloud(silent = false) {
  ...
  await _withSyncLock(async () => {
    const stores = _SYNCABLE_STORES.filter((s) =>
      db.objectStoreNames.contains(s),
    );
    const all = stores.concat(
      ["sync_queue", "sync_meta"].filter((s) =>
        db.objectStoreNames.contains(s),
      ),
    );
    await new Promise((resolve, reject) => {
...
async function restoreLocalFromCloud() {
  if (
    !confirm(
      "Replace local inventory with the latest cloud snapshot?\n\nUnsynced local changes will be discarded.",
    )
  )
    return;
  try {
    await syncRestoreFromCloud(false);
  } catch (e) {
...
async function runSyncDebug() {
  ...
  addLog(
    "Environment: " +
      getFirebaseEnvConfig().label +
      " (" +
      fbColName("items") +
      ",...)",
  );
  addLog(
    "fbReady=" + fbReady + " fbDb=" + !!fbDb + " online=" + navigator.onLine,
  );
...
async function getBusinessDay(dateStr) {
  ...
  return (
    all.find((d) => (d.businessDate || d.business_date) === dateStr) || null
  );
}
...
async function openDay() {
  ...
  if (bday.status === "OPEN") {
    toast("Day is already open!", "err");
    return;
  }
  if (bday.status === "LOCKED") {
    toast("This day is archived.", "err");
    return;
  }
  ...
  if (!bday.opened_at) {
    ...
    const items = await dbAll("items");
    bday.openingStockCost = items.reduce(
      (s, i) => s + (i.buyPrice || i.buy || 0) * (i.qty || 0),
      0,
    );
    bday.openingStockRetail = items.reduce(
      (s, i) => s + (i.sellPrice || i.sell || 0) * (i.qty || 0),
      0,
    );
  }
  ...
  renderDaySessionsList();
  toast(
    isReopen ? "Day reopened. Continue recording." : "Business day opened.",
    "ok",
  );
}
...
async function closeDay() {
  if (!activeDay || activeDay.status !== "OPEN") {
    toast("No open day to close.", "err");
    return;
  }
  ...
  const daySales = sales.filter(
    (s) => (s.businessDate || s.business_date) === _dayDate,
  );
  ...
  const todayStart =
    (activeDay.businessDate || activeDay.business_date) + "T00:00:00";
  const items = await dbAll("items");
  const purchases = items.filter(
    (i) => i.createdAt && i.createdAt >= todayStart,
  );
  const closingStockCost = items.reduce(
    (s, i) => s + (i.buyPrice || i.buy || 0) * (i.qty || 0),
    0,
  );
  ...
  set(
    "ds-date",
    fmtFullDate(activeDay.businessDate || activeDay.business_date),
  );
  ...
  set(
    "ds-purchases-val",
    fmt(purchases.reduce((s, i) => s + i.buy * i.qty, 0)),
  );
  ...
  const pct =
    opening > 0
      ? Math.min(
          100,
          Math.round(((opening - closingStockCost) / opening) * 100),
        )
      : 0;
  ...
  if (verdictEl) {
    ...
    let vBg = "var(--surface2)",
      vColor = "var(--muted)";
    if (daySales.length > 0) {
      const m = parseFloat(margin);
      if (m >= 30) {
        verdict = "Excellent! " + margin + "% margin.";
        vBg = "var(--green-light)";
        vColor = "var(--green)";
      } else if (m >= 15) {
        verdict = "Good day! " + margin + "% margin.";
        vBg = "var(--green-light)";
        vColor = "var(--green)";
      } else {
        verdict = "Decent. " + margin + "% margin.";
        vBg = "var(--amber-light)";
        vColor = "var(--amber)";
      }
    }
    verdictEl.style.cssText =
      ...
      ";border:1px solid " +
      vColor +
      ";border-radius:var(--r);padding:14px 16px;margin-bottom:14px;text-align:center;";
    verdictEl.innerHTML =
      '<div style="font-size:16px;font-weight:800;">' + verdict + "</div>";
  }
  ...
  if (confirmBtn) {
    ...
    confirmBtn.textContent = "Confirm Close Day";
  }
...
async function confirmCloseDay() {
  ...
  const todayStart2 =
    (activeDay.businessDate || activeDay.business_date) + "T00:00:00";
  const purchases = items.filter(
    (i) => i.createdAt && i.createdAt >= todayStart2,
  );
  ...
  activeDay.purchaseCost = purchases.reduce(
    (s, i) => s + (i.buyPrice || i.buy || 0) * (i.qty || 0),
    0,
  );
  activeDay.closingStockCost = items.reduce(
    (s, i) => s + (i.buyPrice || i.buy || 0) * (i.qty || 0),
    0,
  );
...
async function voidSale(saleId) {
  try {
    ...
    if (!(await fbDeleteSale(sale))) {
      toast(
        "Sale was not voided because sync deletion could not be queued",
        "err",
      );
      return;
    }
...
function startDayTimer() {
  /* replaced by automatic date tracking */
}
...
function updateDayBanner() {
  ...
  const BTN =
    "width:100%;padding:16px;border:none;border-radius:var(--r);font-size:16px;font-weight:800;cursor:pointer;font-family:var(--sans);";

  if (status === "OPEN") {
    const mins = opened_at
      ? Math.floor((Date.now() - new Date(opened_at)) / 60000)
      : 0;
    const dur =
      mins < 60 ? mins + "m" : Math.floor(mins / 60) + "h " + (mins % 60) + "m";
    banner.style.cssText =
      "background:var(--green-light);border:2px solid #a8d8b5;border-radius:var(--r-lg);padding:20px 18px;margin-bottom:14px;text-align:center;";
    icon.textContent = "Open";
    badge.textContent = "OPEN";
    badge.style.cssText =
      "display:inline-block;font-size:11px;font-weight:800;font-family:var(--mono);padding:4px 12px;border-radius:20px;margin-bottom:8px;letter-spacing:1px;background:#dcfce7;color:#16a34a;";
    ...
    banner.style.cssText =
      "background:#fef3c7;border:2px solid #f5d9a0;border-radius:var(--r-lg);padding:20px 18px;margin-bottom:14px;text-align:center;";
    icon.textContent = "Closed";
    badge.textContent = "CLOSED";
    badge.style.cssText =
      "display:inline-block;font-size:11px;font-weight:800;font-family:var(--mono);padding:4px 12px;border-radius:20px;margin-bottom:8px;letter-spacing:1px;background:#fef3c7;color:#92400e;";
    ...
    sub.textContent = closed_at
      ? "Closed at " +
        ...
        (auto_closed ? " - auto" : "") +
        (reopened_count > 0
          ? " - Opened " + (reopened_count + 1) + "x today"
          : "") +
        " - Tap to reopen"
    ...
    banner.style.cssText =
      "background:var(--surface2);border:2px solid var(--border);border-radius:var(--r-lg);padding:20px 18px;margin-bottom:14px;text-align:center;";
    icon.textContent = "Locked";
    badge.textContent = "LOCKED";
    badge.style.cssText =
      "display:inline-block;font-size:11px;font-weight:800;font-family:var(--mono);padding:4px 12px;border-radius:20px;margin-bottom:8px;letter-spacing:1px;background:var(--surface2);color:var(--muted);";
...
async function updateDayLiveStats() {
  ...
  const daySales = sales.filter(
    (s) =>
      (s.businessDate || s.business_date || (s.date || "").split("T")[0]) ===
      today,
  );
  const dayFins = fins.filter(
    (e) => (e.date || (e.createdAt || "").split("T")[0]) === today,
  );

  // ── Sales split by payment method ──────────────────────────
  const cashSales = daySales.filter(
  ...
  const injected = dayFins
    .filter((e) => e.type === "injection" || e.type === "investment")
    .reduce((a, e) => a + (e.amount || 0), 0);
  const stockBought = dayFins
    .filter((e) => e.type === "stock_purchase")
    .reduce((a, e) => a + (e.amount || 0), 0);
  const expenses = dayFins
    .filter((e) => e.type === "expense")
    .reduce((a, e) => a + (e.amount || 0), 0);
  const withdrawn = dayFins
  ...
  set(
    "day-cash-sales-count",
    cashSales.length + " sale" + (cashSales.length !== 1 ? "s" : ""),
  );
  set("day-mpesa-sales", fmt(mpesaRev));
  set(
    "day-mpesa-sales-count",
    mpesaSales.length + " sale" + (mpesaSales.length !== 1 ? "s" : ""),
  );
  set("day-injected", fmt(injected));
  const injEntries = dayFins.filter(
    (e) => e.type === "injection" || e.type === "investment",
  );
  set(
    "day-injected-desc",
    injEntries.length
      ? injEntries
          .map((e) => e.description || "Injection")
          .slice(0, 2)
          .join(", ")
      : "no injections today",
  );
  ...
  set(
    "day-stock-purchased-desc",
    stockEntries.length
      ? stockEntries.length +
          " purchase" +
          (stockEntries.length !== 1 ? "s" : "")
      : "no purchases today",
  );
  ...
  set(
    "day-expenses-desc",
    expEntries.length
      ? expEntries.length + " expense" + (expEntries.length !== 1 ? "s" : "")
      : "no expenses today",
  );
  ...
  set(
    "day-withdrawn-desc",
    wdEntries.length
      ? wdEntries.length + " withdrawal" + (wdEntries.length !== 1 ? "s" : "")
      : "none today",
  );
  ...
  if (netEl && netEl.closest) {
    ...
    if (wrap)
      wrap.style.background =
        netFlow >= 0 ? "var(--green-light)" : "var(--red-light)";
  }
  ...
  setColor(
    "day-margin-pct",
    margin >= 20 ? "var(--green)" : margin >= 10 ? "#d97706" : "var(--red)",
  );
  ...
  if (sl) {
    ...
    const txns = [
      ...daySales.map((s) => ({
        ...
        type: "sale",
        label:
          (s.itemName || s.itemCode || "Sale") +
          (s.itemSize ? "  - " + s.itemSize : ""),
...
async function renderDaySessionsList() {
  ...
  const past = all
    .filter(
      (d) =>
        (d.businessDate || d.business_date) !== today &&
        (d.status === "CLOSED" || d.status === "LOCKED"),
    )
    .sort((a, b) =>
      (b.businessDate || b.business_date || "").localeCompare(
        a.businessDate || a.business_date || "",
      ),
    );
  ...
  if (!past.length) {
    list.innerHTML =
      '<div style="color:var(--muted);font-size:13px;padding:8px 0;">No past sessions yet</div>';
    return;
  }
  ...
  list.innerHTML = past
    .map((s) => {
      ...
      return (
        '<div class="card" style="margin-bottom:8px;padding:14px;cursor:pointer;" onclick="viewPastSession(' +
        s.id +
        ')">' +
        '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">' +
        "<div>" +
        '<div style="font-size:14px;font-weight:800;color:var(--text);">' +
        fmtFullDate(s.businessDate || s.business_date) +
        "</div>" +
        '<div style="font-size:11px;color:var(--muted);font-family:var(--mono);margin-top:2px;">' +
        (s.openedAt || s.opened_at ? fmtTime(s.openedAt || s.opened_at) : "-") +
        " to " +
        (s.closedAt || s.closed_at
          ? fmtTime(s.closedAt || s.closed_at)
          : "auto") +
        ((s.reopenedCount || s.reopened_count || 0) > 0
          ? " - Reopened " + (s.reopenedCount || s.reopened_count) + "x"
          : "") +
        "</div>" +
        "</div>" +
        (locked
        ...
        '<div style="text-align:center;background:var(--surface2);border-radius:8px;padding:8px 4px;"><div style="font-size:14px;font-weight:800;font-family:var(--mono);color:var(--accent2);">' +
        fmt(s.revenue || 0) +
        '</div><div style="font-size:10px;color:var(--muted);">Revenue</div></div>' +
        '<div style="text-align:center;background:var(--surface2);border-radius:8px;padding:8px 4px;"><div style="font-size:14px;font-weight:800;font-family:var(--mono);color:' +
        profitColor +
        ';">' +
        fmt(s.profit || 0) +
        '</div><div style="font-size:10px;color:var(--muted);">Profit</div></div>' +
        '<div style="text-align:center;background:var(--surface2);border-radius:8px;padding:8px 4px;"><div style="font-size:14px;font-weight:800;font-family:var(--mono);color:var(--accent);">' +
        (s.salesCount || 0) +
        '</div><div style="font-size:10px;color:var(--muted);">Sales</div></div>' +
        "</div>" +
        (s.notes
...
async function toggleRestock() {
  ...
  if (item && buyEl)
    buyEl.value = item.buyPrice ?? item.buy ?? item.defaultBuy ?? "";
  if (item && sellEl)
    sellEl.value = item.sellPrice ?? item.sell ?? item.defaultSell ?? "";
...
async function confirmRestock() {
  ...
  try {
    ...
    if (buyRaw !== null && buyRaw < 0)
      return Validate.fail("Invalid buy price", "restock-buy");
    if (sellRaw !== null && sellRaw < 0)
      return Validate.fail("Invalid sell price", "restock-sell");
    const item = await dbGet("items", currentDetailId);
    if (!item) {
      toast("Warning: Item not found", "err");
      return;
...
async function updateLowStockBadge() {
  try {
    ...
    console.error("[updateLowStockBadge]", e);
    toast("Error: " + e.message, "err");
  }
}
...
async function deleteSale(saleId) {
  try {
    if (!confirm("Delete this sale record? Stock will NOT be restored."))
      return;
    const sale = await dbGet("sales", saleId);
    if (sale) {
      _rememberDeletedSale(sale);
...
if ("serviceWorker" in navigator) {
    ...
    .then((reg) => {
      ...
      function onNewWorker(worker) {
        ...
        if (t && !document.getElementById("update-dot")) {
          ...
          d.id = "update-dot";
          d.style.cssText =
            "position:absolute;top:4px;right:4px;width:8px;height:8px;background:var(--red);border-radius:50%;";
  ...
  navigator.serviceWorker.addEventListener("controllerchange", () => {
    ...
    ["update-progress-bar", "upd-progress-bar"].forEach((id) => {
      const el = document.getElementById(id);
      if (el) el.style.width = "100%";
    });
    ["update-progress-pct", "upd-progress-pct"].forEach((id) => {
      const el = document.getElementById(id);
      if (el) el.textContent = "100%";
    });
    ["update-progress-label", "upd-progress-label"].forEach((id) => {
      const el = document.getElementById(id);
...
function showInstallBanner() {
  ...
  document.getElementById("install-steps-content").innerHTML = steps
    .map((s) => "<div>" + s + "</div>")
    .join("");
...
async function triggerInstall() {
  try {
    ...
    console.error("[triggerInstall]", e);
    toast("Error: " + e.message, "err");
  }
}
...
function showUserProfile() {
  ...
  document.getElementById("profile-access").textContent =
    roleLabels[currentUser.role] || currentUser.roleLabel;
  document.getElementById("profile-access").style.color =
    roleColors[currentUser.role];
  document.getElementById("profile-tabs").textContent = currentUser.tabs
    .map((t) => tabLabels[t] || t)
    .join(", ");
...
function tidySettingsPage() {
  ...
  updateUpgradeStepUI(
    document.getElementById("update-state-available")?.style.display !== "none"
      ? "available"
      : document.getElementById("update-state-installing")?.style.display !==
          "none"
        ? "installing"
        : "current",
  );
...
function ensurePhotoLightbox() {
  ...
  lb.innerHTML =
    ...
    "</div>";
  lb.querySelector(".photo-lightbox-close").addEventListener(
    "click",
    closePhotoLightbox,
  );
...
function attachPhotoViewer(viewport, img, options) {
  ...
  const key =
    (options && options.key) || viewport.id || "pv" + _photoViewerRegistry.size;
  ...
  function onPointerDown(e) {
    ...
    if (
      e.target.closest(
        ".add-photo-remove, .photo-lightbox-close, #sh-photo-btn",
      )
    )
      return;
...
function initPhotoViewers() {
    ...
    document.getElementById("sh-photo-img"),
    {
      ...
      isActive: () => {
        ...
        return (
          ...
          !!document.getElementById("sh-photo-img")?.src
        );
      },
    },
  );
  attachPhotoViewer(
    ...
    document.getElementById("add-photo-img"),
    {
      ...
      isActive: () =>
        !!_addFormPhotoData &&
        document.getElementById("add-photo-img")?.style.display !== "none",
    },
  );
  attachPhotoViewer(
    ...
    document.getElementById("wish-photo-img"),
    {
      ...
      isActive: () =>
        !!_wishFormPhotoData &&
        document.getElementById("wish-photo-img")?.style.display !== "none",
    },
  );
  attachPhotoViewer(
    ...
    document.getElementById("wd-photo-img"),
    {
      key: "wishDetail",
      isActive: () =>
        document.getElementById("wd-photo-img")?.style.display !== "none",
    },
  );
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (ps)
    ps.addEventListener("click", (e) => {
      if (e.target === ps) closeProfileSheet();
    });
...
function finishAuthUI(user) {
  ...
  if (pill) {
    ...
    pill.innerHTML =
      '<i class="fa-solid fa-user" style="font-size:12px;"></i> ' + user.name;
  }
  const wrap = document.getElementById("user-menu-wrap");
...
function applyRoleRestrictions(user) {
  ...
  if (header) {
    header.textContent =
      user.role === "clerk" ? "Add Stock - Mandela" : "Mandela General Stores";
  }
  if (user.role === "clerk" && user.tabs.includes("inventory")) {
    _activeInventoryTab = "add";
  }
...
function logout() {
  ...
  const wrap2 = document.getElementById("user-menu-wrap");
  if (wrap2) wrap2.style.display = "none";
  const pill2 = document.getElementById("user-pill");
...
async function attemptLogin() {
  ...
  localStorage.setItem(
    KEY_SESSION,
    JSON.stringify({ username: user.username, ts: Date.now() }),
  );
...
window.addEventListener("unhandledrejection", (e) => {
  ...
  if (e.reason && e.reason.message && e.reason.message.includes("Database"))
    toast("Warning: " + e.reason.message, "err");
});
...
function updateUpgradeStepUI(state) {
  ...
  stepIds.forEach((id) =>
    document.getElementById(id)?.classList.remove("settings-step--active"),
  );
  ...
  if (state === "available") {
    ...
    document
      .getElementById("upgrade-step-2")
      ?.classList.add("settings-step--active");
  } else if (state === "installing") {
    if (installBtn) installBtn.style.display = "none";
    if (idleNote) idleNote.style.display = "none";
    document
      .getElementById("upgrade-step-3")
      ?.classList.add("settings-step--active");
  } else {
    if (installBtn) installBtn.style.display = "none";
    if (idleNote) idleNote.style.display = "";
    document
      .getElementById("upgrade-step-1")
      ?.classList.add("settings-step--active");
  }
}
  ...
  if (typeof updateUpgradeStepUI === "function") {
    ...
    updateUpgradeStepUI(
      st && st.style.display !== "none"
        ? "available"
        : installing && installing.style.display !== "none"
          ? "installing"
          : "current",
    );
  }
...
async function checkPublishedAppVersion({ refreshWorker = true } = {}) {
  try {
    ...
    if (
      refreshWorker &&
      swRegistration &&
      typeof swRegistration.update === "function"
    ) {
      await swRegistration.update();
    }
    return true;
...
function _showUpdateBanner() {
  ...
  if (installBtn) {
    ...
    installBtn.textContent = "Install update now";
  }
...
async function _computeFinanceMovement() {
  ...
  const finances = await dbAll("finances");
  const sales = await dbAll("sales");
  const cleanFin = finances.filter(
    (e) => e.type !== "reconciliation" && e.type !== "revenue",
  );
  const cashToBusiness = cleanFin
    .filter((e) => e.type === "injection" || e.type === "investment")
    .reduce((s, e) => s + (e.amount || 0), 0);
  const stockAdded = cleanFin
    .filter((e) => e.type === "stock_purchase")
    .reduce((s, e) => s + (e.amount || 0), 0);
  const businessSpend = cleanFin
    .filter((e) => e.type === "expense" || e.type === "other")
    .reduce((s, e) => s + (e.amount || 0), 0);
  const personalWithdraws = cleanFin
    .filter((e) => e.type === "withdrawal")
    .reduce((s, e) => s + (e.amount || 0), 0);
  ...
  const salesCostOut = sales.reduce((s, e) => {
    const cost =
      Number.isFinite(e.buyPrice) && e.qty
        ? (e.buyPrice || 0) * (e.qty || 0)
        : (e.revenue || 0) - (e.profit || 0);
...
function _setFinanceRecordOptions() {
  const sel = document.getElementById("fin-type");
  if (!sel) return;
  const cur = ["injection", "expense", "withdrawal"].includes(sel.value)
    ? sel.value
    : "";
  sel.innerHTML =
...
async function getActivePurchaseBudget() {
  if (!db || !db.objectStoreNames.contains("purchase_budgets")) return null;
  const budgets = await dbAll("purchase_budgets");
  const selected =
    typeof _selectedPurchaseBudgetId !== "undefined"
      ? _selectedPurchaseBudgetId
      : "";
  return (
    budgets
      .filter((budget) => (budget.status || "active") === "active")
      .filter((budget) => !selected || String(budget.id) === String(selected))
      .sort(
        (a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0),
      )[0] || null
  );
...
async function renderPurchaseListOptions(selectId, selectedId) {
  const select = document.getElementById(selectId);
  if (!select || !db || !db.objectStoreNames.contains("purchase_budgets"))
    return;
  const budgets = (await dbAll("purchase_budgets"))
    .filter((budget) => (budget.status || "active") === "active")
    .sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0));
  select.innerHTML =
    '<option value="">No purchase list</option>' +
    budgets
      .map(
        (budget) =>
          '<option value="' +
          escapeHtml(String(budget.id)) +
          '">' +
          escapeHtml(budget.name || "Purchase list") +
          "</option>",
      )
      .join("");
...
async function selectPurchaseBudget(id) {
  _selectedPurchaseBudgetId = id || "";
  await renderPurchaseBudget();
}
window.selectPurchaseBudget = selectPurchaseBudget;

async function renderPurchaseListItems(budget) {
  const list = document.getElementById("purchase-list-items");
  const actions = document.querySelector(".purchase-list-actions");
  const moveTarget = document.getElementById("purchase-list-move-target");
  if (!list) return;
  if (!budget) {
    if (actions) actions.style.display = "none";
    list.innerHTML =
      '<div class="purchase-list-empty">Create a purchase list, then add Wishlist items to it.</div>';
    return;
  }
  if (actions) actions.style.display = "grid";
  const budgets = (await dbAll("purchase_budgets")).filter(
    (entry) =>
      (entry.status || "active") === "active" &&
      String(entry.id) !== String(budget.id),
  );
  if (moveTarget) {
      ...
      budgets
        .map(
          (entry) =>
            ...
            '">' +
            escapeHtml(entry.name || "Purchase list") +
            "</option>",
        )
        .join("");
  }
  const wishes = (await dbAll("wishlist")).filter(
    (wish) => String(wish.purchaseBudgetId || "") === String(budget.id),
  );
  if (!wishes.length) {
    list.innerHTML =
      '<div class="purchase-list-empty">No items in this purchase list yet. Add items from Wishlist.</div>';
    return;
  }
  list.innerHTML =
    '<div class="purchase-list-items-head">Items in this list <span>' +
    wishes.length +
    "</span></div>" +
    wishes
      .map((wish) => {
        ...
        return (
          ...
          (stocked ? " is-stocked" : "") +
          '">' +
          '<div class="purchase-list-item-main"><strong>' +
          escapeHtml(wish.name || wish.code || "Item") +
          "</strong><span>" +
          (wish.qty || 1) +
          " pcs" +
          (wish.estimatedCost
            ? " - " + fmt((wish.estimatedCost || 0) * (wish.qty || 1))
            : "") +
          (stocked ? " - Stocked" : "") +
          "</span></div>" +
          '<div class="purchase-list-item-actions">' +
          (stocked
            ? '<span class="purchase-list-stocked">Stocked</span>'
            : '<input class="purchase-list-actual-qty" id="purchase-qty-' +
              wish.id +
              '" type="number" min="1" max="' +
              (wish.qty || 1) +
              '" placeholder="Qty" aria-label="Actual quantity for ' +
              escapeHtml(wish.name || "item") +
              '"><input class="purchase-list-actual-price" id="purchase-price-' +
              wish.id +
              '" type="number" min="0" placeholder="Actual price" aria-label="Actual buying price for ' +
              escapeHtml(wish.name || "item") +
              '"><button type="button" onclick="beginPurchaseListPurchase(' +
              wish.id +
              ')">Purchase</button><button type="button" onclick="markPurchaseListItemStocked(' +
              wish.id +
              ')">Close as stocked</button>') +
          '<button type="button" onclick="returnPurchaseListItem(' +
          wish.id +
...
async function togglePurchaseWishlistChooser() {
  const chooser = document.getElementById("purchase-wishlist-chooser");
  if (!chooser) return;
  chooser.hidden = !chooser.hidden;
  if (!chooser.hidden) await renderPurchaseWishlistChooser();
}
window.togglePurchaseWishlistChooser = togglePurchaseWishlistChooser;

async function renderPurchaseWishlistChooser() {
  const list = document.getElementById("purchase-wishlist-chooser-list");
  const budget = await getActivePurchaseBudget();
  if (!list || !budget) return;
  const wishes = (await dbAll("wishlist")).filter(
    (wish) =>
      wishStatus(wish) !== "stocked" &&
      String(wish.purchaseBudgetId || "") !== String(budget.id),
  );
  list.innerHTML = wishes.length
    ? wishes
        .map(
          (wish) =>
            '<label class="purchase-wishlist-option"><input type="checkbox" value="' +
            wish.id +
            '"><span><strong>' +
            escapeHtml(wish.name || wish.code || "Item") +
            "</strong><small>" +
        ...
        .join("")
    : '<div class="purchase-list-empty">No unassigned Wishlist items available.</div>';
}

async function assignSelectedWishlistItems() {
  const budget = await getActivePurchaseBudget();
  if (!budget) return toast("Select a purchase list first", "info");
  const selected = Array.from(
    document.querySelectorAll("#purchase-wishlist-chooser-list input:checked"),
  );
  if (!selected.length)
    return toast("Select at least one Wishlist item", "info");
  for (const checkbox of selected)
    await updatePurchaseListItem(Number(checkbox.value), {
      purchaseBudgetId: budget.id,
    });
  const chooser = document.getElementById("purchase-wishlist-chooser");
...
window.assignSelectedWishlistItems = assignSelectedWishlistItems;

async function updatePurchaseListItem(wishId, changes) {
  const wish = await dbGet("wishlist", wishId);
  if (!wish) return null;
  Object.assign(wish, changes);
  await dbPut("wishlist", wish);
  scheduleSync();
  return wish;
}

async function beginPurchaseListPurchase(wishId) {
  const wish = await dbGet("wishlist", wishId);
  if (!wish) return;
  const qty = parseInt(
    document.getElementById("purchase-qty-" + wishId)?.value || "0",
    10,
  );
  const unitBuy = parseFloat(
    document.getElementById("purchase-price-" + wishId)?.value ||
      wish.estimatedCost ||
      "0",
  );
  const remaining = Number(wish.qty) || 0;
  if (!Number.isInteger(qty) || qty <= 0 || qty > remaining)
    return toast("Enter a quantity up to the remaining amount", "err");
  if (!Number.isFinite(unitBuy) || unitBuy < 0)
    return toast("Enter the actual buying price", "err");
  _purchaseProcessing = {
    wishId,
    qty,
    unitBuy,
    budgetId: wish.purchaseBudgetId || "",
  };
  await startWishlistRestock(wishId);
}
window.beginPurchaseListPurchase = beginPurchaseListPurchase;

async function markPurchaseListItemStocked(wishId) {
  const wish = await dbGet("wishlist", wishId);
  if (!wish) return;
  await markWishlistStockedById(
    wishId,
    wish.stockedItemId || null,
    wish.qty || 0,
    wish.estimatedCost || 0,
  );
  await renderPurchaseBudget();
  await renderWishlistPage();
  toast("Item marked stocked", "ok");
}
window.markPurchaseListItemStocked = markPurchaseListItemStocked;

async function returnPurchaseListItem(wishId) {
  await updatePurchaseListItem(wishId, {
    purchaseBudgetId: "",
    purchaseStatus: "unstocked",
    status: "prospective",
  });
  await renderPurchaseBudget();
  await renderWishlistPage();
  toast("Item returned to Wishlist", "ok");
}
window.returnPurchaseListItem = returnPurchaseListItem;

async function moveAllPurchaseListItems() {
  const budget = await getActivePurchaseBudget();
  const targetId =
    document.getElementById("purchase-list-move-target")?.value || "";
  if (!budget || !targetId)
    return toast("Choose another purchase list", "info");
  const wishes = (await dbAll("wishlist")).filter(
    (wish) =>
      String(wish.purchaseBudgetId || "") === String(budget.id) &&
      wishStatus(wish) !== "stocked",
  ...
  for (const wish of wishes)
    await updatePurchaseListItem(wish.id, { purchaseBudgetId: targetId });
  await renderPurchaseBudget();
  await renderWishlistPage();
  toast("Items moved to the other list", "ok");
}
window.moveAllPurchaseListItems = moveAllPurchaseListItems;

async function emptyPurchaseList() {
  const budget = await getActivePurchaseBudget();
  if (!budget) return;
  const wishes = (await dbAll("wishlist")).filter(
    (wish) =>
  ...
  for (const wish of wishes)
    await updatePurchaseListItem(wish.id, {
      purchaseBudgetId: "",
      purchaseStatus: "unstocked",
      status: "prospective",
    });
  await renderPurchaseBudget();
  await renderWishlistPage();
  toast("Unstocked items returned to Wishlist", "ok");
}
window.emptyPurchaseList = emptyPurchaseList;

async function recyclePurchaseList() {
  const budget = await getActivePurchaseBudget();
  if (!budget) return;
  const wishes = (await dbAll("wishlist")).filter(
    (wish) =>
  ...
  budget.status = "recycled";
  budget.recycledAt = new Date().toISOString();
  await dbPut("purchase_budgets", budget);
  scheduleSync();
  await renderPurchaseBudget();
  await renderWishlistPage();
  toast("Purchase list recycled", "ok");
}
window.recyclePurchaseList = recyclePurchaseList;

async function completePurchaseList() {
  const budget = await getActivePurchaseBudget();
  if (!budget) return;
  const outstanding = (await dbAll("wishlist")).some(
    (wish) =>
      String(wish.purchaseBudgetId || "") === String(budget.id) &&
      wishStatus(wish) !== "stocked",
  );
  if (outstanding)
    return toast("Process or return all unstocked items first", "info");
  budget.status = "completed";
  budget.completedAt = new Date().toISOString();
  await dbPut("purchase_budgets", budget);
  _selectedPurchaseBudgetId = "";
  scheduleSync();
  await renderPurchaseBudget();
  await renderWishlistPage();
  toast("Purchase list completed", "ok");
}
window.completePurchaseList = completePurchaseList;

async function renderPurchaseListHistory() {
  const el = document.getElementById("purchase-list-history");
  if (!el || !db || !db.objectStoreNames.contains("purchase_budgets")) return;
  const history = (await dbAll("purchase_budgets"))
    .filter((budget) => ["completed", "recycled"].includes(budget.status))
    .sort(
      (a, b) =>
        new Date(b.completedAt || b.recycledAt || b.createdAt || 0) -
        new Date(a.completedAt || a.recycledAt || a.createdAt || 0),
    );
  el.innerHTML = history.length
    ? '<div class="purchase-list-history-title">Purchase history</div>' +
      history
        .map((budget) => {
          const total = (budget.purchaseHistory || []).reduce(
            (sum, row) => sum + (Number(row.total) || 0),
            0,
          );
          const items = (budget.purchaseHistory || [])
            .map(
              (row) =>
                ...
                row.qty +
                " @ " +
                fmt(row.actualBuyPrice || 0),
            )
            .join(", ");
          return (
            '<div class="purchase-list-history-row"><strong>' +
            escapeHtml(budget.name || "Purchase list") +
            "</strong><span>" +
            (budget.status === "completed" ? "Completed" : "Recycled") +
            " - " +
            escapeHtml(
              String(
                (
...
function budgetDateInRange(date, budget) {
  if (!date) return true;
  const value = String(date).slice(0, 10);
  if (budget.startDate && value < budget.startDate) return false;
  if (budget.endDate && value > budget.endDate) return false;
  return true;
}

async function computePurchaseBudget() {
  const budget = await getActivePurchaseBudget();
  if (!budget)
    return {
      budget: null,
  ...
  const wishes = db.objectStoreNames.contains("wishlist")
    ? await dbAll("wishlist")
    : [];
  const plannedWishes = wishes.filter((wish) => {
    if ((wish.status || "prospective") === "stocked") return false;
    if (String(wish.purchaseBudgetId || "") !== String(budget.id)) return false;
    if (
      wish.plannedPurchaseDate &&
      !budgetDateInRange(wish.plannedPurchaseDate, budget)
    )
      return false;
    return Number(wish.estimatedCost) > 0;
  });
  const planned = plannedWishes.reduce(
    (sum, wish) =>
      sum + (Number(wish.estimatedCost) || 0) * (Number(wish.qty) || 1),
    0,
  );
  ...
  return {
    ...
    planned,
    purchased,
    remaining: amount - purchased,
    unfunded: Math.max(0, planned - amount),
    plannedCount: plannedWishes.length,
  };
}

function purchaseBudgetMetricsHtml(summary) {
  if (!summary.budget)
    return '<div class="purchase-budget-empty">Create a budget to track planned and actual stock spending.</div>';
  const cells = [
  ...
  return cells
    .map(
      ([label, value]) =>
        '<div class="purchase-budget-metric"><span>' +
        label +
        "</span><strong>" +
        fmt(value) +
        "</strong></div>",
...
async function renderPurchaseBudget() {
  const summary = await computePurchaseBudget();
  await renderPurchaseListOptions(
    "purchase-budget-select",
    summary.budget ? summary.budget.id : "",
  );
  const budgetSelect = document.getElementById("purchase-budget-select");
  if (budgetSelect?.options[0])
    budgetSelect.options[0].textContent = "Select purchase list";
  const status = document.getElementById("purchase-budget-status");
  const sub = document.getElementById("purchase-budget-sub");
  const metrics = document.getElementById("purchase-budget-metrics");
  const warning = document.getElementById("purchase-budget-warning");
  if (status) status.textContent = summary.budget ? "Active" : "No budget";
  if (sub)
    sub.textContent = summary.budget
      ? summary.budget.name || "Current budget"
      : "Plan stock spending before you buy.";
  if (metrics) metrics.innerHTML = purchaseBudgetMetricsHtml(summary);
  await renderPurchaseListItems(summary.budget);
  await renderPurchaseListHistory();
  if (warning) {
    const warnings = [];
    if (summary.unfunded > 0)
      warnings.push(
        "Planned purchases exceed this budget by " +
          fmt(summary.unfunded) +
          ".",
      );
    if (summary.remaining < 0)
      warnings.push(
        "Actual spending is over budget by " +
  ...
  const dash = document.getElementById("dash-budget-metrics");
  if (dash) {
    dash.innerHTML = summary.budget
      ? "<span>" +
        escapeHtml(summary.budget.name || "Active budget") +
        "</span><strong>Budget " +
        fmt(summary.budget.amount) +
        "</strong><strong>Planned " +
        fmt(summary.planned) +
        "</strong><strong>Purchased " +
        fmt(summary.purchased) +
        '</strong><strong class="' +
        (summary.remaining < 0 ? "is-over" : "") +
        '">Remaining ' +
        fmt(summary.remaining) +
        "</strong>"
      : "No active purchase budget";
  }
  return summary;
}

async function savePurchaseBudget() {
  const name = Input.text("budget-name");
  const amount = Input.money("budget-amount");
  const startDate = Input.raw("budget-start-date");
  const endDate = Input.raw("budget-end-date");
  if (!name) return Validate.fail("Enter a budget name", "budget-name");
  if (!Validate.moneyRequired(amount, "budget-amount", "Budget amount")) return;
  ...
  entry.id = await dbAdd("purchase_budgets", entry);
  _selectedPurchaseBudgetId = entry.id;
  [
    "budget-name",
    "budget-amount",
    "budget-start-date",
  ...
  scheduleSync();
  await renderPurchaseListOptions("wish-purchase-list", "");
  await renderPurchaseListOptions("wd-purchase-list", "");
  await renderPurchaseBudget();
  await renderWishlistPage();
  toast("Purchase budget set", "ok");
}
window.savePurchaseBudget = savePurchaseBudget;
window.renderPurchaseBudget = renderPurchaseBudget;
...
renderFinancePage = async function () {
  ...
  const setLabel = (id, text) => {
    ...
    const lbl =
      el && el.parentElement
        ? el.parentElement.querySelector(".fin-kpi-lbl")
        : null;
  ...
  if (netEl)
    netEl.style.color = money.businessPool >= 0 ? "var(--green)" : "var(--red)";
  if (netKpi)
    netKpi.className = "fin-kpi " + (money.businessPool >= 0 ? "green" : "red");
  ...
  const saleRows = money.sales.map((s) => ({
    ...
    revenue: s.revenue || 0,
    description:
      "Sale: " + (s.itemName || s.itemCode || "item") + " x " + (s.qty || 1),
    date: s.businessDate || (s.date || "").split("T")[0],
    createdAt: s.date,
    isSaleRow: true,
  }));
  const financeRows = money.finances.filter((e) =>
    ["injection", "stock_purchase", "expense", "withdrawal"].includes(e.type),
  );
  let listEntries = [...financeRows, ...saleRows];
  if (_finFilter === "investment")
    listEntries = listEntries.filter(
      (e) => e.type === "injection" || e.type === "stock_purchase",
    );
  if (_finFilter === "expense")
    listEntries = listEntries.filter(
      (e) =>
        e.type === "expense" ||
        e.type === "withdrawal" ||
  ...
  if (summaryLine) {
    summaryLine.textContent =
      ...
      fmt(money.businessPool) +
      " - Cash in " +
      fmt(money.cashToBusiness) +
      " - Stock added " +
      fmt(money.stockAdded) +
      " - Profit " +
      fmt(money.salesProfit);
  }
...
renderFinList = function (entries) {
  ...
  if (!entries.length) {
    list.innerHTML =
      '<div style="text-align:center;padding:28px 16px;color:var(--muted);font-size:13px;">No transactions yet.</div>';
    return;
  }
  const cfgMap = {
    sale_out: {
  ...
  const groupLabel = (e) =>
    e.type === "sale_out"
      ? "Sales"
      : e.type === "injection" || e.type === "stock_purchase"
        ? "Business In"
        : "Business Out";
  let lastGroup = "";
  const rows = entries.map((e) => {
    ...
    const header =
      grp !== lastGroup
        ? '<div style="background:var(--surface2);padding:7px 12px;font-size:10px;font-weight:900;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;">' +
          grp +
          "</div>"
        : "";
    lastGroup = grp;
    const delBtn =
      !e.isSaleRow && currentUser && currentUser.role === "super"
        ? '<button onclick="deleteFinanceEntry(' +
          e.id +
          ')" style="font-size:10px;color:var(--muted);background:none;border:none;cursor:pointer;padding:2px 4px;flex-shrink:0;">x</button>'
        : "";
    const sub =
      e.type === "sale_out"
    ...
    return (
      header +
      '<div style="display:flex;align-items:center;gap:10px;padding:9px 12px;background:var(--surface);border-bottom:1px solid var(--border);">' +
      '<span style="font-size:13px;font-weight:900;min-width:24px;text-align:center;color:' +
      c.color +
      ';">' +
      c.icon +
      "</span>" +
      '<div style="flex:1;min-width:0;">' +
      '<div style="font-size:12px;font-weight:800;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' +
      escapeHtml(e.description || c.label) +
      "</div>" +
      '<div style="font-size:10px;color:var(--muted);margin-top:1px;">' +
      sub +
      "</div>" +
      "</div>" +
      '<div style="font-size:13px;font-weight:900;font-family:var(--mono);color:' +
      c.color +
      ';flex-shrink:0;">' +
      (c.out ? "-" : "+") +
      fmt(e.amount || 0) +
...
saveFinanceEntry = async function () {
  ...
  if (!type || !validTypes.includes(type))
    return Validate.fail("Select a transaction type", "fin-type");
  ...
  if (
    dateCheck === "future" &&
    !confirm("Date is in the future - are you sure?")
  )
    return;
  const entry = {
    type,
...
const KEY_WISHLIST_SUPPLIERS = "mgs_wishlist_suppliers";

function getWishlistSuppliers() {
  try {
    const saved = JSON.parse(
      localStorage.getItem(KEY_WISHLIST_SUPPLIERS) || "[]",
    );
    return Array.isArray(saved)
      ? saved.filter(Boolean).sort((a, b) => a.localeCompare(b))
      : [];
  } catch (_) {
    return [];
  }
...
function renderWishlistSupplierOptions() {
...
function renderWishlistUnitOptions() {
  ...
  const selected = units.some((unit) => unit.abbr === current)
    ? current
    : units[0]?.abbr || "";
  ...
  picker.innerHTML = units.length
    ? units
        .map(
          (unit) =>
            '<button type="button" class="unit-chip wish-unit-chip' +
            (unit.abbr === selected ? " unit-chip-on" : "") +
            '" role="radio" aria-checked="' +
            (unit.abbr === selected ? "true" : "false") +
            '" onclick="selectWishlistUnit(&quot;' +
            escapeHtml(unit.abbr) +
            '&quot;)"><span class="unit-chip-abbr">' +
            escapeHtml(unit.abbr) +
            '</span><span class="unit-chip-name">' +
...
function selectWishlistUnit(abbr) {
  ...
  valueInput.value = activeUnits.some((unit) => unit.abbr === abbr)
    ? abbr
    : activeUnits[0]?.abbr || "";
  renderWishlistUnitOptions();
}
...
function renderWishlistSuppliers() {
  ...
  if (!list) return;
  const suppliers = getWishlistSuppliers();
  list.innerHTML = suppliers.length
    ? suppliers
        .map(
          (supplier) =>
            '<div style="display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:1px solid var(--border);"><span>' +
            escapeHtml(supplier) +
            '</span><button type="button" class="settings-btn settings-btn--ghost" onclick="removeWishlistSupplier(' +
            JSON.stringify(supplier).replace(/</g, "\\u003c") +
            ')"><i class="fa-solid fa-trash"></i></button></div>',
        )
        .join("")
    : '<div style="font-size:12px;color:var(--muted);">No suppliers added yet.</div>';
  renderWishlistSupplierOptions();
}

function addWishlistSupplier() {
  const input = document.getElementById("supplier-setting-input");
  const name = String(input?.value || "").trim();
  if (!name) return toast("Enter a supplier name", "err");
  const suppliers = getWishlistSuppliers();
  if (
    !suppliers.some((supplier) => supplier.toLowerCase() === name.toLowerCase())
  )
    suppliers.push(name);
  localStorage.setItem(KEY_WISHLIST_SUPPLIERS, JSON.stringify(suppliers));
  if (input) input.value = "";
  renderWishlistSuppliers();
  toast("Supplier added", "ok");
}
window.addWishlistSupplier = addWishlistSupplier;

function removeWishlistSupplier(name) {
  const suppliers = getWishlistSuppliers().filter(
    (supplier) => supplier !== name,
  );
  localStorage.setItem(KEY_WISHLIST_SUPPLIERS, JSON.stringify(suppliers));
  renderWishlistSuppliers();
}
window.removeWishlistSupplier = removeWishlistSupplier;
...
function toggleUnit(abbr) {
  ...
  _saveUnits(units);
  renderUnitsSettings();
  if (typeof renderWishlistUnitOptions === "function")
    renderWishlistUnitOptions();
}
...
function addCustomUnit() {
  ...
  if (!abbr) {
    toast("Enter abbreviation (e.g. Pkt)", "err");
    return;
  }
  if (!name) {
    toast("Enter full name (e.g. Packet)", "err");
    return;
  }
  const units = getUnits();
  if (units.find((u) => u.abbr.toLowerCase() === abbr.toLowerCase())) {
    toast('"' + abbr + '" already exists', "err");
    return;
  }
...
function renderUnitsSettings() {
  ...
  if (countEl)
    countEl.textContent = units.filter((u) => u.active).length + " active";
}
...
function saveGeminiKey() {
  ...
  if (!val) {
    toast("Paste your Gemini API key first", "err");
    return;
  }
  if (!val.startsWith("AIza")) {
    toast(
      'Key should start with "AIza" — check you copied the full key',
      "err",
    );
    return;
  }
...
function clearGeminiKey() {
  if (!confirm("Remove the Gemini API key? AI features will be disabled."))
    return;
...
function _aiUpdateKeyUI(hasKey) {
  ...
  if (dot) {
    dot.className =
      "ai-set-dot " + (hasKey ? "ai-set-dot-on" : "ai-set-dot-off");
  }
}
...
async function _callGemini(userPrompt, systemPrompt) {
  ...
  if (!key) {
    toast("Add Gemini API key in Settings → AI", "err");
    return null;
  }
  const url =
...
function aiAskMode() {
  ...
  document.getElementById("ai-chat-body").innerHTML =
    '<div class="ai-msg ai-msg-ai">Ask me anything about your inventory, sales, pricing or business performance. I have access to your data.</div>';
}
...
async function _aiContext() {
  ...
  const salesSummary = last30sales
    .map(
      (s) =>
...
async function aiRestockScan() {
  ...
  try {
    ...
    allSales.forEach((s) => {
      ...
      if (s.businessDate > missing[s.itemCode].lastSold)
        missing[s.itemCode].lastSold = s.businessDate || "";
    });
    ...
    if (!list.length) {
      _aiShowResult(
        '<div class="ai-msg ai-msg-ai">✅ All items sold in your history are already in your inventory. Nothing missing!</div>',
      );
      return;
    }
...
For each item: suggest a short restocking note (1 line). Format as JSON array: [{"code":"..","name":"..","suggestion":".."}]`;
    ...
    const sys =
      "You are a concise retail inventory advisor. Respond only with valid JSON.";
    ...
    const cards = list
      .map((x, i) => {
        const sug =
          suggestions.find((s) => s.code === x.code)?.suggestion || "";
    ...
    _aiShowResult(
      `<div class="ai-msg ai-msg-ai" style="margin-bottom:10px;">Found <strong>${list.length}</strong> items sold but not in inventory. Tap <strong>Add</strong> to add any to your stock.</div>${cards}`,
    );
  } catch (e) {
    _aiShowResult(
      '<div class="ai-msg ai-msg-err">Error: ' +
...
async function aiInsights() {
  ...
  try {
    ...
    const sys =
      "You are a friendly business advisor for a small Kenyan retail shop. Be concise, use bullet points, mention specific item names and numbers. Use KES for currency.";
...
async function aiSendQuestion() {
  ...
  body.innerHTML +=
    '<div class="ai-msg ai-msg-user">' + escapeHtml(q) + "</div>";
  body.innerHTML +=
    '<div class="ai-loading" id="ai-inline-loading"><span class="ai-dot"></span><span class="ai-dot"></span><span class="ai-dot"></span></div>';
  ...
  try {
    ...
    const prompt = ctx + "\n\nUser question: " + q;
    const sys =
      "You are a helpful business assistant for Mandela General Stores, a small Kenyan retail shop. Use the inventory and sales data provided. Be concise and practical. Use KES for currency.";
    const text = await _callGemini(prompt, sys);
...
function _aiShowFab() {
  ...
  fab.title = getGeminiKey()
    ? "AI Assistant"
    : "AI Assistant — add API key in Settings";
}
...
async function _gscSearch(raw) {
  ...
  if (!scored.length) {
    container.innerHTML =
      '<div class="gsc-empty"><i class="fa-solid fa-magnifying-glass"></i> No items match "<strong>' +
      escapeHtml(raw) +
      '</strong>"</div>';
  ...
  const soldMap = {};
  allSales.forEach((s) => {
    soldMap[s.itemId] = (soldMap[s.itemId] || 0) + (s.qty || 1);
  });
...
function _getDayRecon(date) {
  try {
    return JSON.parse(localStorage.getItem(DAY_RECON_KEY(date)) || "null");
  } catch (e) {
    return null;
  }
}
function _saveDayRecon(date, data) {
  try {
    localStorage.setItem(DAY_RECON_KEY(date), JSON.stringify(data));
  } catch (e) {}
}
function _clearDayRecon(date) {
  try {
    localStorage.removeItem(DAY_RECON_KEY(date));
  } catch (e) {}
}
...
async function _doCloseDay() {
  ...
  const daySales = allSales.filter(
    (s) => (s.businessDate || s.business_date) === today,
  );
  const items = await dbAll("items");
  activeDay.status = "CLOSED";
  activeDay.closed_at = now.toISOString();
...
function _renderOpeningSummary(data) {
  ...
  const f = (v) => (v ? fmt(v) : "-");
  const t = new Date(data.lockedAt || 0).toLocaleTimeString("en-GB", {
    hour: "2-digit",
    minute: "2-digit",
  });
  el.innerHTML =
    '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:8px;">' +
    '<div style="text-align:center;"><div style="font-size:15px;font-weight:900;font-family:var(--mono);">' +
    f(o.cash) +
    '</div><div style="font-size:9px;color:var(--muted);font-weight:700;text-transform:uppercase;margin-top:2px;">Cash</div></div>' +
    '<div style="text-align:center;"><div style="font-size:15px;font-weight:900;font-family:var(--mono);">' +
    f(o.till) +
    '</div><div style="font-size:9px;color:var(--muted);font-weight:700;text-transform:uppercase;margin-top:2px;">Till</div></div>' +
    '<div style="text-align:center;"><div style="font-size:15px;font-weight:900;font-family:var(--mono);color:#6366f1;">' +
    f(o.mpesa) +
    '</div><div style="font-size:9px;color:var(--muted);font-weight:700;text-transform:uppercase;margin-top:2px;">M-Pesa</div></div>' +
    "</div>" +
    '<div style="display:flex;justify-content:space-between;align-items:center;border-top:1px solid #a8d8b5;padding-top:8px;">' +
    '<span style="font-size:10px;color:var(--muted);">Locked ' +
    t +
    "</span>" +
    '<span style="font-size:13px;font-weight:900;font-family:var(--mono);color:var(--green);">Total: ' +
    fmt(o.total || 0) +
    "</span>" +
    "</div>";
}
...
function _renderReconcileInsights(data, today) {
  ...
  if (an.variance == null && an.correctDay != null)
    an.variance = (an.actualDay || 0) - (an.correctDay || 0);
  ...
  if (isOk) {
    ins.push({
      i: "",
      c: "rc-ok",
      t: "Perfect - every shilling accounted for!",
    });
    ...
    ins.push({
      ...
      t:
        "Surplus of " +
        fmt(an.variance) +
        ". More cash than expected. Check for unrecorded injection, or a deposit not captured.",
    });
  } else {
    ins.push({
      i: "",
      ...
      t:
        "Short by " +
        fmt(absV) +
        ". Less cash than expected. Check for unrecorded expense, undeclared withdrawal, or theft.",
    });
  }
  if (
    cl.expenses > 0 &&
    sy.sysTotalRev > 0 &&
    cl.expenses > sy.sysTotalRev * 0.35
  )
    ins.push({
      i: "",
      c: "rc-warn",
      t:
        ...
        fmt(cl.expenses) +
        ") are " +
        ((cl.expenses / sy.sysTotalRev) * 100).toFixed(0) +
        "% of revenue - high for today.",
    });
  if (sy.margin < 10 && sy.sysTotalRev > 0)
    ins.push({
      i: "",
      c: "rc-warn",
      t: "Low margin: " + sy.margin.toFixed(1) + "%. Review prices or costs.",
    });
  else if (sy.margin >= 30 && sy.sysTotalRev > 0)
    ins.push({
      i: "",
      c: "rc-ok",
      t: "Great margin: " + sy.margin.toFixed(1) + "%!",
    });
  if (an.netMove < 0)
    ins.push({
      i: "",
      c: "rc-bad",
      t:
        "Net movement is negative (" +
        fmt(an.netMove) +
        "). Business paid out more than it earned today.",
    });
  if (sy.salesCount === 0)
  ...
  el.innerHTML =
    ...
    '<div style="padding:10px 4px;border-right:1px solid var(--border);"><div style="font-size:16px;font-weight:900;font-family:var(--mono);color:var(--green);">' +
    sy.salesCount +
    '</div><div style="font-size:9px;color:var(--muted);font-weight:700;text-transform:uppercase;margin-top:2px;">Sales</div></div>' +
    '<div style="padding:10px 4px;border-right:1px solid var(--border);"><div style="font-size:13px;font-weight:900;font-family:var(--mono);color:var(--green);">' +
    fmt(sy.sysTotalRev) +
    '</div><div style="font-size:9px;color:var(--muted);font-weight:700;text-transform:uppercase;margin-top:2px;">Revenue</div></div>' +
    '<div style="padding:10px 4px;border-right:1px solid var(--border);"><div style="font-size:13px;font-weight:900;font-family:var(--mono);color:var(--green);">' +
    fmt(sy.sysTotalProf) +
    '</div><div style="font-size:9px;color:var(--muted);font-weight:700;text-transform:uppercase;margin-top:2px;">Profit</div></div>' +
    '<div style="padding:10px 4px;"><div style="font-size:13px;font-weight:900;font-family:var(--mono);color:var(--accent);">' +
    sy.margin.toFixed(1) +
    '%</div><div style="font-size:9px;color:var(--muted);font-weight:700;text-transform:uppercase;margin-top:2px;">Margin</div></div>' +
    "</div>" +
    (cl.injected > 0
      ? '<div style="display:flex;justify-content:space-between;padding:7px 12px;border-top:1px solid var(--border);font-size:11px;background:var(--surface);"><span>Injected</span><span style="font-weight:800;color:var(--green);">+' +
        fmt(cl.injected) +
        "</span></div>"
      : "") +
    (cl.expenses > 0
      ? '<div style="display:flex;justify-content:space-between;padding:7px 12px;border-top:1px solid var(--border);font-size:11px;background:var(--surface);"><span>Expenses</span><span style="font-weight:800;color:var(--red);">-' +
        fmt(cl.expenses) +
        "</span></div>"
      : "") +
    (cl.withdrawn > 0
      ? '<div style="display:flex;justify-content:space-between;padding:7px 12px;border-top:1px solid var(--border);font-size:11px;background:var(--surface);"><span>Withdrawn</span><span style="font-weight:800;color:#d97706;">-' +
        fmt(cl.withdrawn) +
        "</span></div>"
      : "") +
    ...
    (an.netMove >= 0 ? "var(--green)" : "var(--red)") +
    ';">' +
    (an.netMove >= 0 ? "+" : "") +
    fmt(an.netMove) +
    "</span>" +
    ...
    "</b>" +
    "</div>" +
    '<div style="font-size:18px;font-weight:900;font-family:var(--mono);color:var(--accent);border-top:1px solid var(--border);padding-top:8px;">' +
    fmt(an.correctDay) +
    "</div>" +
    "</div>" +
    '<div style="background:' +
    (isOk ? "var(--green-light)" : isWn ? "#fef3c7" : "var(--red-light)") +
    ";border:1.5px solid " +
    (isOk ? "#a8d8b5" : isWn ? "#f5d9a0" : "#fca5a5") +
    ...
    (cl.withdrawn > 0
      ? "<br>Withdrawn: <b>" + fmt(cl.withdrawn) + "</b>"
      : "") +
    "</div>" +
    ...
    "</div>" +
    "</div>" +
    '<div style="display:flex;justify-content:space-between;align-items:center;padding:12px 14px;background:' +
    (isOk ? "var(--green-light)" : isWn ? "#fef3c7" : "var(--red-light)") +
    ";border:1.5px solid " +
    (isOk ? "#a8d8b5" : isWn ? "#f5d9a0" : "#fca5a5") +
    ...
    vc +
    ';">Variance</span>' +
    '<span style="font-size:20px;font-weight:900;font-family:var(--mono);color:' +
    vc +
    ';">' +
    vi +
    " " +
    ...
    ins
      .map(
        (i) =>
          '<div class="' +
          i.c +
          '" style="display:flex;align-items:flex-start;gap:10px;padding:9px 12px;border-radius:var(--r);margin-bottom:5px;font-size:12px;font-weight:600;line-height:1.4;"><span style="font-size:16px;flex-shrink:0;">' +
          i.i +
          "</span><span>" +
          i.t +
          "</span></div>",
      ...
      .join("") +
    '<div style="text-align:center;font-size:10px;color:var(--muted);padding:6px 0;">Reconciled at ' +
    new Date(data.reconciledAt || 0).toLocaleTimeString("en-GB", {
      hour: "2-digit",
      minute: "2-digit",
    }) +
    "</div>";
...
function _clearOpeningInputsOnly() {
  ["op-cash", "op-till", "op-mpesa"].forEach((id) => {
    const el = document.getElementById(id);
    if (el) el.value = "0";
  });
}
...
function _moveSalesDetailsAfterOpening() {
  ...
  if (!sales) return;
  if (openingLocked && openingLocked.style.display !== "none")
    openingLocked.insertAdjacentElement("afterend", sales);
  else if (openingForm && openingForm.style.display !== "none")
    openingForm.insertAdjacentElement("afterend", sales);
}
...
renderDayState = function () {
  ...
  if (salesDetails)
    salesDetails.style.display = step === "reconciled" ? "none" : "grid";

  ["open", "opening-form", "close-btn", "closing-form", "reconciled"].forEach(
    (s) => {
...
reconcileDay = async function () {
  ...
  if (!data || !data.opening) {
    toast("Record opening balances first", "err");
    return;
  }
  ...
  if (!Validate.moneyOptional(injectedRaw, "cl-injected", "Cash to business"))
    return;
  if (!Validate.moneyOptional(expensesRaw, "cl-expenses", "Expenses")) return;
  if (!Validate.moneyOptional(withdrawnRaw, "cl-withdrawn", "Withdrawn"))
    return;
  ...
  const mismatchLines = _warnFinanceClosingMismatch(finTotals, {
    injected,
    expenses,
    withdrawn,
  });
  ...
  const sysCashRev = daySales
    .filter((s) => !s.paymentMethod || s.paymentMethod === "cash")
    .reduce((a, s) => a + (s.revenue || 0), 0);
  const sysMpesaRev = daySales
  ...
  _saveDayRecon(today, {
    ...
    closing: {
      injected: useInjected,
      cash,
      till,
      mpesa,
      expenses: useExpenses,
      withdrawn: useWithdrawn,
    },
    system: {
      sysCashRev,
      sysMpesaRev,
...
dayStartOver = async function () {
  ...
  if (
    !confirm(
      "Clear today's closing records only?\n\nOpening balances will be kept.",
    )
  )
    return;
  const data = _getDayRecon(today);
  ...
  const fins = await dbAll("finances");
  for (const e of fins) {
    if (e.type === "reconciliation" && (e.date || "").slice(0, 10) === today)
      await dbDelete("finances", e.id);
  }
  if (activeDay) {
...
function scheduleSync(delay = 1200) {
  ...
  _autoSyncTimer = setTimeout(async () => {
    ...
    try {
      ...
      console.warn("[SYNC] scheduled drain failed:", e.message);
    } finally {
      _syncRunning = false;
      updateSyncDot();
    }
...
async function renderHistoryPage() {
  ...
  if (filterVal === "today") {
    ...
    if (customFrom && customTo && customFrom <= customTo) {
      ...
      rangeLabel =
        customFrom && customTo ? "Invalid date range" : "Choose a date range";
    }
...
function _histSortArrow(key) {
  if (_histSort.key !== key) return '<span class="hist-sort-arrow">↕</span>';
...
function sortHistTable(key) {
  if (_histSort.key === key) _histSort.dir *= -1;
    ...
    _histSort.key = key;
    _histSort.dir = key === "item" ? 1 : -1;
  }
  const date = window._histExpandedDate;
  if (!date) return;
  _expandedHistDay = null;
  expandHistDay(date.replace(/-/g, ""));
}
    ...
    const values =
      {
        ...
        revenue: [
          ...
          (b.actualPrice || b.sellPrice || 0) * qtyB,
        ],
        earning: [a.profit || 0, b.profit || 0],
      }[_histSort.key] || [];
    const result = values[0] < values[1] ? -1 : values[0] > values[1] ? 1 : 0;
    return result * _histSort.dir;
  });
...
async function _backfillSalesForItem(item) {
  ...
  const allSales = await dbAll("sales");
  const toFix = allSales.filter(
    (s) => s.itemCode === item.code && s.itemId !== item.id,
  );
  ...
  console.log(
    `[BACKFILL] Linked ${toFix.length} sale(s) to item ${item.code} (id=${item.id})`,
  );
  return toFix.length;
}
...
async function exportSalesReport() {
  ...
  const allSales = await dbAll("sales");
  const filterVal =
    document.getElementById("hist-period-filter")?.value || "all";
  const today = todayDateStr();
  const _histPay = window._histPayFilter || "all";
  const _histSearch = (document.getElementById("hist-item-search")?.value || "")
    .toLowerCase()
    .trim();
  ...
  const filtered = [...allSales]
    .filter((s) => {
      ...
      if (
        _histPay !== "all" &&
        (s.paymentMethod || "cash").toLowerCase() !== _histPay
      )
        return false;
      if (
        _histSearch &&
        !(s.itemName || "").toLowerCase().includes(_histSearch) &&
        !(s.itemCode || "").toLowerCase().includes(_histSearch)
      )
        return false;
      return true;
    })
  ...
  const a = Object.assign(document.createElement("a"), {
    href: url,
    download: "sales-" + periodLabel + ".csv",
  });
...
async function openSaleDetail(saleId) {
  ...
  pmBadge.className =
    "sds-item-badge sds-pm-" + (sale.paymentMethod || "cash").toLowerCase();
  ...
  try {
    ...
    if (stockDelta !== 0 && !item.isRecord) {
      if (size) {
        ...
        item.qty = allSizes.reduce(
          (total, record) => total + Number(record.qty || 0),
          0,
        );
      } else {
...
async function saleAddToInventoryFromDetail() {
  ...
  const matchType = types.find(
    (t) => t.name === sale.itemType && isCategoryActive(t),
  );
  const useType = matchType
    ? sale.itemType
    : types.find((t) => t.name === "General" && isCategoryActive(t))
      ? "General"
      : "";
...
function renderAllShoeGroupCards() {
  ...
  entries.forEach(([g, config]) => {
    ...
    for (let s = min; s <= max; s++) {
      ...
      btn.className =
        "sz-btn" +
        (_shoeState.sizes.has(s) ? " sz-active" : "") +
        (isLocked ? " sz-locked" : "");
      btn.id = "sz-" + s;
...
function setShoeMode(mode) {
  ...
  // Update tab buttons
  document
    .getElementById("mode-tab-shared")
    .classList.toggle("active", !_shoeState.perSizeMode);
  document
    .getElementById("mode-tab-persize")
    .classList.toggle("active", _shoeState.perSizeMode);
  ...
  const perSizeWrap = document.getElementById("shoe-per-size-wrap");
  if (sharedWrap)
    sharedWrap.style.display = _shoeState.perSizeMode ? "none" : "block";
  if (perSizeWrap)
    perSizeWrap.style.display = _shoeState.perSizeMode ? "block" : "none";
  ...
  if (rows) {
    rows.addEventListener("input", (e) => {
      if (
        e.target &&
        e.target.classList &&
        e.target.classList.contains("shoe-cell")
      ) {
        updateShoeCollectiveSummary();
      }
    });
...
async function upsertShoeSize(record, opts) {
  ...
  const existing = all.find(
    (s) => s.itemCode === record.itemCode && s.size === record.size,
  );
  if (existing) {
...
async function saveShoeItems(baseCode, baseName, type) {
  if (_shoeState.sizes.size === 0) {
    toast("Warning: Select at least one size", "err");
    return false;
  }
  ...
  if (!_shoeState.perSizeMode) {
    ...
    if (sharedQty < 0) {
      toast("Warning: Quantity cannot be negative", "err");
      return false;
    }
    if (sharedBuy < 0) {
      toast("Warning: Buy price cannot be negative", "err");
      return false;
    }
    if (sharedSell < 0) {
      toast("Warning: Sell price cannot be negative", "err");
      return false;
    }
    if (sharedBuy > 0 && sharedSell > 0 && sharedSell < sharedBuy) {
      toast("Warning: Sell price cannot be less than buy price", "err");
      return false;
    }
  }
  ...
  for (const size of sorted) {
    ...
    if (_shoeState.perSizeMode) {
      ...
      if (qty < 0) {
        perSizeErrors.push("Size " + size + ": quantity cannot be negative");
        continue;
      }
      if (buy < 0) {
        perSizeErrors.push("Size " + size + ": buy price cannot be negative");
        continue;
      }
      if (sell < 0) {
        perSizeErrors.push("Size " + size + ": sell price cannot be negative");
        continue;
      }
      if (buy > 0 && sell > 0 && sell < buy) {
        perSizeErrors.push(
          "Size " + size + ": sell price cannot be less than buy price",
        );
        continue;
      }
    } else {
    ...
    await upsertShoeSize(
      {
        ...
        itemId: product.id,
        size,
        sizeGroup: _shoeState.groupFor(size),
        qty,
        buyPrice: buy,
        sellPrice: sell,
        profit: sell - buy,
  ...
  if (perSizeErrors.length)
    toast("Warning: Skipped: " + perSizeErrors.join(" - "), "err");
  if (saved === 0) {
    toast("Warning: No sizes saved - check quantity and price values", "err");
    return false;
  }
...
function toggleShoeSize(s) {
  ...
  if (_shoeState.sizes.has(s)) _shoeState.sizes.delete(s);
  else _shoeState.sizes.add(s);
...
async function openShoeSizeEdit(itemId, size) {
  ...
  setTimeout(() => {
    ...
    if (_ml3) {
      _ml3.hidden = false;
      _ml3.textContent = "Edit size " + size + " - " + item.code;
    }
    UI.el("cancel-edit-btn").style.display = "block";
    updateProfitPreview();
  }, 100);
...
async function openSellShoeModal(itemId, size) {
  ...
  const sizes = await getShoeSizes(item.code);
  const sizeRec = sizes.find((s) => s.size === size);
  const availableQty = sizeRec
    ? await readableStockQty(sizeRec, sizeRec.id)
    : 0;
  ...
  const el = (id) => document.getElementById(id);
  if (el("sm-icon")) {
    el("sm-icon").textContent = t.emoji;
    el("sm-icon").style.background = t.color || "var(--surface2)";
  }
  if (el("sm-name"))
...
function renderShoeRows() {
  ...
  rows.innerHTML = sorted
    .map(
      (s) =>
        '<div class="shoe-row">' +
        '<span class="shoe-sz-lbl">' +
        s +
        "</span>" +
        '<input type="number" class="shoe-cell" id="shr-qty-' +
        s +
        '" min="0" inputmode="numeric" placeholder="Qty">' +
        '<input type="number" class="shoe-cell" id="shr-buy-' +
        s +
        '" min="0" inputmode="decimal" placeholder="BP">' +
        '<input type="number" class="shoe-cell" id="shr-sell-' +
        s +
        '" min="0" inputmode="decimal" placeholder="SP">' +
        "</div>",
    )
    .join("");
  updateShoeCollectiveSummary();
...
function renderShoeSummary() {
  ...
  if (saveBtn && panel && panel.style.display !== "none" && sorted.length) {
    setSaveBtnLabel(
      "Save " + sorted.length + " size" + (sorted.length > 1 ? "s" : ""),
    );
  }
}
...
async function extractFromPhoto() {
  ...
  if (thumb && _piImageDataUrl) {
    thumb.src = _piImageDataUrl;
  ...
  try {
    ...
    var prompt =
      "Look at this handwritten stock list image. Extract all inventory items you can see. " +
...
function _piRenderPreviewTable() {
  ...
  if (badge)
    badge.textContent =
      _piExtractedRows.length +
      " item" +
      (_piExtractedRows.length !== 1 ? "s" : "");
  ...
  for (var i = 0; i < _piExtractedRows.length; i++) {
    ...
    html +=
      ...
      i +
      '">' +
      '<td><input class="pi-cell" type="text" value="' +
      escapeHtml(row.name) +
      '"' +
      ' onchange="_piUpdateRow(' +
      i +
      ',\'name\',this.value)" placeholder="Item name"></td>' +
      '<td><input class="pi-cell pi-mono" type="text" value="' +
      escapeHtml(row.code) +
      '"' +
      ' onchange="_piUpdateRow(' +
      i +
      ',\'code\',this.value)" placeholder="CODE" style="text-transform:uppercase;"></td>' +
      '<td><select class="pi-select" onchange="_piUpdateRow(' +
      i +
      ",'category',this.value)\">" +
      catSel +
      "</select></td>" +
      '<td class="pi-num"><input class="pi-cell pi-num" type="number" value="' +
      row.qty +
      '" min="0"' +
      ' onchange="_piUpdateRow(' +
      i +
      ...
      ' onchange="_piUpdateRow(' +
      i +
      ",'buyPrice',this.value)\"></td>" +
      '<td class="pi-num"><input class="pi-cell pi-num" type="number" value="' +
      row.sellPrice +
      '" min="0"' +
      ' onchange="_piUpdateRow(' +
      i +
      ",'sellPrice',this.value)\"></td>" +
      '<td><button class="pi-del-btn" onclick="_piDeleteRow(' +
      i +
      ')" title="Remove row">' +
...
async function importPhotoItems() {
  ...
  for (var ii = 0; ii < valid.length; ii++) {
    ...
    try {
      ...
      for (var ei = 0; ei < allItems.length; ei++) {
        if (
          (allItems[ei].code || "").trim().toUpperCase() ===
  ...
  var msg = "";
  if (imported > 0)
    msg += imported + " new item" + (imported !== 1 ? "s" : "") + " added. ";
  if (restocked > 0)
    msg += restocked + " item" + (restocked !== 1 ? "s" : "") + " restocked. ";
  if (skipped > 0)
    msg +=
      skipped +
      " row" +
      (skipped !== 1 ? "s" : "") +
      " skipped (missing name/code). ";
...
function onCustItemTypeChange() {
  ...
  if (title)
    title.textContent = type === "return" ? "Record Return" : "Add Item";
}
...
async function renderCustomerList(query) {
  ...
  container.innerHTML = customers
    .map((c) => {
      ...
      const balColor =
        bal > 0 ? "#dc2626" : bal < 0 ? "#16a34a" : "var(--muted)";
      const balLabel = bal > 0 ? "Owes" : bal < 0 ? "Credit" : "Clear";
      const balVal = bal !== 0 ? balNum : "";
      const avatarBg =
        bal > 0 ? "#dc2626" : bal < 0 ? "#16a34a" : "var(--accent)";
...
async function openCustomerDetail(customerId) {
  ...
  const customer = (await dbAll("customers")).find(
  ...
  if (
    !confirm(
      'Move "' +
        ...
        '" to recycle bin?\n\nTheir records will be kept and can be restored later.',
    )
  )
    return;
...
async function _renderCustomerOverview(customer) {
  ...
  if (!el) return;
  const txns = (await dbAll("customer_txns")).filter(
...
async function _renderCustomerLedger(customerId) {
  ...
  if (!txns.length) {
    el.innerHTML =
      summaryHtml +
      '<div style="padding:24px;text-align:center;color:var(--muted);font-size:13px;">No transactions yet.</div>';
    return;
  }
  ...
  const rows = txns
    .map((t) => {
      ...
      const debit = isDebit
        ? t.totalValue || (t.qty || 1) * (t.unitPrice || 0)
        : 0;
      ...
      const detail =
        t.type !== "payment" && qty > 0 && price > 0
          ? ` ×${qty} @ ${_fmtNum(price)}`
          : "";
      const amtVal =
        debit > 0 ? _fmtNum(debit) : credit > 0 ? _fmtNum(credit) : "";
  ...
  el.innerHTML =
    summaryHtml +
    `<table class="cust-ledger-table cust-ledger-lined" style="table-layout:fixed;width:100%;">
      <colgroup>
...
async function openCustItemSheet() {
  ...
  const qty = document.getElementById("cust-item-qty");
  if (qty) qty.value = "1";
  const date = document.getElementById("cust-item-date");
  if (date) date.value = todayDateStr();
  const type = document.getElementById("cust-item-type");
  if (type) type.value = "item";
  onCustItemTypeChange();
  const overlay = document.getElementById("cust-item-sheet");
  if (overlay) overlay.classList.add("open");
  setTimeout(() => {
    const n = document.getElementById("cust-item-name");
    if (n) n.focus();
  }, 120);
}
...
async function saveCustItem() {
  ...
  if (!itemName) {
    toast("Description is required", "err");
    return;
  }
  if (qty <= 0) {
    toast("Quantity must be greater than 0", "err");
    return;
  }
  if (unitPrice <= 0) {
    toast("Price must be greater than 0", "err");
    return;
  }
  ...
  const now = new Date().toISOString();
  const customer = (await dbAll("customers")).find(
    (c) => c.customerId === _currentCustomerId,
  );
  if (!customer) {
    toast("Customer not found", "err");
    return;
  ...
  try {
    ...
    const updated = (await dbAll("customers")).find(
      (c) => c.customerId === _currentCustomerId,
    );
...
async function openCustPaymentSheet() {
  const a = document.getElementById("cust-pay-amount");
  if (a) a.value = "";
  const m = document.getElementById("cust-pay-method");
  if (m) m.value = "Cash";
  const n = document.getElementById("cust-pay-note");
  if (n) n.value = "";
  const d = document.getElementById("cust-pay-date");
...
async function _recalcBalance(customerId) {
  ...
  const totalTaken = custTxns
    .filter((t) => t.type === "item")
    .reduce((s, t) => s + (t.totalValue || 0), 0);
  const totalReturns = custTxns
    .filter((t) => t.type === "return")
    .reduce((s, t) => s + (t.totalValue || 0), 0);
  const totalPaid = custTxns
    .filter((t) => t.type === "payment")
    .reduce((s, t) => s + (t.amount || 0), 0);
  const balance = totalTaken - totalReturns - totalPaid;
  const sorted = [...custTxns].sort((a, b) =>
...
async function openEditCustTxn(txnId) {
  ...
  if (!txn) {
    toast("Transaction not found", "err");
    return;
  }
  const g = (id) => document.getElementById(id);
  if (g("edit-cust-txn-id")) g("edit-cust-txn-id").value = txnId;
  if (g("edit-cust-txn-date"))
    g("edit-cust-txn-date").value = txn.date || todayDateStr();
  if (g("edit-cust-txn-desc"))
    g("edit-cust-txn-desc").value = txn.itemName || "";
  if (g("edit-cust-txn-qty")) g("edit-cust-txn-qty").value = txn.qty || 1;
  if (g("edit-cust-txn-price"))
    g("edit-cust-txn-price").value =
      txn.type === "payment" ? txn.amount || 0 : txn.unitPrice || 0;
...
async function saveEditCustTxn() {
  ...
  if (txn.type === "payment") {
    if (price <= 0) {
      toast("Amount must be > 0", "err");
      return;
    }
    txn.amount = price;
  } else {
    if (!desc) {
      toast("Description required", "err");
      return;
    }
    if (price <= 0) {
      toast("Price must be > 0", "err");
      return;
    }
    txn.itemName = desc;
    txn.qty = qty;
  ...
  fbSyncCustTxn(txn);
  const upd = (await dbAll("customers")).find(
    (c) => c.customerId === txn.customerId,
  );
